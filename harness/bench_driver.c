...
int64_t arc_game_bench_steps(ArcGame *game, int32_t iters, uint32_t seed) {
    ...
    for (int32_t k = 0; k < iters; k++) {
        ...
        total += arc_game_step(game, action, frame, &reward, &terminated);
        if (terminated) arc_game_perform_action(game, ARC_ACTION_RESET, 0, 0);
    }
    return total;
}
...
void arc_game_bench_split(ArcGame *game, int32_t iters, uint32_t seed, int64_t *frames_out,
    ...
    for (int32_t k = 0; k < iters; k++) {
        ...
        if (game->engine.status == WIN || game->engine.status == GAME_OVER) {
            t0 = now_ns();
            count = arc_game_perform_action(game, ARC_ACTION_RESET, 0, 0);
            t1 = now_ns();
            arc_game_frame(game, frame);
            t2 = now_ns();
            logic_ns += t1 - t0;
...
int64_t arc_game_bench_parallel(ArcGame **games, int32_t num_threads, int32_t iters,
                            uint32_t seed) {
...
static void *round_robin_thread(void *arg) {
    ...
    uint8_t terminated;
    int64_t total = 0;
    for (int32_t i = 0; i < a->num_games; i++) arc_game_init(a->games[i]);
    for (int32_t k = 0; k < a->iters; k++) {
        ArcGame *g = a->games[k % a->num_games];
        int32_t action = (int32_t)(xorshift() % (uint32_t)g->num_actions);
        total += arc_game_step(g, action, frame, &reward, &terminated);
        if (terminated) arc_game_perform_action(g, ARC_ACTION_RESET, 0, 0);
    }
    a->result = total;
    return NULL;
}

int64_t arc_game_bench_envs(ArcGame **games, int32_t num_games, int32_t num_threads,
