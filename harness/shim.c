...
ArcWorld *arc_world_new(int32_t num_slots, int32_t num_tags, int32_t ph, int32_t pw,
                 const int8_t *pixels, const uint8_t *tags) {
    ArcWorld *world = calloc(1, sizeof(ArcWorld));
...
void arc_world_free(ArcWorld *world) {
    ArcSprites *s = &world->sprites;
...
void arc_world_set(ArcWorld *world, const int32_t *x, const int32_t *y,
...
void arc_world_render(ArcWorld *world, int32_t cx, int32_t cy, int32_t cw, int32_t ch,
                  int8_t background, int8_t letter_box, int8_t *out) {
    ArcCamera cam = {cx, cy, cw, ch, background, letter_box};
    arc_render(&world->sprites, &cam, &world->scratch, out);
}

int32_t arc_world_get_sprite_at(ArcWorld *world, int32_t x, int32_t y, int32_t tag,
                            int ignore_collidable) {
    return arc_get_sprite_at(&world->sprites, x, y, tag, ignore_collidable);
}

int arc_world_collides(ArcWorld *world, int32_t i, int ignore_mode) {
    return arc_collides(&world->sprites, i, ignore_mode);
}

void arc_world_set_pixels(ArcWorld *world, const int8_t *pixels) {
    ArcSprites *s = &world->sprites;
    const ArcAtlas *a = &world->atlas;
...
double arc_world_bench_render(ArcWorld *world, int32_t cx, int32_t cy, int32_t cw,
