...
static void ka59_display_to_grid(const Camera *camera, int32_t display_x, int32_t display_y,
...
static int ka59_push_chain(Sprites *s, const Ka59Static *st, int32_t level, int32_t slot,
    ...
    for (int32_t k = 0; k < border_n; k++) {
        if (collides_pair(s, slot, border[k], 0)) {
            s->x[slot] -= dx;
            s->y[slot] -= dy;
            return 1;
        }
    ...
    for (int32_t k = 0; k < occ_n; k++) {
        if (collides_pair(s, slot, occ[k], 0)) hit_slots[hit_n++] = occ[k];
    }
...
static int ka59_strip_touches(Sprites *s, const Ka59Static *st, int32_t level,
    ...
    memcpy(saved_patch, sprite_pixels(s, slot), area);
    ...
    int result = 0;
    for (int32_t k = 0; k < occ_n; k++) {
        if (collides_pair(s, slot, occ[k], 0)) {
            result = 1;
            break;
        }
    }
    ...
    memcpy(sprite_pixels_mut(s, slot), saved_patch, area);

    return result;
}

static void ka59_refresh_borders(Sprites *s, const Ka59Static *st, int32_t level,
                                 const Ka59Aux *aux) {
    ...
    int8_t *patch = sprite_pixels_mut(s, active);
...
static int ka59_targets_satisfied(const Sprites *s, const int32_t *container, int32_t container_n,
...
static int ka59_is_won(const Sprites *s, const Ka59Static *st, int32_t level) {
...
static int ka59_tick_fuses(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux) {
    size_t area = ka59_area(st);
    ...
    for (int32_t k = 0; k < bomb_n; k++) {
        ...
        const int8_t *frame_src = st->fuse_frames +
                                  (((idx * 2 + (size_t)cycle) * (size_t)(st->max_bomb_h + 1) +
                                    (size_t)p_index)) *
                                      area;
        int8_t *dst = sprite_pixels_mut(s, b);
...
static void ka59_tail_common(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux,
...
static void ka59_tail_with_tick(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux,
                                int32_t *score, int32_t *status, uint8_t *next_level,
                                uint8_t *action_complete) {
...
static void ka59_select_box(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux,
                            int32_t new_slot) {
    size_t area = ka59_area(st);
    int32_t old_slot = aux->active_box;

    int8_t *old_patch = sprite_pixels_mut(s, old_slot);
    ...
    int8_t *new_patch = sprite_pixels_mut(s, new_slot);
...
static void ka59_handle_click(Sprites *s, const Camera *camera, const Ka59Static *st, int32_t level,
    ...
    int32_t hit = get_sprite_at(s, wx, wy, st->box_tag, 0);
...
static int ka59_probe_move(Sprites *s, const Ka59Static *st, int32_t level, int32_t slot,
    ...
    for (int32_t k = 0; k < wall_n && !hard_hit; k++)
        if (collides_pair(s, slot, wall[k], 0)) hard_hit = 1;
    const int32_t *border = st->border_slots + (size_t)level * st->max_border;
    int32_t border_n = st->border_count[level];
    for (int32_t k = 0; k < border_n && !hard_hit; k++)
        if (collides_pair(s, slot, border[k], 0)) hard_hit = 1;
    ...
    for (int32_t k = 0; k < occ_n; k++) {
        int32_t o = occ[k];
        int hit = !hard_hit && collides_pair(s, slot, o, 0);
...
static void ka59_handle_move(Sprites *s, const Ka59Static *st, int32_t level, int32_t dir_idx,
    ...
    if (any_pushable) {
        ...
        int8_t *cpatch = sprite_pixels_mut(s, st->collider_slot);
        for (int32_t row = 0; row < ph; row++) {
            for (int32_t col = 0; col < pw; col++) {
                int strip;
                switch (dir_idx) {
...
static int ka59_retry_push_frame(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux) {
    const int32_t *occ = st->occupant_slots + (size_t)level * st->max_occupant;
    int32_t occ_n = st->occupant_count[level];
    ...
    for (int32_t k = 0; k < occ_n; k++) {
        ...
        for (int32_t w = 0; w < wall_n && !wall_hit; w++)
            if (collides_pair(s, i, wall[w], 0)) wall_hit = 1;
...
static void ka59_push_retry_dispatch(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux,
                                     int32_t *score, int32_t *status, uint8_t *next_level,
                                     uint8_t *action_complete) {
...
static void ka59_spawn_explosion_frame(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux,
                                       int32_t *next_order) {
    size_t area = ka59_area(st);
    ...
    for (int32_t bi = 0; bi < bomb_n; bi++) {
        ...
        int8_t *dst = sprite_pixels_mut(s, ex_slot);
        memcpy(dst, pix, area);
        s->x[ex_slot] = bx;
        s->y[ex_slot] = by;
        s->h[ex_slot] = eh;
        ...
        for (int32_t oi = 0; oi < occ_n; oi++) {
            int32_t o = occ[oi];
            if (!collides_pair(s, ex_slot, o, 0)) continue;
...
static void ka59_finalize_explosion(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux,
                                    int32_t *score, int32_t *status, uint8_t *next_level,
                                    uint8_t *action_complete) {
    size_t area = ka59_area(st);
    ...
    for (int32_t oi = 0; oi < occ_n; oi++) {
        ...
        int32_t wall_n = st->wall_count[level];
        for (int32_t w = 0; w < wall_n && !wall_hit; w++)
            if (collides_pair(s, o, wall[w], 0)) wall_hit = 1;
...
static void ka59_explosion_dispatch(Sprites *s, const Ka59Static *st, int32_t level, Ka59Aux *aux,
...
static void ka59_dispatch_action(Sprites *s, const Camera *camera, const Ka59Static *st,
...
void ka59_on_set_level(Sprites *sprites, const Ka59Static *st, int32_t level, Ka59Aux *aux,
                       int32_t *next_order) {
    *next_order = st->num_slots;
    ...
    size_t area = ka59_area(st);
    int8_t *patch = sprite_pixels_mut(sprites, active);
...
void ka59_step_once(Sprites *sprites, const Camera *camera, const Ka59Static *st, int32_t level,
