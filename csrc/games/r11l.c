...
enum { R11L_GUIDE_COLOUR = 1, R11L_GUIDE_TARGET_A = 5, R11L_GUIDE_TARGET_B = 10 };
...
static int32_t r11l_find_piece(const Sprites *s, const R11lStatic *st, int32_t level,
    ...
    for (int32_t rank = 0; rank < R11L_NPIECES; rank++) {
        ...
        if (slot < 0) continue;
        if (wx >= s->x[slot] && wx < s->x[slot] + s->w[slot] && wy >= s->y[slot] &&
...
static void r11l_select_piece(Sprites *s, R11lAux *aux, int32_t slot) {
    int32_t prev = aux->selected;
    if (prev >= 0) color_remap(s, prev, 1, R11L_PIECE_SELECTED, R11L_PIECE_IDLE);
    if (slot >= 0) color_remap(s, slot, 1, R11L_PIECE_IDLE, R11L_PIECE_SELECTED);
    aux->selected = slot;
}

static void r11l_recompute_group(Sprites *s, const R11lStatic *st, int32_t level, int32_t group) {
    int32_t base = level * R11L_G + group;
    int32_t count = st->group_piece_count[base];
    const int32_t *slots = st->group_piece_slots + (size_t)base * st->max_group_pieces;
    int32_t sx = 0, sy = 0;
    ...
    set_position(s, target_slot, tx, ty);
}

static void r11l_maybe_merge(Sprites *s, const R11lStatic *st, int32_t level, int32_t group) {
    int32_t base = level * R11L_G + group;
    if (!st->group_is_composite[base]) return;
    int32_t target_slot = st->group_target_slot[base];
    if (target_slot < 0) return;

    int32_t count = st->fragment_count[level];
    const int32_t *frags = st->fragment_slots + (size_t)level * st->max_fragments;
    int32_t area = s->atlas->ph * s->atlas->pw;
    for (int32_t k = 0; k < count; k++) {
        int32_t i = frags[k];
        if (!s->alive[i]) continue;
        if (!collides_pair(s, target_slot, i, 0)) continue;
        const int8_t *frag_px = sprite_pixels(s, i);
        int8_t *tgt_px = sprite_pixels_mut(s, target_slot);
        for (int32_t p = 0; p < area; p++) {
            int8_t v = frag_px[p];
            if (v != -1 && v != 0) tgt_px[p] = v;
        }
        remove_sprite(s, i);
    }
}

static int r11l_hazard_hit(const Sprites *s, const R11lStatic *st, int32_t level, int32_t group) {
    ...
    for (int32_t k = 0; k < count; k++)
        if (collides_pair(s, target_slot, hz[k], 0)) return 1;
    return 0;
}

static int r11l_key_matched(const Sprites *s, const R11lStatic *st, int32_t level, int32_t k) {
    ...
    int direct = has_direct && collides_pair(s, clue_c, tgt_c, 0);
    ...
    for (int32_t c = 0; c < R11L_NCOMP; c++) {
        ...
        if (!comp_active) continue;
        int coll = collides_pair(s, clue_c, comp_slot, 0);
        if (!coll) continue;
        uint8_t present[15];
        memset(present, 0, sizeof present);
        const int8_t *patch = sprite_pixels(s, comp_slot);
        for (int32_t p = 0; p < area; p++) {
            int8_t v = patch[p];
            if (v >= 1 && v <= 15) present[v - 1] = 1;
        }
        int eq = 1;
        for (int32_t cc = 0; cc < 15; cc++)
            if (present[cc] != clue_colours[cc]) { eq = 0; break; }
        comp_match = comp_match || eq;
    }
    return has_direct ? direct : comp_match;
}

static int r11l_win_check(const Sprites *s, const R11lStatic *st, int32_t level) {
    for (int32_t k = 0; k < R11L_K; k++) {
        int32_t base = level * R11L_K + k;
        int active = st->key_clue_slot[base] >= 0;
        int skip = st->key_skip_win[base];
        int need = active && !skip;
        if (need && !r11l_key_matched(s, st, level, k)) return 0;
    }
    return 1;
}

static int r11l_check_blocked(Sprites *s, const R11lStatic *st, int32_t level, int32_t sel,
                              int32_t tx, int32_t ty) {
    int32_t ox = s->x[sel], oy = s->y[sel];
    set_position(s, sel, tx, ty);
    int blocked = 0;
    ...
    for (int32_t k = 0; k < count && !blocked; k++)
        if (collides_pair(s, sel, walls[k], 0)) blocked = 1;
    set_position(s, sel, ox, oy);
    return blocked;
}

static void r11l_handle_click(Sprites *s, const Camera *cam, const R11lStatic *st, int32_t level,
                              int32_t action_id, int32_t action_x, int32_t action_y, R11lAux *aux,
                              uint8_t *action_complete) {
    int32_t scale, x_pad, y_pad;
    scale_and_offset(cam, &scale, &x_pad, &y_pad);
    int32_t dx = action_x - x_pad, dy = action_y - y_pad;
    int32_t gx = dx >= 0 ? dx / scale : -1;
    int32_t gy = dy >= 0 ? dy / scale : -1;
    int on_board = gx >= 0 && gy >= 0 && gx < cam->width && gy < cam->height &&
                   action_id == R11L_ACTION6;
...
static void r11l_update_key(Sprites *s, const R11lStatic *st, int32_t level, int32_t k,
                            R11lAux *aux, int32_t *next_order) {
    int32_t base = level * R11L_K + k;
    int32_t clue_slot = st->key_clue_slot[base];
    ...
    if (want_visible && !s->alive[icon_slot]) {
        set_position(s, icon_slot, s->x[clue_slot], s->y[clue_slot]);
        add_sprite(s, icon_slot, *next_order);
        *next_order += 1;
    }
    if (want_hidden && s->alive[icon_slot]) remove_sprite(s, icon_slot);
...
static void r11l_update_blinks(Sprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
                               int32_t *next_order) {
    for (int32_t k = 0; k < R11L_K; k++) r11l_update_key(s, st, level, k, aux, next_order);
}

static int r11l_any_key_blinking(const R11lAux *aux) {
    for (int32_t k = 0; k < R11L_K; k++)
        if (aux->key_blinking[k]) return 1;
    return 0;
}

static void r11l_wobble_tick(Sprites *s, const R11lStatic *st, R11lAux *aux, int32_t *next_order) {
    ...
    if (want_visible && !s->alive[slot]) {
        set_position(s, slot, aux->wobble_x, aux->wobble_y);
        add_sprite(s, slot, *next_order);
        *next_order += 1;
    }
    if (want_hidden && s->alive[slot]) remove_sprite(s, slot);

    if (t % 4 == 0) {
        int32_t b = aux->wobble_blinks + 1;
        aux->wobble_blinks = b;
        if (b >= 5) {
            if (s->alive[slot]) remove_sprite(s, slot);
            aux->wobbling = 0;
            aux->wobble_blinks = 0;
            aux->wobble_ticks = 0;
            aux->reverting = 1;
...
static void r11l_forward_tween(Sprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
    ...
    set_position(s, selected, aux->target_x, aux->target_y);
    aux->tween = 1;
    ...
    if (r11l_hazard_hit(s, st, level, group_c)) {
        ...
        if (hits >= R11L_MAX_HAZARD_HITS) {
            ...
            *action_complete = 1;
            ...
            int32_t target_slot = st->group_target_slot[level * R11L_G + group_c];
            aux->wobbling = 1;
            aux->wobble_blinks = 0;
            aux->wobble_ticks = 0;
...
static void r11l_reverse_tween(Sprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
                               uint8_t *action_complete) {
    int32_t selected = aux->selected;
    int32_t sel_c = selected >= 0 ? selected : 0;
    int32_t group = st->piece_group[(size_t)level * st->num_slots + sel_c];
    ...
    set_position(s, selected, aux->start_x, aux->start_y);
    aux->tween = 0;
    r11l_recompute_group(s, st, level, group_c);
    aux->dragging = 0;
    aux->reverting = 0;
...
static void r11l_continue_drag(Sprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
...
void r11l_on_set_level(Sprites *sprites, const R11lStatic *st, int32_t level, R11lAux *aux,
                       int32_t *next_order) {
    const int32_t *order = st->pieces_order + (size_t)level * R11L_NPIECES;
    ...
    for (int32_t rank = 1; rank < R11L_NPIECES; rank++) {
        int32_t slot = order[rank];
        if (slot < 0) continue;
        color_remap(sprites, slot, 1, R11L_PIECE_SELECTED, R11L_PIECE_IDLE);
    }
    for (int32_t g = 0; g < R11L_G; g++) {
        int32_t base = level * R11L_G + g;
        if (st->group_piece_count[base] >= 2) r11l_recompute_group(sprites, st, level, g);
    }
    r11l_zero_aux(aux);
    aux->selected = rank0;
...
void r11l_step_once(Sprites *sprites, const Camera *camera, const R11lStatic *st, int32_t level,
                    int32_t action_id, int32_t action_x, int32_t action_y, int32_t action_count,
                    R11lAux *aux, int32_t *next_order, int32_t *score, int32_t *status,
                    uint8_t *next_level, uint8_t *action_complete) {
    ...
    if (aux->dragging) {
        r11l_continue_drag(sprites, st, level, aux, next_order, score, status, next_level,
                           action_complete);
    } else {
        r11l_handle_click(sprites, camera, st, level, action_id, action_x, action_y, aux,
                          action_complete);
    }
}

static void r11l_draw_budget(int8_t *frame, int32_t action_count) {
    int32_t remaining = r11l_clampi(R11L_MAX_ACTIONS - action_count, 0, R11L_MAX_ACTIONS);
    int32_t total = FRAME_SIZE * remaining;
    ...
    for (int32_t r = 0; r < FRAME_SIZE; r++) {
        int8_t v = (FRAME_SIZE - r <= filled) ? R11L_BUDGET_FILLED : R11L_BUDGET_EMPTY;
        frame[(size_t)r * FRAME_SIZE] = v;
    }
}

static void r11l_draw_line(int8_t *frame, int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                           int active) {
    ...
    for (int32_t step = 0; step < R11L_LINE_STEPS; step++) {
        int inb = x >= 0 && x < FRAME_SIZE && y >= 0 && y < FRAME_SIZE;
        int32_t yc = r11l_clampi(y, 0, FRAME_SIZE - 1);
        int32_t xc = r11l_clampi(x, 0, FRAME_SIZE - 1);
        int8_t cur = frame[(size_t)yc * FRAME_SIZE + xc];
        int want = active && inb && (cur == R11L_GUIDE_TARGET_A || cur == R11L_GUIDE_TARGET_B);
        if (want) frame[(size_t)yc * FRAME_SIZE + xc] = R11L_GUIDE_COLOUR;
...
static void r11l_draw_guides(int8_t *frame, const Sprites *s, const Camera *cam,
                             const R11lStatic *st, int32_t level) {
    int32_t scale, x_off, y_off;
    scale_and_offset(cam, &scale, &x_off, &y_off);
    const int32_t *order = st->pieces_order + (size_t)level * R11L_NPIECES;

    for (int32_t rank = 0; rank < R11L_NPIECES; rank++) {
        int32_t i = order[rank];
        if (i < 0) continue;
        int32_t group = st->piece_group[(size_t)level * st->num_slots + i];
        int32_t group_c = group >= 0 ? group : 0;
        int32_t target_slot = st->group_target_slot[level * R11L_G + group_c];
        ...
        int32_t tx = (s->x[tgt_c] + s->w[tgt_c] / 2 - cam->x) * scale + x_off;
        int32_t ty = (s->y[tgt_c] + s->h[tgt_c] / 2 - cam->y) * scale + y_off;
        r11l_draw_line(frame, px, py, tx, ty, active);
    }
}

void r11l_render_interface(int8_t *frame, const Sprites *sprites, const Camera *camera,
