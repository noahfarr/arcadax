...
static void ls20_set_slot_visible(Sprites *sprites, int32_t slot, int visible) {
    if (slot < 0) return;
    set_visible(sprites, slot, visible);
}

static void ls20_set_tag_visible(Sprites *sprites, int32_t num_slots, int32_t tag_idx,
                                 int visible) {
    const Atlas *atlas = sprites->atlas;
    for (int32_t i = 0; i < num_slots; i++) {
        if (!sprites->alive[i]) continue;
        if (!sprites->tags[(size_t)i * atlas->num_tags + tag_idx]) continue;
        set_visible(sprites, i, visible);
    }
}

static void ls20_style_loadout(Sprites *sprites, const Ls20Static *st, int32_t level,
    ...
    memcpy(sprite_pixels_mut(sprites, htk), variant, area);
}

static void ls20_apply_hint(Sprites *sprites, const Ls20Static *st, int32_t level,
                            Ls20Aux *aux) {
    int any_hint = 0;
...
static int ls20_h_goal(Sprites *sprites, const Ls20Static *st, int32_t level,
    ...
    int mismatch = !(aux->shape_idx == st->want_shape[idx] &&
                     ...
                     aux->rot_idx == st->want_rot[idx]);
    if (mismatch) {
        color_remap(sprites, st->htk2_slot[level], 0, 0, LS20_COLOR_BLACK);
        aux->flash_frame = LS20_FLASH_FRAMES;
    }
    return mismatch;
}

static void ls20_h_refill(Sprites *sprites, const Ls20Static *st, int32_t level,
                          Ls20Aux *aux, int32_t slot) {
    remove_sprite(sprites, slot);
    aux->steps = st->budget[level];
}

static void ls20_h_button(Sprites *sprites, const Ls20Static *st, int32_t level,
...
static void ls20_process_target(Sprites *sprites, const Ls20Static *st, int32_t level,
...
static void ls20_step_patrols(Sprites *sprites, const Ls20Static *st, int32_t level,
    ...
    for (int32_t k = 0; k < LS20_MAX_PATROLS; k++) {
        ...
        const int8_t *area_pixels = sprite_pixels(sprites, aslot);
        ...
        if (found) set_position(sprites, pslot, px + best_dx, py + best_dy);
        aux->patrol_dir[k] = best_dir;
    }
}

static void ls20_undo_patrols(Sprites *sprites, const Ls20Static *st, int32_t level,
    ...
    for (int32_t k = 0; k < LS20_MAX_PATROLS; k++) {
        ...
        if (pslot >= 0) set_position(sprites, pslot, prev_x[k], prev_y[k]);
        aux->patrol_dir[k] = prev_dir[k];
    }
}

static void ls20_reset_patrols(Sprites *sprites, const Ls20Static *st, int32_t level,
                               Ls20Aux *aux) {
    for (int32_t k = 0; k < LS20_MAX_PATROLS; k++) {
        int32_t idx = level * LS20_MAX_PATROLS + k;
        int32_t pslot = st->patrol_slot[idx];
        if (pslot >= 0) set_position(sprites, pslot, st->patrol_start_x[idx], st->patrol_start_y[idx]);
        aux->patrol_dir[k] = 0;
    }
}

static void ls20_maybe_start_push(Sprites *sprites, const Ls20Static *st, int32_t level,
                                  Ls20Aux *aux) {
...
static int32_t ls20_restore_removables(Sprites *sprites, const Ls20Static *st, int32_t level,
                                       int32_t next_order) {
...
static void ls20_start_death(Sprites *sprites, const Ls20Static *st, int32_t level,
    ...
    int32_t player = st->player_slot[level];
    set_position(sprites, player, st->player_start_x[level], st->player_start_y[level]);
...
static void ls20_advance_death(Sprites *sprites, const Ls20Static *st, int32_t level,
...
static void ls20_advance_flash(Sprites *sprites, const Ls20Static *st, int32_t level,
                               Ls20Aux *aux, uint8_t *action_complete) {
    int32_t frame = aux->flash_frame - 1;
    aux->flash_frame = frame;
    if (frame > 0) return;
    color_remap(sprites, st->htk2_slot[level], 0, 0, LS20_COLOR_FOG);
    ls20_set_slot_visible(sprites, st->hint_glow_slot[level], 0);
    ls20_set_tag_visible(sprites, st->num_slots, st->hint_ring_tag, 0);
    *action_complete = 1;
}

static int ls20_check_goals(Sprites *sprites, const Ls20Static *st, int32_t level,
                            Ls20Aux *aux) {
    int32_t player = st->player_slot[level];
    int32_t px = sprites->x[player], py = sprites->y[player];
    for (int32_t g = 0; g < LS20_MAX_GOALS; g++) {
...
static void ls20_advance_push(Sprites *sprites, const Ls20Static *st, int32_t level,
                              Ls20Aux *aux, uint8_t *action_complete) {
    ...
    move_sprite(sprites, slot, dx, dy);
    int32_t player = st->player_slot[level];
    if (k <= LS20_PUSH_PUSH_PHASE) move_sprite(sprites, player, dx, dy);
...
static void ls20_play_move(Sprites *sprites, const Ls20Static *st, int32_t level,
    ...
    else set_position(sprites, player, target_x, target_y);
...
static void ls20_play(Sprites *sprites, const Ls20Static *st, int32_t level,
                      int32_t action_id, Ls20Aux *aux, int32_t *next_order,
                      int32_t *score, int32_t *status, uint8_t *next_level,
                      uint8_t *action_complete) {
...
void ls20_on_set_level(Sprites *sprites, const Ls20Static *st, int32_t level,
...
void ls20_step_once(Sprites *sprites, const Ls20Static *st, int32_t level,
                    int32_t action_id, Ls20Aux *aux, int32_t *next_order,
                    int32_t *score, int32_t *status, uint8_t *next_level,
                    uint8_t *action_complete) {
    if (aux->push_frame > 0) {
...
void ls20_render_interface(int8_t *frame, const Sprites *sprites, const Ls20Static *st,
    ...
    if (st->fog[level]) {
        ...
        for (int32_t r = 0; r < FRAME_SIZE; r++) {
            for (int32_t c = 0; c < FRAME_SIZE; c++) {
                ...
                if (a * a + b * b > 1600) frame[(size_t)r * FRAME_SIZE + c] = LS20_COLOR_FOG;
            }
        }
        int32_t htk = st->htk_slot[level];
        if (sprite_visible(sprites, htk)) {
            const int8_t *patch = sprite_pixels(sprites, htk);
            int32_t pw = sprites->atlas->pw;
            for (int32_t v = 0; v < 6; v++) {
                for (int32_t u = 0; u < 6; u++) {
                    int8_t pv = patch[(size_t)v * pw + u];
                    if (pv >= 0) frame[(size_t)(55 + v) * FRAME_SIZE + (3 + u)] = pv;
                }
            }
        }
    }
    ...
    for (int32_t c = 0; c < FRAME_SIZE; c++) {
        ...
        frame[61 * FRAME_SIZE + c] = colour;
        frame[62 * FRAME_SIZE + c] = colour;
    }

    for (int32_t b = 0; b < 3; b++) {
        int32_t x0 = 56 + 3 * b;
        int8_t colour = aux->lives > b ? LS20_COLOR_LIVES_FILLED : LS20_COLOR_STEP_EMPTY;
        frame[61 * FRAME_SIZE + x0] = colour;
        frame[61 * FRAME_SIZE + x0 + 1] = colour;
        frame[62 * FRAME_SIZE + x0] = colour;
        frame[62 * FRAME_SIZE + x0 + 1] = colour;
    }
}
