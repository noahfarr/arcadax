...
void s5i5_aux_alloc(S5i5Aux *aux, int32_t num_slots, int32_t ph, int32_t pw) {
    ...
    aux->backup_pixels = calloc((size_t)num_slots * (size_t)ph * (size_t)pw, 1);
}
...
void s5i5_zero_aux(S5i5Aux *aux, const S5i5Static *st) {
    ...
    memset(aux->backup_pixels, 0, (size_t)n * (size_t)st->ph * (size_t)st->pw);
}
...
static int8_t *sprite_pixels_mut_(Sprites *s, int32_t i) {
...
static int32_t rotation_of(const Sprites *s, int32_t slot) {
    const Atlas *a = s->atlas;
    ...
    if (bottom == S5I5_CAP_MARKER) return 0;
    if (left == S5I5_CAP_MARKER) return 90;
    if (top == S5I5_CAP_MARKER) return 180;
    return 270;
}

static void rot90(const int8_t *src, int8_t *dst, int32_t ph, int32_t pw, int32_t h, int32_t w) {
    for (int32_t r = 0; r < ph; r++) {
        int32_t src_col = w - 1 - r;
        if (src_col < 0) src_col = 0;
        if (src_col > pw - 1) src_col = pw - 1;
        for (int32_t c = 0; c < pw; c++) {
            int32_t src_row = c > ph - 1 ? ph - 1 : c;
            int valid = (r < w) && (c < h);
            dst[(size_t)r * pw + c] = valid ? src[(size_t)src_row * pw + src_col] : (int8_t)-1;
        }
    }
}

static void rotate_one(Sprites *s, int32_t ph, int32_t pw, int32_t anchor_x, int32_t anchor_y,
                       int32_t k) {
    ...
    rot90(sprite_pixels(s, k), rotated, ph, pw, hk, wk);
    set_position(s, k, new_x, new_y);
    s->h[k] = wk;
    s->w[k] = hk;
...
static void rotate_closure(Sprites *s, const S5i5Static *st, int32_t level, int32_t root) {
    const Atlas *a = s->atlas;
    ...
    int32_t off = st->desc_offset[base], cnt = st->desc_offset[base + 1] - off;
    for (int32_t k = 0; k < cnt; k++)
        rotate_one(s, ph, pw, anchor_x, anchor_y, st->desc_flat[off + k]);
}

static void resize_one(Sprites *s, const S5i5Static *st, int32_t level, int32_t root,
                       int32_t new_len) {
    const Atlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
    int32_t h0 = s->h[root], w0 = s->w[root];
    ...
    for (int32_t r = 0; r < ph; r++) {
        for (int32_t c = 0; c < pw; c++) {
            ...
            int marker;
            if (is0) marker = (r == new_len - 1) && (c < S5I5_PITCH);
            else if (is90) marker = (c == 0) && (r < S5I5_PITCH);
            else if (is180) marker = (r == 0) && (c < S5I5_PITCH);
            else marker = (c == new_len - 1) && (r < S5I5_PITCH);
            patch[(size_t)r * pw + c] = marker ? (int8_t)S5I5_CAP_MARKER
                                        : body ? (int8_t)colour
                                               : (int8_t)-1;
        }
    }
    ...
    move_sprite(s, root, root_dx, root_dy);
    s->h[root] = new_h;
    s->w[root] = new_w;
    ...
    int32_t off = st->desc_offset[base], cnt = st->desc_offset[base + 1] - off;
    for (int32_t k = 0; k < cnt; k++)
        move_sprite(s, st->desc_flat[off + k], dx0, dy0);
}

static void backup_one(Sprites *s, S5i5Aux *aux, int32_t k) {
    const Atlas *a = s->atlas;
    int32_t area = a->ph * a->pw;
    ...
    memcpy(aux->backup_pixels + (size_t)k * (size_t)area, sprite_pixels(s, k), (size_t)area);
}

static void backup_closure(Sprites *s, S5i5Aux *aux, const S5i5Static *st, int32_t level,
                           int32_t root) {
...
static void do_revert(Sprites *s, S5i5Aux *aux) {
    int32_t n = s->atlas->num_slots;
    int32_t area = s->atlas->ph * s->atlas->pw;
    for (int32_t k = 0; k < n; k++) {
        if (!aux->backup_valid[k]) continue;
        set_position(s, k, aux->backup_x[k], aux->backup_y[k]);
        s->h[k] = aux->backup_h[k];
        s->w[k] = aux->backup_w[k];
        int8_t *dst = sprite_pixels_mut_(s, k);
        memcpy(dst, aux->backup_pixels + (size_t)k * (size_t)area, (size_t)area);
    }
    aux->pending = 0;
    memset(aux->backup_valid, 0, (size_t)n);
}

static int pixel_overlap_(const Sprites *s, int32_t i, int32_t j) {
    const Atlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
    ...
    for (int32_t v = 0; v < ph; v++) {
        ...
        for (int32_t u = 0; u < pw; u++) {
            ...
            if (pi[(size_t)v * pw + u] != -1 && pj[(size_t)vv * pw + uu] != -1) return 1;
        }
    }
    return 0;
}

static int pair_collides(const Sprites *s, int32_t i, int32_t j) {
    ...
    int pp = s->blocking[i] == PIXEL_PERFECT || s->blocking[j] == PIXEL_PERFECT;
    if (pp && !pixel_overlap_(s, i, j)) return 0;
    return 1;
}

static int any_pipe_collision(const Sprites *s, const S5i5Static *st, int32_t level) {
    int32_t off = st->pipe_offset[level], cnt = st->pipe_offset[level + 1] - off;
    for (int32_t a = 0; a < cnt; a++) {
        int32_t i = st->pipe_flat[off + a];
        for (int32_t b = 0; b < cnt; b++) {
            int32_t j = st->pipe_flat[off + b];
            if (i == j) continue;
            if (pair_collides(s, i, j)) return 1;
        }
    }
    return 0;
}

static int check_win(const Sprites *s, const S5i5Static *st, int32_t level) {
    int32_t toff = st->target_offset[level], tcnt = st->target_offset[level + 1] - toff;
    int32_t coff = st->conn_offset[level], ccnt = st->conn_offset[level + 1] - coff;
    for (int32_t a = 0; a < tcnt; a++) {
        ...
        for (int32_t b = 0; b < ccnt; b++) {
            ...
            if (s->x[c] == s->x[t] && s->y[c] == s->y[t]) { ok = 1; break; }
        }
        if (!ok) return 0;
    }
    return 1;
...
static void engine_complete(S5i5Engine *e) { e->action_complete = 1; }

static void engine_next_level(S5i5Engine *e, int32_t num_levels) {
    e->score += 1;
    int is_last = e->level_index == num_levels - 1;
    e->next_level = is_last ? 0 : 1;
    if (is_last) e->status = S5I5_STATE_WIN;
}

static void engine_lose(S5i5Engine *e) { e->status = S5I5_STATE_GAME_OVER; }

static void finish(Sprites *s, S5i5Engine *e, S5i5Aux *aux, const S5i5Static *st) {
...
static void resolve_move(Sprites *s, S5i5Engine *e, S5i5Aux *aux, const S5i5Static *st,
                         int32_t level) {
...
static void process_root_rotation(Sprites *s, const S5i5Static *st, int32_t level, int32_t root) {
    ...
    if (parent >= 0) {
        ...
        if (diff < 0) diff = -diff;
        if (diff == 180) rotate_closure(s, st, level, root);
    }
    rotate_closure(s, st, level, root);
}

static void process_rotate(Sprites *s, S5i5Aux *aux, const S5i5Static *st, int32_t level,
                           int32_t handle) {
    int32_t clicked_color = st->handle_color[level * st->num_slots + handle];
    ...
    for (int32_t idx = 0; idx < cnt; idx++) {
        ...
        if (st->pipe_color[lbase + root] != clicked_color) continue;
        backup_closure(s, aux, st, level, root);
        process_root_rotation(s, st, level, root);
    }
}

static void display_to_grid(const Camera *cam, int32_t display_x, int32_t display_y,
                            int32_t *grid_x, int32_t *grid_y, int *valid) {
...
static void process_resize(Sprites *s, S5i5Aux *aux, const S5i5Static *st, const Camera *cam,
                           int32_t level, int32_t slider, int32_t action_x, int32_t action_y) {
    ...
    int32_t offset = horizontal ? world_x - s->x[slider] : world_y - s->y[slider];
    memset(aux->backup_valid, 0, (size_t)st->num_slots);

    int32_t base = level * st->num_slots + slider;
    int32_t off = st->slider_pipe_offset[base], cnt = st->slider_pipe_offset[base + 1] - off;
    for (int32_t k = 0; k < cnt; k++) {
        ...
        int32_t new_len = grow ? (index + 1) * S5I5_PITCH
                          : shrink ? (index - 1) * S5I5_PITCH
                                   : index * S5I5_PITCH;
        if (grow || shrink) resize_one(s, st, level, j, new_len);
    }
}

static void do_click(Sprites *s, S5i5Engine *e, S5i5Aux *aux, const S5i5Static *st,
                     const Camera *cam) {
    aux->steps = aux->steps > 0 ? aux->steps - 1 : 0;
    int32_t world_x, world_y, on_board;
    display_to_grid(cam, e->action_x, e->action_y, &world_x, &world_y, &on_board);
    if (!on_board) {
        finish(s, e, aux, st);
        return;
    }
    int32_t level = e->level_index;
    int32_t handle = get_sprite_at(s, world_x, world_y, st->handle_tag, 0);
    if (handle >= 0) {
        process_rotate(s, aux, st, level, handle);
        resolve_move(s, e, aux, st, level);
        return;
    }
    int32_t slider = get_sprite_at(s, world_x, world_y, st->slider_tag, 0);
    if (slider >= 0) {
        process_resize(s, aux, st, cam, level, slider, e->action_x, e->action_y);
        resolve_move(s, e, aux, st, level);
        return;
    }
    finish(s, e, aux, st);
...
void s5i5_step_once(Sprites *sprites, const Camera *camera, S5i5Engine *engine, S5i5Aux *aux,
                    const S5i5Static *st) {
    if (aux->pending) {
...
void s5i5_render_interface(int8_t *frame, const S5i5Aux *aux, const S5i5Static *st,
    ...
    if (2 * rest > budget) filled += 1;
    else if (2 * rest == budget && (whole % 2) == 1) filled += 1;
    int8_t *row = frame + (size_t)(FRAME_SIZE - 1) * FRAME_SIZE;
    for (int32_t c = 0; c < FRAME_SIZE; c++)
        row[c] = c < filled ? (int8_t)S5I5_BAR_FILLED : (int8_t)S5I5_BAR_EMPTY;
}
