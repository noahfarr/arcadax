...
static void ar25_display_to_grid(const Camera *camera, int32_t display_x,
...
static int ar25_hit_test(const Sprites *s, int32_t slot, int32_t x, int32_t y) {
    if (slot < 0) return 0;
    int32_t sx = s->x[slot], sy = s->y[slot], sw = s->w[slot], sh = s->h[slot];
    if (!(x >= sx && y >= sy && x < sx + sw && y < sy + sh)) return 0;
    int32_t pw = s->atlas->pw;
    int8_t pv = sprite_pixels(s, slot)[(size_t)(y - sy) * pw + (x - sx)];
    return pv != -1;
}

static void ar25_compute_layers(const Sprites *s, const Ar25Static *st,
    ...
    for (int32_t pi = 0; pi < 2; pi++) {
        ...
        if (slot < 0) continue;
        const int8_t *pixels = sprite_pixels(s, slot);
...
static void ar25_refresh_ghost_overlay(Sprites *s, const Ar25Static *st, int32_t level) {
    ...
    int8_t *buf = sprite_pixels_mut(s, st->ghost_slot);
...
static int ar25_check_win(const Sprites *s, const Ar25Static *st, int32_t level) {
    int8_t color_grid[AR25_GRID][AR25_GRID];
    int8_t owner_grid[AR25_GRID][AR25_GRID];
    uint8_t true_grid[AR25_GRID][AR25_GRID];
    ar25_compute_layers(s, st, level, color_grid, owner_grid, true_grid);
...
static void ar25_push_undo(Sprites *s, const Ar25Static *st, int32_t level, Ar25Aux *aux) {
    int32_t roles[4];
...
static void ar25_pop_undo(Sprites *s, const Ar25Static *st, int32_t level, Ar25Aux *aux) {
    ...
    for (int32_t r = 0; r < 4; r++) {
        if (roles[r] < 0) continue;
        set_position(s, roles[r], aux->undo_x[top][r], aux->undo_y[top][r]);
    }
    aux->undo_top = top;
    ar25_refresh_ghost_overlay(s, st, level);
}

static void ar25_move_selected(Sprites *s, const Ar25Static *st, int32_t level,
...
static void ar25_advance_hint_animation(Sprites *s, const Ar25Static *st, int32_t level,
    ...
    set_position(s, st->hint_slot, hint_x, piece_y);
...
static void ar25_click_select(Sprites *s, const Camera *camera, const Ar25Static *st,
...
void ar25_on_set_level(Sprites *sprites, const Ar25Static *st, int32_t level, Ar25Aux *aux) {
    ar25_zero_aux(aux);
    ...
    int8_t *gp = sprite_pixels_mut(sprites, ghost);
    ...
    add_sprite(sprites, ghost, st->num_slots);
    set_visible(sprites, ghost, 1);

    int8_t *hp = sprite_pixels_mut(sprites, hint);
    ...
    sprites->x[hint] = 500;
    sprites->y[hint] = 0;
    add_sprite(sprites, hint, st->num_slots + 1);
    set_visible(sprites, hint, 1);

    ar25_refresh_ghost_overlay(sprites, st, level);
}

void ar25_step_once(Sprites *sprites, const Camera *camera, const Ar25Static *st,
...
static void ar25_render_energy_bar(int8_t *frame, const Ar25Static *st, int32_t level,
    ...
    int8_t col[FRAME_SIZE];
    for (int32_t row = 0; row < FRAME_SIZE; row++) col[row] = frame[(size_t)row * FRAME_SIZE + FRAME_SIZE - 1];
    for (int32_t row = 0; row < FRAME_SIZE; row++)
        if (row >= within) col[row] = color;
    if (!is_last_tier && within > 0)
        for (int32_t row = 0; row < within; row++) col[row] = next_color;
    for (int32_t row = 0; row < FRAME_SIZE; row++) frame[(size_t)row * FRAME_SIZE + FRAME_SIZE - 1] = col[row];
}
...
static void ar25_paint_axis_slot(int8_t grid[AR25_GRID][AR25_GRID],
                                 ...
                                 const Sprites *s, const Ar25Static *st, int32_t level,
...
static void ar25_render_minimap(int8_t *frame, const Sprites *s, const Ar25Static *st,
    ...
    for (int32_t y = 0; y < AR25_GRID; y++) {
        for (int32_t x = 0; x < AR25_GRID; x++) {
            ...
            frame[(size_t)sy * FRAME_SIZE + sx] = grid[y][x];
        }
    }
}

void ar25_render_interface(int8_t *frame, const Sprites *sprites,
