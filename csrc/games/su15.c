...
static int su15_overlap(const Sprites *s, int32_t a, int32_t b) {
...
static void su15_display_to_grid(const Camera *camera, int32_t display_x, int32_t display_y,
...
static void su15_clear_glow(Sprites *s, const Su15Static *st) {
    int8_t *patch = sprite_pixels_mut(s, st->glow_slot);
    memset(patch, -1, (size_t)(st->ph * st->pw));
}

static void su15_draw_glow(Sprites *s, const Su15Static *st, const Su15Aux *aux) {
...
static void su15_apply_flash_color(Sprites *s, const Su15Static *st, const Su15Aux *aux, int lit) {
    int32_t area = st->ph * st->pw;
...
static void su15_begin_mismatch_flash(Sprites *s, const Su15Static *st, Su15Aux *aux, int32_t a,
                                      int32_t b) {
...
static void su15_merge_and_flash(Sprites *s, const Su15Static *st, Su15Aux *aux,
    ...
    for (int32_t k = 0; k < m; k++) {
        ...
        if (keep_as_new) {
            ...
            memcpy(sprite_pixels_mut(s, slot), pix, (size_t)(st->ph * st->pw));
            s->w[slot] = w;
            s->h[slot] = h;
            s->layer[slot] = layer;
            int32_t nx, ny;
...
static void su15_check_fruit_merge(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
                                   int32_t *next_order) {
...
static void su15_check_blob_merge(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
                                  int32_t *next_order) {
...
static void su15_check_blob_merge_from_settle(Sprites *s, const Su15Static *st, int32_t level,
...
static void su15_trigger_swallow(Sprites *s, Su15Aux *aux, int32_t fruit_slot, int32_t blob_slot) {
...
static void su15_swallow_scan(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux) {
    const int32_t *fruits = st->fruit_slot + (size_t)level * SU15_MAX_FRUITS;
    const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
...
static void su15_move_fruits_toward_target(Sprites *s, const Su15Static *st, int32_t level,
                                           Su15Aux *aux) {
    const int32_t *fruits = st->fruit_slot + (size_t)level * SU15_MAX_FRUITS;
    const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
...
static void su15_advance_swallow_animations(Sprites *s, const Su15Static *st, int32_t level,
                                            Su15Aux *aux) {
    const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
    for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
        int32_t i = blobs[k];
        ...
        if (promote_now) {
            ...
            memcpy(sprite_pixels_mut(s, i), pix, (size_t)(st->ph * st->pw));
            s->w[i] = w;
            s->h[i] = h;
        }
...
static void su15_settle_fruits(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
                               int32_t *next_order) {
    const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
    const int32_t *fruits = st->fruit_slot + (size_t)level * SU15_MAX_FRUITS;
...
static void su15_pull_move_one(Sprites *s, const Su15Static *st, const Su15Aux *aux, int32_t i) {
...
static void su15_pull_step(Sprites *s, const Su15Static *st, int32_t level, const Su15Aux *aux) {
    int frozen = su15_any_blob_in_move_phase(st, level, aux);
    const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
    const int32_t *fruits = st->fruit_slot + (size_t)level * SU15_MAX_FRUITS;
...
static int su15_pkrdtzfrth(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
                           int32_t *next_order) {
...
static int su15_win_check_point(const Sprites *s, const Su15Static *st, int32_t level,
                                const Su15Aux *aux) {
...
static int su15_win_check_weighted(const Sprites *s, const Su15Static *st, int32_t level,
                                   const Su15Aux *aux) {
    const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
...
static void su15_paint_zone_pieces(Sprites *s, const Su15Static *st, int32_t level, int32_t color) {
    ...
    for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
        int32_t z = za[k];
        if (z >= 0) color_remap(s, z, 0, 0, (int8_t)color);
    }
    for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
        int32_t z = zb[k];
        if (z >= 0) color_remap(s, z, 0, 0, (int8_t)color);
...
static void su15_restore_zone_pixels(Sprites *s, const Su15Static *st, int32_t level) {
    int32_t area = st->ph * st->pw;
    const int32_t *za = st->zone_a_slot + (size_t)level * SU15_MAX_ZONES;
    const int32_t *zb = st->zone_b_slot + (size_t)level * SU15_MAX_ZONES;
    for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
        int32_t z = za[k];
        if (z < 0) continue;
        memcpy(sprite_pixels_mut(s, z), st->level_pixels + ((size_t)level * st->num_slots + z) * area,
              (size_t)area);
    }
    for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
        int32_t z = zb[k];
...
static void su15_push_undo(Sprites *s, const Su15Static *st, Su15Aux *aux) {
...
static void su15_pop_undo(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
                          int32_t *next_order) {
...
static void su15_handle_tutorial_hide_if_moved(Sprites *s, const Su15Static *st, int32_t level,
                                               Su15Aux *aux) {
    if (level != 0) return;
...
static void su15_hide_tutorial_arrow_if_clicked(Sprites *s, const Su15Static *st, int32_t level,
...
static void su15_begin_pull(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux, int32_t gx,
                            int32_t gy) {
...
static void su15_advance_pulling(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
...
static void su15_advance_win_flash(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
...
static void su15_end_mismatch_flash(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
                                    int32_t *next_order, int32_t *status, uint8_t *action_complete) {
...
static void su15_advance_mismatch_flash(Sprites *s, const Su15Static *st, int32_t level, Su15Aux *aux,
...
static void su15_handle_idle_action(Sprites *s, const Camera *camera, const Su15Static *st,
...
void su15_on_set_level(Sprites *sprites, const Su15Static *st, int32_t level, Su15Aux *aux,
                       int32_t *next_order) {
    su15_zero_aux(aux);
    int32_t n = st->num_slots;
    ...
    int32_t slot = st->glow_slot;
    int8_t *patch = sprite_pixels_mut(sprites, slot);
    memset(patch, -1, (size_t)(st->ph * st->pw));
    sprites->w[slot] = SU15_GLOW_SIZE;
    sprites->h[slot] = SU15_GLOW_SIZE;
    sprites->x[slot] = 0;
    sprites->y[slot] = 0;
    add_sprite(sprites, slot, *next_order);
    set_visible(sprites, slot, 1);
    *next_order += 1;

    su15_push_undo(sprites, st, aux);
}

void su15_step_once(Sprites *sprites, const Camera *camera, const Su15Static *st, int32_t level,
...
void su15_render_interface(int8_t *frame, const Su15Static *st, int32_t level, const Su15Aux *aux) {
    ...
    if (budget <= 0) return;
    int32_t steps = aux->steps_remaining;
    int32_t a = -steps * FRAME_SIZE;
    int32_t filled = -su15_floor_div(a, budget);
    int32_t end;
    if (filled >= 0) {
        end = filled < FRAME_SIZE ? filled : FRAME_SIZE;
    } else {
        int32_t v = FRAME_SIZE + filled;
        end = v > 0 ? v : 0;
    }
