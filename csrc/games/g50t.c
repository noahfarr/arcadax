...
static int g50t_pixel_hit(const Sprites *s, int32_t slot, int32_t x, int32_t y, int solid) {
    ...
    int8_t pixel = sprite_pixels(s, slot)[(size_t)py * pw + px];
    int opaque = solid ? (pixel != -1) : (pixel >= 0);
    return inside && opaque;
}

static void g50t_start_death(Sprites *s, int32_t main_slot, const int32_t *death_slots) {
    ...
    set_position(s, piece0, s->x[main_slot] + offset_x, s->y[main_slot] + offset_y);
    set_interaction(s, piece0, TANGIBLE);
    set_interaction(s, main_slot, REMOVED);
}

static int g50t_in_bounds_and_open(const Sprites *s, const G50tStatic *st, int32_t level,
    ...
    for (int32_t i = 0; i < G50T_MAX_DOORS; i++) {
        ...
        if (slot < 0 || !sprite_visible(s, slot)) continue;
        if (g50t_pixel_hit(s, slot, cx, cy, 1)) return 0;
    }
    return 1;
}

static int32_t g50t_sensor_index_at(const Sprites *s, const G50tStatic *st, int32_t level,
                                    int32_t cx, int32_t cy) {
...
static void g50t_set_door(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
    ...
    int8_t center_px = sprite_pixels(s, slot)[(size_t)(h / 2) * s->atlas->pw + (w / 2)];
...
static void g50t_teleport(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                          int32_t portal_role) {
    ...
    for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++) {
        ...
        set_position(s, s_, nx, ny);
    }
}

static void g50t_set_portal(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static void g50t_forward_button(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static void g50t_update_sensor(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static void g50t_register(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                          int32_t tracked_index, int32_t cx, int32_t cy) {
    g50t_update_sensor(s, st, level, aux, tracked_index, cx, cy, 1);
}

static void g50t_deregister(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                            int32_t tracked_index, int32_t cx, int32_t cy) {
    g50t_update_sensor(s, st, level, aux, tracked_index, cx, cy, 0);
}

static int g50t_try_animated_move(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static int g50t_move_gen(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static int g50t_move_ice(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static int g50t_ice_in_boundary(const Sprites *s, const G50tStatic *st, int32_t level,
...
static void g50t_ice_slide_and_record(Sprites *s, const G50tStatic *st, int32_t level,
...
static void g50t_do_move(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static void g50t_pop_move(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux) {
    ...
    for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++) {
        ...
        set_interaction(s, ice_slot, TANGIBLE);
        int32_t r = rec - 1;
...
static void g50t_do_undo(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux) {
...
static void g50t_tick_gen(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                          int32_t g) {
    ...
    if (aux->dying[g]) {
        ...
        if (!finished) {
            ...
            set_position(s, nxt, x, y);
            set_interaction(s, nxt, TANGIBLE);
        }
    ...
    set_position(s, slot, new_x, new_y);
    aux->delay[g] = new_delay;
    aux->instant[g] = (uint8_t)new_instant;
    aux->active[g] = (uint8_t)busy;
}

static void g50t_tick_ice(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                          int32_t ii) {
...
static void g50t_tick_door(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                           int32_t i) {
    ...
    g50t_tick_mover(s->x[s_], s->y[s_], aux->door_target_x[i], aux->door_target_y[i],
                    ...
                    &new_instant, &busy);
    if (active) set_position(s, s_, new_x, new_y);
    ...
    for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++) {
        int32_t ice_slot = st->ice_slot[(size_t)level * G50T_MAX_ICE + ii];
        if (ice_slot < 0) continue;
        int collide = collides_pair(s, ice_slot, s_, 0);
...
static void g50t_tick_portal(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                             int32_t i) {
...
static void g50t_tick_ghost(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux) {
...
static void g50t_tick_all(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux) {
...
static void g50t_register_presence(Sprites *s, const G50tStatic *st, int32_t level,
                                   G50tAux *aux) {
    int32_t gen = aux->gen;
    int32_t pslot = st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + gen];
...
static void g50t_checkpoint_swap(Sprites *s, const G50tStatic *st, G50tAux *aux) {
    int32_t idx = aux->checkpoint_idx;
    if (idx == 0) {
        for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
            set_interaction(s, st->checkpoint_on_slot[k], REMOVED);
        for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
            set_interaction(s, st->checkpoint_off_slot[k], REMOVED);
    } else {
        int32_t prev = idx - 1;
        prev = prev < 0 ? 0 : (prev > G50T_MAX_CHECKPOINTS - 1 ? G50T_MAX_CHECKPOINTS - 1 : prev);
        set_interaction(s, st->checkpoint_on_slot[prev], REMOVED);
        set_interaction(s, st->checkpoint_off_slot[prev], TANGIBLE);
    }
    int32_t ci = idx < 0 ? 0 : (idx > G50T_MAX_CHECKPOINTS - 1 ? G50T_MAX_CHECKPOINTS - 1 : idx);
    set_interaction(s, st->checkpoint_on_slot[ci], TANGIBLE);
    aux->swap_pending = 0;
}

static void g50t_commit_checkpoint(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                                   int32_t *next_order) {
    int32_t gen = aux->gen;
    ...
    set_position(s, old_slot, spawn_x, spawn_y);

    if (is_last) {
        for (int32_t g = 0; g < G50T_PLAYER_GENERATIONS; g++) {
            if (!aux->echo[g]) continue;
            int32_t slot = st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + g];
            set_interaction(s, slot, REMOVED);
        }
        ...
        int8_t *old_px = sprite_pixels_mut(s, old_slot);
        ...
        int8_t *new_px = sprite_pixels_mut(s, new_slot);
        const int8_t *fresh = st->player_pixels + (size_t)level * area;
        memcpy(new_px, fresh, (size_t)area);

        set_position(s, new_slot, spawn_x, spawn_y);
        add_sprite(s, new_slot, *next_order);
        set_interaction(s, new_slot, TANGIBLE);
        *next_order += 1;
...
static void g50t_advance_checkpoint_or_undo(Sprites *s, const G50tStatic *st, int32_t level,
...
static void g50t_do_resolve(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
                            int32_t *next_order) {
    if (aux->swap_pending) {
...
static int g50t_is_lost(const Sprites *s, const G50tStatic *st, int32_t level,
                        const G50tAux *aux) {
...
static int g50t_is_won(const Sprites *s, const G50tStatic *st, int32_t level,
                       const G50tAux *aux) {
...
static void g50t_maybe_finish(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
static void g50t_dispatch(Sprites *s, const G50tStatic *st, int32_t level, int32_t action_id,
...
static void g50t_tick_then_resolve(Sprites *s, const G50tStatic *st, int32_t level, G50tAux *aux,
...
void g50t_on_set_level(Sprites *sprites, const G50tStatic *st, int32_t level, G50tAux *aux) {
    g50t_zero_aux(aux);
    ...
    for (int32_t g = 1; g < G50T_PLAYER_GENERATIONS; g++)
        set_interaction(sprites, st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + g], REMOVED);
    for (int32_t k = 0; k < G50T_DEATH_PIECES; k++)
        set_interaction(sprites, st->player_death_slot[k], REMOVED);
    for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++)
        for (int32_t k = 0; k < G50T_DEATH_PIECES; k++)
            set_interaction(sprites, st->ice_death_slot[(size_t)ii * G50T_DEATH_PIECES + k], REMOVED);
    for (int32_t pi = 0; pi < G50T_MAX_PORTALS; pi++)
        for (int32_t k = 0; k < G50T_CROSSFADE_PIECES; k++)
            set_interaction(sprites, st->portal_crossfade_slot[(size_t)pi * G50T_CROSSFADE_PIECES + k],
                            REMOVED);
    for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
        set_interaction(sprites, st->checkpoint_on_slot[k], REMOVED);
    for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
        set_interaction(sprites, st->checkpoint_off_slot[k], REMOVED);

    set_interaction(sprites, player_slot, TANGIBLE);
    if (st->checkpoint_count[level] > 0) set_interaction(sprites, st->checkpoint_on_slot[0], TANGIBLE);
...
void g50t_step_once(Sprites *sprites, const G50tStatic *st, int32_t level, int32_t action_id,
...
void g50t_render_interface(int8_t *frame, const Sprites *sprites, const Camera *camera,
