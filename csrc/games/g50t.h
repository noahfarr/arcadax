#ifndef ARCADAX_GAMES_G50T_H
#define ARCADAX_GAMES_G50T_H
...
void g50t_on_set_level(Sprites *sprites, const G50tStatic *st, int32_t level,
                       G50tAux *aux);

void g50t_step_once(Sprites *sprites, const G50tStatic *st, int32_t level,
                    int32_t action_id, int32_t action_x, int32_t action_y,
                    int32_t action_count, G50tAux *aux, int32_t *next_order,
                    int32_t *score, int32_t *status, uint8_t *next_level,
                    uint8_t *action_complete);

void g50t_render_interface(int8_t *frame, const Sprites *sprites,
                           const Camera *camera, const G50tStatic *st,
                           int32_t level, const G50tAux *aux);
