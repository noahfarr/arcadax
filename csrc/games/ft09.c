...
static int8_t *ft09_pixels_mut(Sprites *s, int32_t i) {
...
static void ft09_cell_at(const Sprites *s, const Ft09Static *st, int32_t x,
                         int32_t y, int32_t *slot, int *is_pattern) {
    int32_t plain = get_sprite_at(s, x, y, st->plain_tag, 0);
    int32_t pattern = get_sprite_at(s, x, y, st->pattern_tag, 0);
    int found_plain = plain >= 0;
    *slot = found_plain ? plain : pattern;
    *is_pattern = !found_plain && pattern >= 0;
}

static void ft09_advance_colour(Sprites *s, const Ft09Static *st, int32_t level,
                                int32_t slot) {
    int32_t pw = s->atlas->pw;
    int8_t current = sprite_pixels(s, slot)[pw + 1];
    ...
    color_remap(s, slot, 1, current, neu);
}

static void ft09_stamp(Sprites *s, const Ft09Static *st, int32_t level,
...
static int ft09_solved(const Sprites *s, const Ft09Static *st, int32_t level) {
    ...
    for (int32_t ci = 0; ci < count; ci++) {
        ...
        for (int32_t o = 0; o < 8; o++) {
            ...
            int8_t colour = sprite_pixels(s, target)[pw + 1];
...
static void ft09_animate_hint(Sprites *s, const Ft09Static *st, int32_t level,
    ...
    color_remap(s, slot, 0, 0, colour);
    aux->hint = hint;
    if (hint == 0) *action_complete = 1;
}

static void ft09_miss(const Sprites *s, const Ft09Static *st, int32_t level,
                      int32_t world_x, int32_t world_y, Ft09Aux *aux,
                      uint8_t *action_complete) {
    int32_t clue = get_sprite_at(s, world_x, world_y, st->clue_tag, 0);
...
static void ft09_resolve(Sprites *s, const Ft09Static *st, int32_t level,
...
static void ft09_play(Sprites *s, const Camera *camera, const Ft09Static *st,
...
void ft09_on_set_level(Sprites *sprites, const Ft09Static *st, int32_t level,
    ...
    for (int32_t i = 0; i < a->num_slots; i++) {
        ...
        if (is_plain) {
            int8_t plain_colour = sprite_pixels(sprites, i)[0];
            color_remap(sprites, i, 1, plain_colour, base);
        }
        if (is_pattern) {
...
void ft09_render_interface(int8_t *frame, const Ft09Static *st, int32_t level,
    ...
    for (int32_t col = 0; col < FRAME_SIZE; col++)
