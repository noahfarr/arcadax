...
static void sc25_recolor_all(Sprites *s, int32_t slot, int8_t colour) {
...
static void sc25_recolor_all_if(Sprites *s, int32_t slot, int8_t colour) {
    if (colour >= 0) sc25_recolor_all(s, slot, colour);
}

static void sc25_recolor_bounded(Sprites *s, int32_t slot, int8_t colour,
                                 const int32_t *box) {
...
static void sc25_compute_grid_bbox(Sc25Aux *aux, const Sprites *s, const Sc25Static *st,
                                   int32_t level) {
...
static void sc25_recolor_grid_cells(Sprites *s, const Sc25Static *st, int32_t level,
...
static void sc25_write_grid(Sprites *s, const Sc25Static *st, int32_t level,
...
static void sc25_recolour_grid(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux) {
...
static void sc25_fill_bar(Sprites *s, const Sc25Static *st, int32_t level, const Sc25Aux *aux) {
...
static void sc25_apply_overlay_visibility(Sprites *s, const Sc25Static *st, int32_t level,
                                          const Sc25Aux *aux) {
    int32_t a_slot = st->acyylh_ovl[level], s_slot = st->smzaik_ovl[level];
    if (a_slot >= 0) set_visible(s, a_slot, aux->scale2);
    if (s_slot >= 0) set_visible(s, s_slot, !aux->scale2);
}

static void sc25_reposition_overlay(Sprites *s, const Sc25Static *st, int32_t level,
                                    const Sc25Aux *aux) {
    ...
    set_position(s, a_slot, tx, ty);
}

static void sc25_spend_step(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
...
static void sc25_reset_drawn(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux) {
    memset(aux->drawn, 0, sizeof aux->drawn);
    sc25_recolour_grid(s, st, level, aux);
}

static void sc25_finish_effect(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                               int32_t *status, uint8_t *action_complete) {
...
static void sc25_apply_player_sprite(Sprites *s, const Sc25Static *st, int32_t level,
                                     Sc25Aux *aux) {
    ...
    memcpy(sprite_pixels_mut(s, slot), canvas, (size_t)ph * pw);
    s->h[slot] = h;
    s->w[slot] = w;
}
...
void sc25_on_set_level(Sprites *sprites, const Sc25Static *st, int32_t level, Sc25Aux *aux) {
...
static void sc25_step_slide(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                            int32_t *score, int32_t *status, uint8_t *next_level,
                            uint8_t *action_complete) {
    int32_t slot = st->pluyoo[level];
    if (aux->slide_frame > 0) {
        move_sprite(s, slot, aux->slide_dx, aux->slide_dy);
        aux->slide_frame -= 1;
    } else {
        aux->slide_active = 0;
...
static void sc25_step_miss(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                           uint8_t *action_complete) {
...
static void sc25_step_demo(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                           uint8_t *action_complete) {
...
static void sc25_start_teleport(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                                int32_t *status, uint8_t *action_complete) {
...
static void sc25_step_teleport(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                               int32_t *status, uint8_t *action_complete) {
    ...
    if (frame_next < SC25_TELEPORT_FRAMES) {
        ...
        memcpy(sprite_pixels_mut(s, target_slot), clean, (size_t)area);
...
static void sc25_blocked_offset(const Sc25Static *st, int32_t level, int32_t px, int32_t py,
    ...
    const uint8_t *block = st->grow_block + (size_t)level * FRAME_SIZE * FRAME_SIZE;
    for (int32_t k = 0; k < 9; k++) {
        int32_t ox = OFF[k][0], oy = OFF[k][1];
        int32_t nx = px + ox, ny = py + oy;
        if (nx < 0 || ny < 0 || nx + 4 > FRAME_SIZE || ny + 4 > FRAME_SIZE) continue;
        int hit = 0;
        for (int32_t dy = 0; dy < 4 && !hit; dy++)
            for (int32_t dx = 0; dx < 4; dx++)
                if (block[(size_t)(ny + dy) * FRAME_SIZE + (nx + dx)]) { hit = 1; break; }
...
static void sc25_start_resize(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux) {
...
static void sc25_collect_blockers(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux) {
    int32_t slot = st->pluyoo[level];
    int32_t px = s->x[slot], py = s->y[slot];
...
static void sc25_step_resize(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                             int32_t *status, uint8_t *action_complete) {
...
static void sc25_start_fireball(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux) {
    ...
    const int32_t *hit_grid = st->fb_hit_slot + (size_t)level * FRAME_SIZE * FRAME_SIZE;
    int32_t max_distance = 63;
    int32_t hit_slot = -1;
    for (int32_t ii = 1; ii < 64; ii++) {
        int32_t xs = origin_x + dx * ii, ys = origin_y + dy * ii;
        if (xs < 0 || xs >= FRAME_SIZE || ys < 0 || ys >= FRAME_SIZE) {
            max_distance = ii - 1;
            break;
        }
        int32_t hv = hit_grid[(size_t)ys * FRAME_SIZE + xs];
        if (hv >= 0) {
            max_distance = ii - 1;
            hit_slot = hv;
            break;
    ...
    sc25_rotate_pad(canvas, ph, pw, base, base_true_h, base_true_w, 0, rot_k, base_true_h,
                    base_true_w);
    memcpy(sprite_pixels_mut(s, fslot), canvas, (size_t)ph * pw);
    s->alive[fslot] = 1;
...
static void sc25_step_fireball(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                               int32_t *status, uint8_t *action_complete) {
    int32_t fslot = st->fireball_slot;
    ...
    if (!aux->fb_shrinking) {
        ...
        if (hit_kind == 1) {
            remove_sprite(s, st->tagsmh[level]);
            int32_t g = st->dosorb[level];
            if (g >= 0) remove_sprite(s, g);
        } else if (hit_kind == 2) {
            remove_sprite(s, st->seofsw_tagsmh[level]);
            int32_t g = st->seofsw_dosorb[level];
            if (g >= 0) remove_sprite(s, g);
        }
        remove_sprite(s, fslot);
        aux->fb_active = 0;
        sc25_finish_effect(s, st, level, aux, status, action_complete);
    }
}

static void sc25_step_cast(Sprites *s, const Sc25Static *st, int32_t level, Sc25Aux *aux,
                           int32_t *status, uint8_t *action_complete) {
...
static void sc25_display_to_grid(const Camera *camera, int32_t display_x, int32_t display_y,
...
static void sc25_grid_cell(Sprites *s, const Sc25Static *st, int32_t level, int32_t hit,
...
static void sc25_spell_select(Sprites *s, const Sc25Static *st, int32_t level, int32_t hit,
...
static void sc25_step_click(Sprites *s, const Camera *camera, const Sc25Static *st, int32_t level,
    ...
    int32_t hit = get_sprite_at(s, gx, gy, -1, 0);
...
static void sc25_step_move(Sprites *s, const Sc25Static *st, int32_t level, int32_t action_id,
                           Sc25Aux *aux, int32_t *status, uint8_t *action_complete) {
    int32_t facing = aux->facing;
    switch (action_id) {
    ...
    int32_t slot = st->pluyoo[level];
    int blocked = try_move(s, slot, mdx, mdy);
    int still_blocked = blocked;
    if (blocked && aux->scale2 && (abs(mdx) + abs(mdy) == 4))
        still_blocked = try_move(s, slot, mdx / 2, mdy / 2);
...
void sc25_step_once(Sprites *sprites, const Camera *camera, const Sc25Static *st, int32_t level,
...
void sc25_render_interface(int8_t *frame, const Sprites *sprites, const Camera *camera,
