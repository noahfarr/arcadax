...
{
	struct arc_vec_env *vec = reinterpret_cast<struct arc_vec_env *>(
		static_cast<intptr_t>(handle.typed_data()[0]));
	arc_vecenv_step(vec, actions.typed_data(), obs->typed_data(),
			reward->typed_data(),
...
XLA_FFI_DEFINE_HANDLER_SYMBOL(ArcStep, StepImpl,
			      ffi::Ffi::Bind()
			    ...
			    ffi::Result<ffi::Buffer<ffi::S8>> obs)
{
	struct arc_vec_env *vec = reinterpret_cast<struct arc_vec_env *>(
		static_cast<intptr_t>(handle.typed_data()[0]));
	arc_vecenv_reset(vec, obs->typed_data());
	return ffi::Error::Success();
}

XLA_FFI_DEFINE_HANDLER_SYMBOL(ArcReset, ResetImpl,
			      ffi::Ffi::Bind()
				      .Arg<ffi::Buffer<ffi::S64>>()
				      .Ret<ffi::Buffer<ffi::S8>>());
