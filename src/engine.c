...
static inline int32_t patch_area(const ArcAtlas *a) { return a->ph * a->pw; }
...
static inline int32_t sort_key_ascending(const ArcSprites *s, int32_t i) {
    return s->layer[i] * ARC_ORDER_BITS + s->order[i];
}

static inline int32_t sort_key_descending(const ArcSprites *s, int32_t i) {
...
void arc_raw_render(const ArcSprites *s, const ArcCamera *cam, ArcRenderScratch *scratch,
    ...
    for (int32_t i = 0; i < n; i++)
        if (arc_sprite_visible(s, i)) order[count++] = i;
    ...
    for (int32_t r = 0; r < ARC_FRAME_SIZE; r++) {
        ...
        for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) {
            ...
            frame[(size_t)r * ARC_FRAME_SIZE + c] =
                inside ? view[(size_t)clamp(gy, 0, ARC_FRAME_SIZE - 1) * ARC_FRAME_SIZE +
...
static inline int contains(const ArcSprites *s, int32_t i, int32_t x, int32_t y) {
    return x >= s->x[i] && y >= s->y[i] && x < s->x[i] + s->w[i] &&
           y < s->y[i] + s->h[i];
}

static inline int8_t pixel_at(const ArcSprites *s, int32_t i, int32_t x,
                              int32_t y) {
    const ArcAtlas *a = s->atlas;
...
int32_t arc_get_sprite_at(const ArcSprites *s, int32_t x, int32_t y, int32_t tag,
    ...
    for (int32_t i = 0; i < a->num_slots; i++) {
        ...
        if (!ignore_collidable && !arc_sprite_collidable(s, i)) continue;
...
static int pixel_overlap(const ArcSprites *s, int32_t i, int32_t j) {
    const ArcAtlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
...
int arc_collides_pair(const ArcSprites *s, int32_t i, int32_t j, int ignore_mode) {
    ...
    if (!(s->x[i] < s->x[j] + s->w[j] && s->x[i] + s->w[i] > s->x[j] &&
        ...
        return 0;
    if (!ignore_mode) {
        if (!arc_sprite_collidable(s, j) || !arc_sprite_collidable(s, i)) return 0;
...
int arc_collides(const ArcSprites *s, int32_t i, int ignore_mode) {
    ...
    for (int32_t j = 0; j < a->num_slots; j++) {
        ...
        if (!(xi < s->x[j] + s->w[j] && xi + wi > s->x[j] &&
            ...
            continue;
        if (!ignore_mode) {
            if (!arc_sprite_collidable(s, j) || !arc_sprite_collidable(s, i)) continue;
...
void arc_remove_sprite(ArcSprites *s, int32_t i) { s->alive[i] = 0; }
