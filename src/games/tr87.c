...
static void tr87_place_cursor_sprites(ArcSprites *sprites, const Tr87Static *st, int32_t level,
                                      const Tr87Aux *aux) {
    ...
    memcpy(arc_sprite_pixels_mut(sprites, top), st->cursor_patch_top + (size_t)width_id * area, area);
    memcpy(arc_sprite_pixels_mut(sprites, bottom), st->cursor_patch_bottom + (size_t)width_id * area,
           area);
}
...
static void tr87_cycle_selected_unit(ArcSprites *sprites, const Tr87Static *st, int32_t level,
...
static void tr87_move_cursor(ArcSprites *sprites, const Tr87Static *st, int32_t level,
                             int32_t action_id, Tr87Aux *aux, int32_t *status,
                             uint8_t *action_complete) {
...
static void tr87_play(ArcSprites *sprites, const Tr87Static *st, int32_t level, int32_t action_id,
...
static void tr87_advance_win_animation(ArcSprites *sprites, const Tr87Static *st, int32_t level,
    ...
    if (substep == 1) {
        ...
        for (int32_t i = 0; i < TR87_MAX_HIGHLIGHTS; i++) {
            ...
            memcpy(arc_sprite_pixels_mut(sprites, pool_slot), st->highlight_patch, area);
        }
    }
...
void tr87_on_set_level(ArcSprites *sprites, const Tr87Static *st, int32_t level, Tr87Aux *aux) {
...
void tr87_step_once(ArcSprites *sprites, const Tr87Static *st, int32_t level, int32_t action_id,
...
void tr87_render_interface(int8_t *frame, const Tr87Static *st, int32_t level,
    ...
    for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) row[c] = (int8_t)(c < filled ? 1 : 4);
}
