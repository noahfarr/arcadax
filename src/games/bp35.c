...
static void seq_init(struct seq *s, int32_t start)
{
	s->offset = 0;
	s->cur = start;
...
static void seq_move(struct seq *s, int32_t dur, int32_t target,
		     const float *table, int combine_delta)
{
	if (dur > 0) {
		int32_t start = s->cur;
		...
		for (int32_t k = s->offset; k < BP35_MAX_FRAMES; k++) {
			...
			if (idx >= dur) {
				...
				if (combine_delta)
					val = start +
					      (int32_t)truncf(delta * t);
				else
					val = (int32_t)truncf((float)start +
							      delta * t);
			}
			s->track[k] = val;
		}
...
static void seq_hold(struct seq *s, int32_t dur)
{
	if (dur > 0) {
		for (int32_t k = s->offset; k < BP35_MAX_FRAMES; k++)
			s->track[k] = s->cur;
...
static void seq_discrete(struct seq *s, int32_t dur, const int32_t *values,
			 int32_t d)
{
	if (dur > 0) {
		for (int32_t k = s->offset; k < BP35_MAX_FRAMES; k++) {
...
static void splice(int32_t *out, const int32_t *a_track, int32_t a_dur,
		   const int32_t *b_track)
{
	for (int32_t k = 0; k < BP35_MAX_FRAMES; k++) {
		if (k < a_dur) {
			out[k] = a_track[k];
...
static void classify(const uint8_t *wall_level, const uint8_t *spike_level,
		     const uint8_t *jcyh_now, const int8_t *kind, int32_t x,
		     int32_t y, int32_t W, int32_t H, int32_t *k, int *is_wall,
		     int *is_spike, int *is_jcyh)
{
...
{
	return !is_wall && !is_spike && !is_jcyh &&
	       (k == BP35_NONE || k == BP35_BOMB_SAFE);
}

static void fall_from(const uint8_t *wall_level, const uint8_t *spike_level,
		      const uint8_t *jcyh_now, const int8_t *kind, int32_t x0,
		      int32_t y0, int32_t dy, int32_t W, int32_t H,
		      int32_t *steps_out, int32_t *land_y_out, int *gem_out,
		      int *spk_out)
{
	int active = 1;
	int32_t steps = 0;
	...
	for (int32_t i = 0; i < BP35_HMAX && active; i++) {
		...
		int32_t k;
		int iw, isk, ij;
		classify(wall_level, spike_level, jcyh_now, kind, x0, cand_y, W,
			 H, &k, &iw, &isk, &ij);
...
static void jcyh_now_grid(const struct bp35_static *st, int32_t level,
			  int32_t shift, uint8_t *out)
{
	const uint8_t *ceil_w0_level =
		st->ceil_w0 + (size_t)level * BP35_HMAX * BP35_WMAX;
	for (int32_t r = 0; r < BP35_HMAX; r++) {
		...
		for (int32_t x = 0; x < BP35_WMAX; x++)
			out[(size_t)r * BP35_WMAX + x] =
				ok ? ceil_w0_level[(size_t)srci * BP35_WMAX +
						   x] :
				     0;
	}
}

static void push_history(struct bp35_aux *aux)
{
...
static void recipe_static(const struct bp35_aux *in, struct bp35_aux *out)
{
	...
	*out = *in;
	...
	for (int32_t i = 0; i < 5; i++) {
		...
		out->cell_y[i] = -1;
		for (int32_t k = 0; k < BP35_MAX_FRAMES; k++)
			out->traj_cell_img[i][k] = -1;
	}
	out->win = 0;
	out->lose = 0;
}

static void recipe_undo(const struct bp35_aux *in, struct bp35_aux *out)
{
	struct bp35_aux restored = *in;
...
static void recipe_move(const struct bp35_aux *in, const struct bp35_static *st,
			int32_t level, int32_t W, int32_t H,
			const uint8_t *wall_level, const uint8_t *spike_level,
			const uint8_t *jcyh_now, int32_t dx,
			struct bp35_aux *out)
{
	int facing = dx > 0;
	...
	int32_t k;
	int iw, isk, ij;
	classify(wall_level, spike_level, jcyh_now, in->kind, tx, ty, W, H, &k,
		 &iw, &isk, &ij);
	...
	int gem, spk;
	fall_from(wall_level, spike_level, jcyh_now, in->kind, tx, ty,
		  gravity_dy, W, H, &extra, &land_y, &gem, &spk);
	int32_t land_x = tx;
	int settling = extra == 0;

	int ceil_present =
		st->terrain_present[(size_t)level * BP35_NUM_TERRAIN + 1];
	int ceil_trigger = ceil_present && (step_par % 2) == 0;
	int32_t ceil_m_y_now =
		st->terrain_anchor[((size_t)level * BP35_NUM_TERRAIN + 1) * 2 +
				   1] -
		in->ceiling_shift;
	...
	seq_move(&ceil_seq, ceil_trigger ? 3 : 0, ceil_base - 6, st->ease_lin,
		 1);
	seq_hold(&ceil_seq, ceil_trigger ? 1 : 0);

	struct seq win_x, win_y;
	...
	struct seq stl_full_x, stl_full_y, stl_par_x, stl_par_y;
	...
	int32_t death_img[10] = {
		...
		-1, BP35_IMG_player_right_2, BP35_IMG_player_right_2,
		-1,
	};
	...
	struct seq fall_x, fall_y, fall_pimg;
	...
	int32_t pan_dur_raw =
		iabs32(floor_div(cam_target - in->cam_y, BP35_PITCH)) * 3;
	...
	int32_t new_ceiling_shift =
		in->ceiling_shift + (mylefxfaev_called && ceil_trigger ? 1 : 0);
	int32_t total_frames =
		is_win_t ? win_frames :
			   (is_bump ? bmp_frames :
				      (is_fall ? fall_frames : stl_frames));
	...
	*out = *in;
	...
	for (int32_t i = 0; i < BP35_MAX_FRAMES; i++) {
		out->traj_px[i] = is_win_t     ? win_x.track[i] :
				  ...
				  ceil_trigger ? stl_par_x.track[i] :
						 stl_full_x.track[i];
		out->traj_py[i] = is_win_t     ? win_y.track[i] :
				  is_bump      ? bmp_py_const :
				  is_fall      ? fall_y.track[i] :
				  ceil_trigger ? stl_par_y.track[i] :
						 stl_full_y.track[i];
		out->traj_pimg[i] = is_fall ? fall_pimg.track[i] : facing_img_v;
		out->traj_cam[i] = is_fall ? fall_cam.track[i] : in->cam_y;
		out->traj_ceil_dy[i] =
			mylefxfaev_called ? ceil_seq.track[i] : ceil_base;
	}
	for (int32_t i = 0; i < 5; i++) {
		out->cell_x[i] = -1;
		out->cell_y[i] = -1;
		for (int32_t k2 = 0; k2 < BP35_MAX_FRAMES; k2++)
			out->traj_cell_img[i][k2] = -1;
	}
	out->win = (uint8_t)win_flag;
	out->lose = (uint8_t)lose_flag;
}

static void recipe_click(const struct bp35_aux *in,
			 const struct bp35_static *st, int32_t W, int32_t H,
			 const uint8_t *wall_level, const uint8_t *spike_level,
			 const uint8_t *jcyh_now, int32_t screen_x,
			 int32_t screen_y, struct bp35_aux *out)
{
	...
	int32_t kind_at =
		in_bounds ? in->kind[(size_t)kyi * BP35_WMAX + kxi] : BP35_NONE;
	...
	int is_active = is_breakable || is_switch || is_bomb_arm ||
			is_bomb_safe || is_spread;
	...
	int32_t new_here =
		(is_breakable || is_switch || is_spread) ?
			BP35_NONE :
			(is_bomb_arm ?
				 BP35_BOMB_SAFE :
				 (is_bomb_safe ? BP35_BOMB_ARMED : existing));
	...
	for (int i = 0; i < 4; i++) {
		...
		int32_t nk;
		int nw, nsk, nj;
		classify(wall_level, spike_level, jcyh_now, in->kind, nx, ny, W,
			 H, &nk, &nw, &nsk, &nj);
		...
		n_y[i] = ny;
		n_valid[i] = valid;
		int32_t nxi = clampi(nx, 0, BP35_WMAX - 1),
			nyi = clampi(ny, 0, BP35_HMAX - 1);
		if (valid)
			kind_after[(size_t)nyi * BP35_WMAX + nxi] = BP35_SPREAD;
	}
	...
	int32_t k2;
	int iw2, isk2, ij2;
	classify(wall_level, spike_level, jcyh_now, in->kind, in->px, ey, W, H,
		 &k2, &iw2, &isk2, &ij2);
	int is_solid_indiv = k2 == BP35_SWITCH || k2 == BP35_BOMB_ARMED ||
			     k2 == BP35_BREAKABLE || k2 == BP35_SPREAD;
	int blocked = iw2 || (qssroarxob && is_solid_indiv);

	int32_t extra, land_y;
	int gem, spk;
	fall_from(wall_level, spike_level, jcyh_now, in->kind, in->px, ey, dy,
		  W, H, &extra, &land_y, &gem, &spk);
	...
	int32_t death_img[3] = { BP35_IMG_player_left_0, BP35_IMG_player_left_1,
				 BP35_IMG_player_left_1 };
	int32_t cam_target = land_y * BP35_PITCH - 31 + tprcybckbl;
	int32_t pan_dur_raw =
		iabs32(floor_div(cam_target - in->cam_y, BP35_PITCH)) * 3;
	int32_t pan_dur = pan_dur_raw < 20 ? pan_dur_raw : 20;
	...
	int32_t land_xs[3] = { land_x * BP35_PITCH, land_x * BP35_PITCH,
			       land_x * BP35_PITCH };
	int32_t land_ys[3] = { land_y * BP35_PITCH, land_y * BP35_PITCH,
			       land_y * BP35_PITCH };

	struct seq real_x, real_y, real_pimg, real_cam;
	...
	int32_t sel_dur =
		!proceed ? noop_dur : (blocked ? blocked_dur : real_dur);
	...
	int32_t sel_new_cam =
		proceed ? (blocked ? recenter_target : cam_target) : in->cam_y;

	int32_t cell0_track[BP35_MAX_FRAMES];
	{
		struct seq s;
		if (is_breakable) {
			int32_t vals[4] = { BP35_IMG_fijhgcrvsfx,
					    BP35_IMG_ucflxtuuxln,
					    BP35_IMG_xqapkpdjuet, -1 };
			seq_init(&s, -1);
			seq_discrete(&s, 4, vals, 4);
		} else if (is_switch) {
			int32_t vals[4] = { BP35_IMG_ebjoowkheai,
					    BP35_IMG_gnqvqkdqlpt,
					    BP35_IMG_ippnakjmssl, -1 };
			seq_init(&s, -1);
			seq_discrete(&s, 4, vals, 4);
		} else if (is_bomb_arm) {
			int32_t vals[4] = { BP35_IMG_txjcfisalqu,
					    BP35_IMG_cvkgqlojfnh,
					    BP35_IMG_ltorejwifje,
					    BP35_IMG_oonshderxef };
			seq_init(&s, -1);
			seq_discrete(&s, 4, vals, 4);
			seq_hold(&s, 1);
		} else if (is_bomb_safe) {
			int32_t vals[4] = { BP35_IMG_ltorejwifje,
					    BP35_IMG_cvkgqlojfnh,
					    BP35_IMG_txjcfisalqu,
					    BP35_IMG_yuuqpmlxorv };
			seq_init(&s, -1);
			seq_discrete(&s, 4, vals, 4);
			seq_hold(&s, 1);
		} else if (is_spread) {
			int32_t vals[4] = { BP35_IMG_wpulgmixnbz,
					    BP35_IMG_hihodtibubm,
					    BP35_IMG_yxaxjsryovv, -1 };
			seq_init(&s, -1);
			seq_discrete(&s, 4, vals, 4);
		} else {
			seq_init(&s, -1);
	...
	int32_t neighbour_track[BP35_MAX_FRAMES];
	{
		...
		if (is_spread)
			memcpy(neighbour_track, s.track,
			       sizeof neighbour_track);
		else
			for (int32_t i = 0; i < BP35_MAX_FRAMES; i++)
				neighbour_track[i] = -1;
	}
	...
	int32_t total_frames_active =
		swap_total + sel_dur + (is_spread ? 1 : 0);

	int32_t player_hold_x[BP35_MAX_FRAMES], player_hold_y[BP35_MAX_FRAMES];
	int32_t player_hold_img[BP35_MAX_FRAMES],
		player_hold_cam[BP35_MAX_FRAMES];
	...
	for (int32_t i = 0; i < BP35_MAX_FRAMES; i++) {
		...
		sel_px[i] = !proceed ?
				    noop_px_c :
				    (blocked ? blocked_px_c : real_x.track[i]);
		sel_py[i] = !proceed ?
				    noop_py_c :
				    (blocked ? blocked_py_c : real_y.track[i]);
		sel_pimg[i] = !proceed ? noop_pimg_c :
					 (blocked ? blocked_pimg_c :
						    real_pimg.track[i]);
		sel_cam[i] = !proceed ? noop_cam_c :
					(blocked ? blocked_cam.track[i] :
						   real_cam.track[i]);
	}
	...
	*out = *in;
	if (select)
		memcpy(out->kind, kind_after, sizeof out->kind);
	out->gravity_down =
		(uint8_t)(is_switch ? gravity_now : in->gravity_down);
	out->step_par = step_par;
	...
	if (select) {
		...
		memcpy(out->traj_pimg, statics.traj_pimg,
		       sizeof statics.traj_pimg);
		memcpy(out->traj_cam, statics.traj_cam,
		       sizeof statics.traj_cam);
	}
	memcpy(out->traj_ceil_dy, statics.traj_ceil_dy,
	       sizeof statics.traj_ceil_dy);
	if (select) {
		memcpy(out->cell_x, active_cell_x, sizeof active_cell_x);
		memcpy(out->cell_y, active_cell_y, sizeof active_cell_y);
		for (int i = 0; i < 5; i++) {
			const int32_t *src =
				i == 0 ? cell0_track : neighbour_track;
		...
		memcpy(out->traj_cell_img, statics.traj_cell_img,
		       sizeof statics.traj_cell_img);
	}
	out->win = (uint8_t)(select && sel_win);
	out->lose = (uint8_t)(select && sel_lose);
}

static void dispatch(const struct bp35_static *st, int32_t level,
		     int32_t action_id, int32_t action_x, int32_t action_y,
		     struct bp35_aux *aux)
{
	int32_t W = st->level_w[level], H = st->level_h[level];
	const uint8_t *wall_level =
		st->wall + (size_t)level * BP35_HMAX * BP35_WMAX;
	const uint8_t *spike_level =
		st->spike + (size_t)level * BP35_HMAX * BP35_WMAX;
	...
	struct bp35_aux snapshot = *aux;
	int32_t dx = action_id == 4 ? 1 : -1;

	if (is_move) {
		recipe_move(&snapshot, st, level, W, H, wall_level, spike_level,
			    jcyh_now, dx, aux);
	} else if (is_click) {
		recipe_click(&snapshot, st, W, H, wall_level, spike_level,
			     jcyh_now, action_x, action_y, aux);
	} else if (is_undo) {
...
static void compose(struct arc_scene_table *scene, const struct bp35_static *st,
		    int32_t level, const int8_t *kind, int32_t px, int32_t py,
		    int32_t pimgidx, int32_t cam_y, int32_t ceil_dy,
		    const int32_t *cell_x, const int32_t *cell_y,
		    const int32_t *cellimgidx)
{
	static const int is_ceiling_slot[BP35_NUM_TERRAIN] = { 0, 1, 1, 0, 0 };
	static const int32_t kind_idle_imgidx[7] = {
		-1,
		BP35_IMG_qclfkhjnaac,
		BP35_IMG_lrpkmzabbfa,
		BP35_IMG_yuuqpmlxorv,
		BP35_IMG_oonshderxef,
		BP35_IMG_etlsaqqtjvn,
		BP35_IMG_fjlzdjxhant,
	};
	...
	for (int32_t y = 0; y < BP35_HMAX; y++) {
		for (int32_t x = 0; x < BP35_WMAX; x++) {
			...
			int32_t imgidx = active_mask[idx] ?
						 -1 :
						 kind_idle_imgidx[kind[idx]];
			scene->image[idx] =
				imgidx >= 0 ? st->img_atlas[imgidx] : -1;
			scene->x[idx] = x * BP35_PITCH;
			scene->y[idx] = y * BP35_PITCH - cam_y;
			scene->layer[idx] =
				imgidx >= 0 ? st->img_layer[imgidx] : 0;
		}
	}

	scene->image[BP35_PLAYER_SLOT] =
		pimgidx >= 0 ? st->img_atlas[pimgidx] : -1;
	scene->x[BP35_PLAYER_SLOT] = px;
	scene->y[BP35_PLAYER_SLOT] = py - cam_y;
	scene->layer[BP35_PLAYER_SLOT] =
		pimgidx >= 0 ? st->img_layer[pimgidx] : 0;
	...
	for (int i = 0; i < BP35_NUM_TERRAIN; i++) {
		...
		scene->image[BP35_TERRAIN_SLOT0 + i] =
			present ? st->terrain_atlas[idx] : -1;
		scene->x[BP35_TERRAIN_SLOT0 + i] = anchor_x * BP35_PITCH;
		scene->y[BP35_TERRAIN_SLOT0 + i] =
			anchor_y * BP35_PITCH - cam_y + ceil_offset;
		scene->layer[BP35_TERRAIN_SLOT0 + i] = st->terrain_layer[idx];
	}

	for (int i = 0; i < 5; i++) {
		int32_t ci = cellimgidx[i];
		scene->image[BP35_ANIM_SLOT0 + i] =
			ci >= 0 ? st->img_atlas[ci] : -1;
		scene->x[BP35_ANIM_SLOT0 + i] = cell_x[i] * BP35_PITCH;
		scene->y[BP35_ANIM_SLOT0 + i] = cell_y[i] * BP35_PITCH - cam_y;
		scene->layer[BP35_ANIM_SLOT0 + i] =
			ci >= 0 ? st->img_layer[ci] : 0;
	}

	for (int32_t i = 0; i < BP35_NUM_SLOTS; i++)
		scene->order[i] = i;
	scene->dirty = 1;
}

void bp35_zero_aux(struct bp35_aux *aux)
{
	...
	for (int32_t i = 0; i < 5; i++) {
		...
		aux->cell_y[i] = -1;
		for (int32_t k = 0; k < BP35_MAX_FRAMES; k++)
			aux->traj_cell_img[i][k] = -1;
	}
}

void bp35_build_level(struct arc_scene_table *scene,
		      const struct bp35_static *st, int32_t level,
		      int32_t action_id, int32_t action_x, int32_t action_y,
		      uint8_t next_level, struct bp35_aux *aux)
{
	int32_t px = st->player_start[level * 2 + 0];
	int32_t py = st->player_start[level * 2 + 1];
	int32_t cam_y = py * BP35_PITCH - 31 - 5;

	struct bp35_aux fresh;
	bp35_zero_aux(&fresh);
	memcpy(fresh.kind, st->kind0 + (size_t)level * BP35_HMAX * BP35_WMAX,
	       sizeof fresh.kind);
	fresh.px = px;
	fresh.py = py;
	fresh.cam_y = cam_y;
	...
	int32_t f_image[BP35_NUM_SLOTS], f_x[BP35_NUM_SLOTS],
		f_y[BP35_NUM_SLOTS];
	int32_t f_layer[BP35_NUM_SLOTS], f_order[BP35_NUM_SLOTS];
	struct arc_scene_table fresh_scene = { f_image, f_x,	       f_y,
					       f_layer, f_order,       NULL,
					       0,	BP35_NUM_SLOTS };
	compose(&fresh_scene, st, level, fresh.kind, px * BP35_PITCH,
		py * BP35_PITCH, facing_img(fresh.facing_right), cam_y, 0, neg5,
		neg5, neg5);

	struct bp35_aux dispatched = fresh;
	dispatch(st, level, action_id, action_x, action_y, &dispatched);
	int32_t t = clampi(dispatched.total_frames - 1, 0, BP35_MAX_FRAMES - 1);

	int32_t cellimg_t[5];
	for (int i = 0; i < 5; i++)
		cellimg_t[i] = dispatched.traj_cell_img[i][t];
	int32_t e_image[BP35_NUM_SLOTS], e_x[BP35_NUM_SLOTS],
		e_y[BP35_NUM_SLOTS];
	int32_t e_layer[BP35_NUM_SLOTS], e_order[BP35_NUM_SLOTS];
	struct arc_scene_table echo_scene = {
		e_image, e_x, e_y, e_layer, e_order, NULL, 0, BP35_NUM_SLOTS
	};
	compose(&echo_scene, st, level, dispatched.kind, dispatched.traj_px[t],
		dispatched.traj_py[t], dispatched.traj_pimg[t],
		dispatched.traj_cam[t], dispatched.traj_ceil_dy[t],
		dispatched.cell_x, dispatched.cell_y, cellimg_t);

	const struct arc_scene_table *chosen =
		next_level ? &echo_scene : &fresh_scene;
...
void bp35_step_once(struct arc_scene_table *scene, const struct bp35_static *st,
		    int32_t level, int32_t action_id, int32_t action_x,
		    int32_t action_y, struct bp35_aux *aux, int32_t *score,
		    int32_t *status, uint8_t *next_level,
		    uint8_t *action_complete)
{
	if (aux->frame_idx == 0)
		dispatch(st, level, action_id, action_x, action_y, aux);

	int32_t f = aux->frame_idx;
	int32_t cellimg_f[5];
	for (int i = 0; i < 5; i++)
		cellimg_f[i] = aux->traj_cell_img[i][f];
	compose(scene, st, level, aux->kind, aux->traj_px[f], aux->traj_py[f],
		aux->traj_pimg[f], aux->traj_cam[f], aux->traj_ceil_dy[f],
		aux->cell_x, aux->cell_y, cellimg_f);
...
void bp35_render_interface(int8_t *frame, const struct bp35_static *st,
			   int32_t level, const struct bp35_aux *aux)
{
	(void)st;
	int32_t level1 = level + 1;
	int32_t ct = aux->action_ct;
	if (aux->total_frames == 1)
		return;
	for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) {
		...
		frame[(ARC_FRAME_SIZE - 1) * ARC_FRAME_SIZE + c] = (int8_t)val;
	}
}
