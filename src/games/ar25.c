...
enum {
	AR25_TARGET_COLOR = 11,
	AR25_EXCLUDED_MOVABLE_COLOR = 5,
	AR25_EXCLUDED_MIRROR_COLOR = 10,
	AR25_SELECTED_COLOR = 0,
	AR25_PLAIN_COLOR = 9
};
enum {
	AR25_PRIORITY_TARGET = 4,
	AR25_PRIORITY_EXCLUDED = 3,
	AR25_PRIORITY_SELECTED = 2,
	AR25_PRIORITY_PLAIN = 1,
...
static const int8_t AR25_ENERGY_COLORS[AR25_ENERGY_TIERS] = { 11, 12, 15, 8,
							      14 };

static void ar25_display_to_grid(const struct arc_camera *camera,
...
static int ar25_hit_test(const struct arc_sprites *s, int32_t slot, int32_t x,
			 int32_t y)
{
	if (slot < 0)
		return 0;
	int32_t sx = s->x[slot], sy = s->y[slot], sw = s->w[slot],
		sh = s->h[slot];
	if (!(x >= sx && y >= sy && x < sx + sw && y < sy + sh))
		return 0;
	int32_t pw = s->atlas->pw;
	int8_t pv =
		arc_sprite_pixels(s, slot)[(size_t)(y - sy) * pw + (x - sx)];
	return pv != -1;
}

static void ar25_compute_layers(const struct arc_sprites *s,
...
static void ar25_refresh_ghost_overlay(struct arc_sprites *s,
				       const struct ar25_static *st,
				       int32_t level)
{
	int8_t color_grid[AR25_GRID][AR25_GRID];
	int8_t owner_grid[AR25_GRID][AR25_GRID];
...
static int ar25_check_win(const struct arc_sprites *s,
			  const struct ar25_static *st, int32_t level)
{
	int8_t color_grid[AR25_GRID][AR25_GRID];
	int8_t owner_grid[AR25_GRID][AR25_GRID];
	...
	const uint8_t *target =
		st->target_grid + (size_t)level * AR25_GRID * AR25_GRID;
	for (int32_t y = 0; y < AR25_GRID; y++)
		for (int32_t x = 0; x < AR25_GRID; x++)
			if (target[(size_t)y * AR25_GRID + x] &&
			    color_grid[y][x] < 0)
				return 0;
	return 1;
}

static void ar25_role_slots(const struct ar25_static *st, int32_t level,
			    int32_t roles[4])
{
...
static void ar25_push_undo(struct arc_sprites *s, const struct ar25_static *st,
			   int32_t level, struct ar25_aux *aux)
{
	int32_t roles[4];
	ar25_role_slots(st, level, roles);
	int32_t idx = aux->undo_top < AR25_MAX_UNDO - 1 ? aux->undo_top :
							  AR25_MAX_UNDO - 1;
	...
	aux->undo_top = aux->undo_top + 1 < AR25_MAX_UNDO - 1 ?
				aux->undo_top + 1 :
				AR25_MAX_UNDO - 1;
}

static void ar25_pop_undo(struct arc_sprites *s, const struct ar25_static *st,
			  int32_t level, struct ar25_aux *aux)
{
	if (aux->undo_top <= 0)
		return;
	...
	for (int32_t r = 0; r < 4; r++) {
		if (roles[r] < 0)
			continue;
		arc_set_position(s, roles[r], aux->undo_x[top][r],
				 aux->undo_y[top][r]);
	}
	aux->undo_top = top;
...
static void ar25_move_selected(struct arc_sprites *s,
			       const struct ar25_static *st, int32_t level,
			       int32_t dx0, int32_t dy0, struct ar25_aux *aux,
			       int32_t *status, uint8_t *action_complete)
{
	int32_t selected = aux->selected_slot;
	int32_t vslot = st->vmirror_slot[level],
		hslot = st->hmirror_slot[level];
	...
	if (!still_alive)
		*status = AR25_GAME_OVER;

	int hint_trigger = level == 1 && aux->hint_frame == 0 &&
			   aux->energy < 50 && !aux->changed_selection;
	if (hint_trigger) {
		aux->hint_frame = 1;
	} else {
		*action_complete = 1;
...
static void ar25_advance_hint_animation(struct arc_sprites *s,
					const struct ar25_static *st,
					int32_t level, struct ar25_aux *aux,
...
static void ar25_cycle_selection(const struct ar25_static *st, int32_t level,
				 struct ar25_aux *aux, int32_t *status)
{
	...
	int32_t new_selected =
		count > 0 ? order[next_index] : aux->selected_slot;
	int changed = new_selected != aux->selected_slot;
	aux->selected_slot = new_selected;
	if (changed)
		aux->changed_selection = 1;
...
static void ar25_click_select(struct arc_sprites *s,
			      ...
			      struct ar25_aux *aux)
{
	...
	int32_t toggled = selected == vslot ?
				  hslot :
				  (selected == hslot ? vslot : default_pick);
	int32_t mirror_pick = not_among_mirrors ? default_pick : toggled;
	int32_t picked_mirror =
		both ? mirror_pick : (v_hit ? vslot : (h_hit ? hslot : -1));
	int any_mirror = v_hit || h_hit;

	int32_t picked = any_movable ? picked_movable :
				       (any_mirror ? picked_mirror : -1);
...
{
	if (aux->hint_frame > 0) {
		ar25_advance_hint_animation(sprites, st, level, aux,
					    action_complete);
		return;
	}
	...
	int is_movement = aux->selected_slot >= 0 &&
			  action_id >= AR25_ACTION1 &&
	...
	if (action_id == AR25_ACTION7) {
		...
		*action_complete = 1;
		...
		int32_t dx = action_id == AR25_ACTION3 ?
				     -1 :
				     (action_id == AR25_ACTION4 ? 1 : 0);
		int32_t dy = action_id == AR25_ACTION1 ?
				     -1 :
				     (action_id == AR25_ACTION2 ? 1 : 0);
		ar25_move_selected(sprites, st, level, dx, dy, aux, status,
				   action_complete);
	} else if (action_id == AR25_ACTION5) {
		ar25_cycle_selection(st, level, aux, status);
		*action_complete = 1;
	} else if (action_id == AR25_ACTION6) {
		ar25_click_select(sprites, camera, st, level, action_x,
				  action_y, aux);
		*action_complete = 1;
	} else {
		*action_complete = 1;
...
static void ar25_render_energy_bar(int8_t *frame, const struct ar25_static *st,
				   int32_t level, const struct ar25_aux *aux)
{
	...
	if (tiers < 1)
		tiers = 1;
	if (tiers > AR25_ENERGY_TIERS)
		tiers = AR25_ENERGY_TIERS;
	...
	for (int32_t row = 0; row < ARC_FRAME_SIZE; row++)
		col[row] = frame[(size_t)row * ARC_FRAME_SIZE + ARC_FRAME_SIZE -
				 1];
	for (int32_t row = 0; row < ARC_FRAME_SIZE; row++)
		if (row >= within)
			col[row] = color;
	if (!is_last_tier && within > 0)
		for (int32_t row = 0; row < within; row++)
			col[row] = next_color;
	for (int32_t row = 0; row < ARC_FRAME_SIZE; row++)
		frame[(size_t)row * ARC_FRAME_SIZE + ARC_FRAME_SIZE - 1] =
			col[row];
}
...
static void ar25_paint_axis_slot(int8_t grid[AR25_GRID][AR25_GRID],
				 ...
				 const struct ar25_static *st, int32_t level,
				 int32_t slot, int32_t axis_index,
				 int32_t selected_slot)
{
	if (slot < 0)
		return;
	int32_t sx = s->x[slot], sy = s->y[slot], sh = s->h[slot],
		sw = s->w[slot];
	int32_t pw = s->atlas->pw;
	const int8_t *pixels = arc_sprite_pixels(s, slot);
	const uint8_t *target =
	...
	int8_t plain_priority =
		selected ? AR25_PRIORITY_SELECTED : AR25_PRIORITY_PLAIN;
	...
	for (int32_t li = 0; li < sh; li++) {
		for (int32_t lj = 0; lj < sw; lj++) {
			...
			int8_t color =
				is_target ?
					AR25_TARGET_COLOR :
					(excluded ? AR25_EXCLUDED_MIRROR_COLOR :
						    plain_color);
			int8_t cell_priority =
				is_target ? AR25_PRIORITY_TARGET :
					    (excluded ? AR25_PRIORITY_EXCLUDED :
							plain_priority);

			int8_t owner_at = owner_grid[ay][ax];
			uint8_t true_at = true_grid[ay][ax];
			int skip = !is_target && color == AR25_PLAIN_COLOR &&
				   owner_at >= 0 && !true_at;
			if (skip)
				continue;

			int8_t current_priority = priority[ay][ax];
			int win = current_priority == AR25_PRIORITY_NONE ||
				  cell_priority > current_priority;
			if (win) {
				grid[ay][ax] = color;
...
static void ar25_render_minimap(int8_t *frame, const struct arc_sprites *s,
				const struct ar25_static *st, int32_t level,
				const struct ar25_aux *aux)
{
	...
	for (int32_t y = 0; y < AR25_GRID; y++) {
		for (int32_t x = 0; x < AR25_GRID; x++) {
			...
			int8_t movable_color =
				excluded_here ?
					AR25_EXCLUDED_MOVABLE_COLOR :
					(selected_here ? AR25_SELECTED_COLOR :
							 AR25_PLAIN_COLOR);
			int8_t movable_priority =
				excluded_here ?
					AR25_PRIORITY_EXCLUDED :
					(selected_here ?
						 AR25_PRIORITY_SELECTED :
						 AR25_PRIORITY_PLAIN);
	...
	for (int32_t axis_index = 0; axis_index < 2; axis_index++) {
		...
		ar25_paint_axis_slot(grid, priority, owner_grid, true_grid, s,
				     st, level, slot, axis_index,
				     aux->selected_slot);
	}
	...
	for (int32_t y = 0; y < AR25_GRID; y++) {
		for (int32_t x = 0; x < AR25_GRID; x++) {
			if (grid[y][x] < 0)
				continue;
			int32_t sy =
				y * AR25_MINIMAP_SCALE + AR25_MINIMAP_OFFSET;
			int32_t sx =
				x * AR25_MINIMAP_SCALE + AR25_MINIMAP_OFFSET;
