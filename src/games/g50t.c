...
enum {
	G50T_DOOR_LOCK_SENTINEL = 11,
	G50T_ECHO_BODY_COLOR = 2,
	G50T_ECHO_CENTER_COLOR = 5
};
...
static int g50t_pixel_hit(const struct arc_sprites *s, int32_t slot, int32_t x,
			  int32_t y, int solid)
{
	int32_t sx = s->x[slot], sy = s->y[slot], w = s->w[slot],
		h = s->h[slot];
	int inside = x >= sx && y >= sy && x < sx + w && y < sy + h;
	int32_t ph = s->atlas->ph, pw = s->atlas->pw;
	int32_t py = y - sy;
	py = py < 0 ? 0 : (py > ph - 1 ? ph - 1 : py);
	int32_t px = x - sx;
	px = px < 0 ? 0 : (px > pw - 1 ? pw - 1 : px);
	int8_t pixel = arc_sprite_pixels(s, slot)[(size_t)py * pw + px];
	int opaque = solid ? (pixel != -1) : (pixel >= 0);
	return inside && opaque;
}

static void g50t_start_death(struct arc_sprites *s, int32_t main_slot,
			     const int32_t *death_slots)
{
	...
	arc_set_position(s, piece0, s->x[main_slot] + offset_x,
			 s->y[main_slot] + offset_y);
	arc_set_interaction(s, piece0, TANGIBLE);
	arc_set_interaction(s, main_slot, REMOVED);
}

static int g50t_in_bounds_and_open(const struct arc_sprites *s,
				   const struct g50t_static *st, int32_t level,
				   int32_t cx, int32_t cy)
{
	if (!g50t_pixel_hit(s, st->bounds_slot[level], cx, cy, 1))
		return 0;
	const int32_t *doors = st->door_slot + (size_t)level * G50T_MAX_DOORS;
	for (int32_t i = 0; i < G50T_MAX_DOORS; i++) {
		int32_t slot = doors[i];
		if (slot < 0 || !arc_sprite_visible(s, slot))
			continue;
		if (g50t_pixel_hit(s, slot, cx, cy, 1))
			return 0;
	}
	return 1;
}

static int32_t g50t_sensor_index_at(const struct arc_sprites *s,
				    const struct g50t_static *st, int32_t level,
				    int32_t cx, int32_t cy)
{
	const int32_t *sensors =
		st->sensor_slot + (size_t)level * G50T_MAX_SENSORS;
...
static void g50t_set_door(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, struct g50t_aux *aux,
			  int32_t door_role, int value, int instant)
{
	int32_t slot =
		st->door_slot[(size_t)level * G50T_MAX_DOORS + door_role];
	int32_t h = s->h[slot], w = s->w[slot];
	int8_t center_px = arc_sprite_pixels(
		s, slot)[(size_t)(h / 2) * s->atlas->pw + (w / 2)];
	int toggle = center_px == G50T_DOOR_LOCK_SENTINEL;
	int ignore = toggle && !value;
	if (toggle && value && aux->door_open[door_role])
		value = 0;
	int changed =
		!ignore && ((aux->door_open[door_role] != 0) != (value != 0));
	if (!changed)
		return;
...
static void g50t_teleport(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, struct g50t_aux *aux,
			  int32_t portal_role)
{
	const int32_t *eps =
		st->portal_endpoints +
		((size_t)level * G50T_MAX_PORTALS + portal_role) * 2;
	...
	int32_t e0_slot =
		st->endpoint_slot[(size_t)level * G50T_MAX_ENDPOINTS + e0c];
	int32_t e1_slot =
		st->endpoint_slot[(size_t)level * G50T_MAX_ENDPOINTS + e1c];
	if (e0_slot < 0)
		e0_slot = 0;
	if (e1_slot < 0)
		e1_slot = 0;
	...
	if (valid) {
		for (int32_t g = 0; g < G50T_PLAYER_GENERATIONS; g++) {
			int32_t slot =
				st->gen_slot[(size_t)level *
						     G50T_PLAYER_GENERATIONS +
					     g];
...
static void g50t_set_portal(struct arc_sprites *s, const struct g50t_static *st,
			    int32_t level, struct g50t_aux *aux,
			    int32_t portal_role, int value, int instant)
{
	if (instant) {
		if (!value)
			g50t_teleport(s, st, level, aux, portal_role);
		return;
	}
	if (!value)
		return;
	if (aux->portal_animating[portal_role])
		return;
...
static void g50t_forward_button(struct arc_sprites *s,
				const struct g50t_static *st, int32_t level,
				struct g50t_aux *aux, int32_t gate_role,
				int value)
{
	if (gate_role < 0)
		return;
	...
	int32_t kind =
		st->gate_kind[(size_t)level * G50T_MAX_GATES + gate_role];
	int32_t target =
		st->gate_target[(size_t)level * G50T_MAX_GATES + gate_role];
	if (kind == G50T_GATE_DOOR)
		g50t_set_door(s, st, level, aux, target, value, instant);
	else if (kind == G50T_GATE_PORTAL)
		g50t_set_portal(s, st, level, aux, target, value, instant);
}

static void g50t_update_sensor(struct arc_sprites *s,
			       const struct g50t_static *st, int32_t level,
			       struct g50t_aux *aux, int32_t tracked_index,
			       int32_t cx, int32_t cy, int value)
{
	...
	if (idx < G50T_MAX_BUTTONS) {
		...
		int was_on = 0;
		for (int32_t t = 0; t < G50T_MAX_TRACKED; t++)
			was_on |= members[t];
		...
		if ((was_on != 0) != (now_on != 0)) {
			int32_t gate_role =
				st->button_gate[(size_t)level *
							G50T_MAX_BUTTONS +
						role];
			g50t_forward_button(s, st, level, aux, gate_role,
					    now_on);
		}
	} else {
...
static void g50t_register(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, struct g50t_aux *aux,
...
static void g50t_deregister(struct arc_sprites *s, const struct g50t_static *st,
			    int32_t level, struct g50t_aux *aux,
			    int32_t tracked_index, int32_t cx, int32_t cy)
{
...
static int g50t_try_animated_move(struct arc_sprites *s,
				  const struct g50t_static *st, int32_t level,
				  struct g50t_aux *aux, int32_t slot,
				  int32_t dx, int32_t dy, int32_t tracked_index)
{
	...
	if (!g50t_in_bounds_and_open(s, st, level, ncx, ncy))
		return 0;
	g50t_deregister(s, st, level, aux, tracked_index, x + w / 2, y + h / 2);
	return 1;
}

static void g50t_queue_gen_move(struct g50t_aux *aux, int32_t gen, int32_t tx,
				int32_t ty, int32_t delay, int instant)
{
	aux->target_x[gen] = tx;
	aux->target_y[gen] = ty;
	aux->delay[gen] = delay;
...
static void g50t_queue_ice_move(struct g50t_aux *aux, int32_t ii, int32_t tx,
				int32_t ty, int32_t delay, int instant)
{
...
static int g50t_move_gen(struct arc_sprites *s, const struct g50t_static *st,
			 int32_t level, struct g50t_aux *aux, int32_t gen,
			 int32_t dx, int32_t dy)
{
	int32_t slot =
		st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + gen];
	if (!g50t_try_animated_move(s, st, level, aux, slot, dx, dy, gen))
		return 0;
...
static int g50t_move_ice(struct arc_sprites *s, const struct g50t_static *st,
			 int32_t level, struct g50t_aux *aux, int32_t ii,
			 int32_t dx, int32_t dy)
{
	int32_t slot = st->ice_slot[(size_t)level * G50T_MAX_ICE + ii];
	if (slot < 0)
	...
	if (!g50t_try_animated_move(s, st, level, aux, slot, dx, dy, tracked))
		return 0;
	int32_t nx = s->x[slot] + dx * G50T_GRID;
	int32_t ny = s->y[slot] + dy * G50T_GRID;
...
{
	static const int32_t deltas[4][2] = {
		{ 0, 1 }, { 1, 0 }, { 0, -1 }, { -1, 0 }
	};
...
static int g50t_ice_in_boundary(const struct arc_sprites *s,
				const struct g50t_static *st, int32_t level,
				int32_t ii, int32_t x, int32_t y)
{
	int32_t bound_slot =
		st->ice_bound_slot[(size_t)level * G50T_MAX_ICE + ii];
	if (bound_slot < 0)
		return 0;
	return g50t_pixel_hit(s, bound_slot, x, y, 0);
}

static void g50t_ice_slide_and_record(struct arc_sprites *s,
				      const struct g50t_static *st,
				      int32_t level, struct g50t_aux *aux,
				      int32_t ii)
{
	...
	for (int32_t k = 0; k < 4 && !moved; k++) {
		...
		if (!g50t_ice_in_boundary(s, st, level, ii, x, y))
			continue;
		if (!g50t_move_ice(s, st, level, aux, ii, dx, dy))
			continue;
		moved = 1;
		final_dir = d;
		mdx = dx;
...
static void g50t_do_move(struct arc_sprites *s, const struct g50t_static *st,
			 int32_t level, struct g50t_aux *aux, int32_t dx,
			 int32_t dy)
{
	int32_t gen = aux->gen;
	if (!g50t_move_gen(s, st, level, aux, gen, dx, dy))
		return;
	...
	for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++) {
		if (st->ice_slot[(size_t)level * G50T_MAX_ICE + ii] < 0)
			continue;
		if (aux->ice_dead[ii])
			continue;
...
static void g50t_pop_move(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, struct g50t_aux *aux)
{
	...
	for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++) {
		int32_t ice_slot =
			st->ice_slot[(size_t)level * G50T_MAX_ICE + ii];
		if (ice_slot < 0)
			continue;
		int32_t rec = aux->ice_move_count[ii];
		if (step_idx >= rec)
...
static void g50t_do_undo(struct arc_sprites *s, const struct g50t_static *st,
			 int32_t level, struct g50t_aux *aux)
{
	if (aux->move_count <= 0)
		return;
...
static int g50t_any_active(const struct g50t_aux *aux)
{
	for (int32_t g = 0; g < G50T_PLAYER_GENERATIONS; g++)
		if (aux->active[g])
			return 1;
	for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++)
		if (aux->ice_active[ii])
			return 1;
	for (int32_t i = 0; i < G50T_MAX_DOORS; i++)
		if (aux->door_active[i])
			return 1;
	for (int32_t i = 0; i < G50T_MAX_GATES; i++)
		if (aux->gate_active[i])
			return 1;
	for (int32_t i = 0; i < G50T_MAX_PORTALS; i++)
		if (aux->portal_active[i])
			return 1;
	return aux->ghost_active != 0;
}

static void g50t_tick_mover(int32_t x, int32_t y, int32_t target_x,
			    int32_t target_y, int32_t delay, int instant,
			    int32_t *out_delay, int32_t *out_x, int32_t *out_y,
			    int *out_instant, int *out_busy)
{
	if (delay > 0) {
		*out_delay = delay - 1;
		*out_x = x;
...
static void g50t_tick_gen(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, struct g50t_aux *aux, int32_t g)
{
	if (!aux->active[g])
		return;
	int32_t slot =
		st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + g];
	if (aux->dying[g]) {
		...
		if (!finished) {
			int32_t clipped = next_step > G50T_DEATH_PIECES - 1 ?
						  G50T_DEATH_PIECES - 1 :
						  next_step;
			int32_t nxt = st->player_death_slot[clipped];
			int32_t x = aux->target_x[g] +
				    g50t_fdiv2(s->w[slot] - s->w[cur]);
			int32_t y = aux->target_y[g] +
				    g50t_fdiv2(s->h[slot] - s->h[cur]);
	...
	g50t_tick_mover(s->x[slot], s->y[slot], aux->target_x[g],
			aux->target_y[g], aux->delay[g], aux->instant[g],
			&new_delay, &new_x, &new_y, &new_instant, &busy);
...
static void g50t_tick_ice(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, struct g50t_aux *aux, int32_t ii)
{
	if (!aux->ice_active[ii])
		return;
	...
	if (aux->ice_dying[ii]) {
		...
		const int32_t *death_slot =
			st->ice_death_slot + (size_t)ii * G50T_DEATH_PIECES;
		...
		if (!finished) {
			...
			int32_t x = aux->ice_target_x[ii] +
				    g50t_fdiv2(s->w[slot] - s->w[cur]);
			int32_t y = aux->ice_target_y[ii] +
				    g50t_fdiv2(s->h[slot] - s->h[cur]);
			arc_set_position(s, nxt, x, y);
			arc_set_interaction(s, nxt, TANGIBLE);
		}
	...
	int32_t new_delay, new_x, new_y;
	int new_instant, busy;
	g50t_tick_mover(s->x[slot], s->y[slot], aux->ice_target_x[ii],
			aux->ice_target_y[ii], aux->ice_delay[ii],
			aux->ice_instant[ii], &new_delay, &new_x, &new_y,
			&new_instant, &busy);
	...
	int32_t pslot =
		st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + gen];
	int32_t icx = s->x[slot] + s->w[slot] / 2,
		icy = s->y[slot] + s->h[slot] / 2;
	int32_t ax = s->x[slot], ay = s->y[slot], aw = s->w[slot],
		ah = s->h[slot];
	int32_t bx = s->x[pslot], by = s->y[pslot], bw = s->w[pslot],
		bh = s->h[pslot];
	int bbox = ax < bx + bw && ax + aw > bx && ay < by + bh && ay + ah > by;
	int crush = bbox && g50t_pixel_hit(s, pslot, icx, icy, 0) &&
...
static void g50t_tick_door(struct arc_sprites *s, const struct g50t_static *st,
			   int32_t level, struct g50t_aux *aux, int32_t i)
{
	...
	int32_t new_delay, new_x, new_y;
	int new_instant, busy;
	g50t_tick_mover(s->x[s_], s->y[s_], aux->door_target_x[i],
			aux->door_target_y[i], aux->door_delay[i],
			aux->door_instant[i], &new_delay, &new_x, &new_y,
			&new_instant, &busy);
	if (active)
		arc_set_position(s, s_, new_x, new_y);

	int just_settled = active && !busy;
	int32_t gen = aux->gen;
	int32_t pslot =
		st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + gen];
	int32_t px = s->x[pslot] + s->w[pslot] / 2,
		py = s->y[pslot] + s->h[pslot] / 2;
	int32_t ax = s->x[s_], ay = s->y[s_], aw = s->w[s_], ah = s->h[s_];
	int32_t bx = s->x[pslot], by = s->y[pslot], bw = s->w[pslot],
		bh = s->h[pslot];
	int bbox = ax < bx + bw && ax + aw > bx && ay < by + bh && ay + ah > by;
	int closing = !aux->door_open[i];
	int crush = just_settled && closing && bbox &&
		    g50t_pixel_hit(s, s_, px, py, 1) && !aux->dead[gen] &&
		    !aux->dying[gen];
	if (crush)
		g50t_start_death(s, pslot, st->player_death_slot);
	...
	for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++) {
		...
		int32_t icx = s->x[ice_slot] + s->w[ice_slot] / 2,
			icy = s->y[ice_slot] + s->h[ice_slot] / 2;
		int icrush = just_settled && closing && collide &&
			     g50t_pixel_hit(s, s_, icx, icy, 1) &&
			     !aux->ice_dead[ii] && !aux->ice_dying[ii];
		if (icrush) {
			g50t_start_death(s, ice_slot,
					 st->ice_death_slot +
						 (size_t)ii *
							 G50T_DEATH_PIECES);
			aux->ice_dying[ii] = 1;
...
static void g50t_tick_portal(struct arc_sprites *s,
			     const struct g50t_static *st, int32_t level,
			     struct g50t_aux *aux, int32_t i)
{
	if (!aux->portal_active[i])
		return;
	...
	const int32_t *slots =
		st->portal_crossfade_slot + (size_t)i * G50T_CROSSFADE_PIECES;
	if (progress > 0) {
		int32_t clipped = progress - 1;
		clipped = clipped > G50T_CROSSFADE_PIECES - 1 ?
				  G50T_CROSSFADE_PIECES - 1 :
				  clipped;
		arc_set_interaction(s, slots[clipped], REMOVED);
	}
	int more = progress < G50T_CROSSFADE_PIECES;
	if (more) {
		int32_t clipped = progress > G50T_CROSSFADE_PIECES - 1 ?
					  G50T_CROSSFADE_PIECES - 1 :
					  progress;
...
static void g50t_tick_ghost(struct arc_sprites *s, const struct g50t_static *st,
			    int32_t level, struct g50t_aux *aux)
{
	if (!aux->ghost_active)
		return;
	...
	int32_t nx = x + dx, ny = y + dy;
	arc_set_position(s, slot, nx, ny);
	aux->ghost_active = (uint8_t)((aux->ghost_target_x != nx) ||
				      (aux->ghost_target_y != ny));
}

static void g50t_tick_all(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, struct g50t_aux *aux)
{
	for (int32_t g = 0; g < G50T_PLAYER_GENERATIONS; g++)
		g50t_tick_gen(s, st, level, aux, g);
	for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++)
		g50t_tick_ice(s, st, level, aux, ii);
	for (int32_t i = 0; i < G50T_MAX_DOORS; i++)
		g50t_tick_door(s, st, level, aux, i);
	for (int32_t i = 0; i < G50T_MAX_GATES; i++) {
		if (!aux->gate_active[i])
			continue;
	...
	for (int32_t i = 0; i < G50T_MAX_PORTALS; i++)
		g50t_tick_portal(s, st, level, aux, i);
	g50t_tick_ghost(s, st, level, aux);
}

static void g50t_register_presence(struct arc_sprites *s,
				   const struct g50t_static *st, int32_t level,
				   struct g50t_aux *aux)
{
	int32_t gen = aux->gen;
	...
	for (int32_t ii = 0; ii < G50T_MAX_ICE; ii++) {
		...
		if (slot < 0)
			continue;
		g50t_register(s, st, level, aux, G50T_PLAYER_GENERATIONS + ii,
			      s->x[slot] + s->w[slot] / 2,
			      s->y[slot] + s->h[slot] / 2);
	}
	...
	for (int32_t g = 0; g < G50T_PLAYER_GENERATIONS; g++) {
		if (!aux->echo[g])
			continue;
		int32_t slot =
			st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS +
				     g];
		g50t_register(s, st, level, aux, g, s->x[slot] + s->w[slot] / 2,
			      s->y[slot] + s->h[slot] / 2);
	}
}

static void g50t_checkpoint_swap(struct arc_sprites *s,
				 const struct g50t_static *st,
				 struct g50t_aux *aux)
{
	...
	if (idx == 0) {
		for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
			arc_set_interaction(s, st->checkpoint_on_slot[k],
					    REMOVED);
		for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
			arc_set_interaction(s, st->checkpoint_off_slot[k],
					    REMOVED);
	} else {
		int32_t prev = idx - 1;
		prev = prev < 0 ? 0 :
				  (prev > G50T_MAX_CHECKPOINTS - 1 ?
					   G50T_MAX_CHECKPOINTS - 1 :
					   prev);
		arc_set_interaction(s, st->checkpoint_on_slot[prev], REMOVED);
		arc_set_interaction(s, st->checkpoint_off_slot[prev], TANGIBLE);
	}
	int32_t ci = idx < 0 ? 0 :
			       (idx > G50T_MAX_CHECKPOINTS - 1 ?
					G50T_MAX_CHECKPOINTS - 1 :
					idx);
...
static void g50t_commit_checkpoint(struct arc_sprites *s,
				   const struct g50t_static *st, int32_t level,
				   struct g50t_aux *aux, int32_t *next_order)
{
	int32_t gen = aux->gen;
	int32_t old_slot =
		st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + gen];
	...
	if (is_last) {
		...
		for (int32_t k = 0; k < area; k++)
			if (old_px[k] >= 0)
				old_px[k] = G50T_ECHO_BODY_COLOR;
		old_px[(size_t)(h / 2) * s->atlas->pw + (w / 2)] =
			G50T_ECHO_CENTER_COLOR;
	...
	int32_t target_slot =
		st->checkpoint_slot[(size_t)level * G50T_MAX_CHECKPOINTS +
				    cp_idx_final];
	aux->ghost_active = 1;
...
static void g50t_advance_checkpoint_or_undo(struct arc_sprites *s,
					    const struct g50t_static *st,
					    int32_t level, struct g50t_aux *aux,
					    int32_t *next_order)
{
...
static void g50t_do_resolve(struct arc_sprites *s, const struct g50t_static *st,
			    int32_t level, struct g50t_aux *aux,
			    int32_t *next_order)
{
	if (aux->swap_pending) {
	...
	if (g50t_any_active(aux))
		return;
	if (aux->commit_pending)
		g50t_advance_checkpoint_or_undo(s, st, level, aux, next_order);
}

static int g50t_is_lost(const struct arc_sprites *s,
			const struct g50t_static *st, int32_t level,
...
static int g50t_is_won(const struct arc_sprites *s,
		       const struct g50t_static *st, int32_t level,
		       const struct g50t_aux *aux)
{
	...
	int32_t player_slot =
		st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS +
			     aux->gen];
	return (s->x[goal_slot] + 1 == s->x[player_slot]) &&
	       (s->y[goal_slot] + 1 == s->y[player_slot]);
}

static void g50t_complete(struct g50t_aux *aux, uint8_t *action_complete)
{
	aux->dispatched = 0;
	*action_complete = 1;
}

static void g50t_next_level(const struct g50t_static *st, int32_t level,
...
static void g50t_maybe_finish(struct arc_sprites *s,
			      const struct g50t_static *st, int32_t level,
			      struct g50t_aux *aux, int32_t *score,
...
static void g50t_finish_win(const struct g50t_static *st, int32_t level,
			    struct g50t_aux *aux, int32_t *score,
			    int32_t *status, uint8_t *next_level,
			    uint8_t *action_complete)
{
...
static void g50t_finish_lose(struct g50t_aux *aux, int32_t *status,
			     uint8_t *action_complete)
{
	aux->lost_pending = 0;
...
static void g50t_dispatch(struct arc_sprites *s, const struct g50t_static *st,
			  int32_t level, int32_t action_id,
			  struct g50t_aux *aux, int32_t *score, int32_t *status,
			  uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (action_id == G50T_ACTION5)
		g50t_do_undo(s, st, level, aux);
	...
	aux->dispatched = 1;
	g50t_maybe_finish(s, st, level, aux, score, status, next_level,
			  action_complete);
}

static void g50t_tick_then_resolve(struct arc_sprites *s,
				   const struct g50t_static *st, int32_t level,
				   struct g50t_aux *aux, int32_t *next_order,
				   int32_t *score, int32_t *status,
				   uint8_t *next_level,
		       ...
		       struct g50t_aux *aux)
{
	g50t_zero_aux(aux);
	int32_t player_slot =
		st->gen_slot[(size_t)level * G50T_PLAYER_GENERATIONS + 0];
	int32_t spawn_x = sprites->x[player_slot],
		spawn_y = sprites->y[player_slot];
	...
	for (int32_t pi = 0; pi < G50T_MAX_PORTALS; pi++)
		for (int32_t k = 0; k < G50T_CROSSFADE_PIECES; k++)
			arc_set_interaction(
				sprites,
				st->portal_crossfade_slot
					[(size_t)pi * G50T_CROSSFADE_PIECES +
					 k],
				REMOVED);
	for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
		arc_set_interaction(sprites, st->checkpoint_on_slot[k],
				    REMOVED);
	for (int32_t k = 0; k < G50T_MAX_CHECKPOINTS; k++)
		arc_set_interaction(sprites, st->checkpoint_off_slot[k],
				    REMOVED);

	arc_set_interaction(sprites, player_slot, TANGIBLE);
	if (st->checkpoint_count[level] > 0)
		arc_set_interaction(sprites, st->checkpoint_on_slot[0],
				    TANGIBLE);
	...
	for (int32_t i = 0; i < G50T_MAX_DOORS; i++) {
		int32_t slot =
			st->door_slot[(size_t)level * G50T_MAX_DOORS + i];
		    ...
		    uint8_t *action_complete)
{
	...
	(void)action_count;
	if (aux->won_pending) {
		g50t_finish_win(st, level, aux, score, status, next_level,
				action_complete);
		return;
	}
	if (aux->lost_pending) {
	...
	if (aux->dispatched)
		g50t_tick_then_resolve(sprites, st, level, aux, next_order,
				       score, status, next_level,
				       action_complete);
	else
		g50t_dispatch(sprites, st, level, action_id, aux, score, status,
