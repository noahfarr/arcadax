...
static int32_t r11l_find_piece(const ArcSprites *s, const R11lStatic *st, int32_t level,
...
static void r11l_select_piece(ArcSprites *s, R11lAux *aux, int32_t slot) {
    int32_t prev = aux->selected;
    if (prev >= 0) arc_color_remap(s, prev, 1, R11L_PIECE_SELECTED, R11L_PIECE_IDLE);
    if (slot >= 0) arc_color_remap(s, slot, 1, R11L_PIECE_IDLE, R11L_PIECE_SELECTED);
    aux->selected = slot;
}

static void r11l_recompute_group(ArcSprites *s, const R11lStatic *st, int32_t level, int32_t group) {
...
static void r11l_maybe_merge(ArcSprites *s, const R11lStatic *st, int32_t level, int32_t group) {
    int32_t base = level * R11L_G + group;
    ...
    for (int32_t k = 0; k < count; k++) {
        ...
        if (!arc_collides_pair(s, target_slot, i, 0)) continue;
...
static int r11l_hazard_hit(const ArcSprites *s, const R11lStatic *st, int32_t level, int32_t group) {
    ...
    for (int32_t k = 0; k < count; k++)
        if (arc_collides_pair(s, target_slot, hz[k], 0)) return 1;
    return 0;
}

static int r11l_key_matched(const ArcSprites *s, const R11lStatic *st, int32_t level, int32_t k) {
...
static int r11l_win_check(const ArcSprites *s, const R11lStatic *st, int32_t level) {
...
static int r11l_check_blocked(ArcSprites *s, const R11lStatic *st, int32_t level, int32_t sel,
    ...
    for (int32_t k = 0; k < count && !blocked; k++)
        if (arc_collides_pair(s, sel, walls[k], 0)) blocked = 1;
    arc_set_position(s, sel, ox, oy);
    return blocked;
}

static void r11l_handle_click(ArcSprites *s, const ArcCamera *cam, const R11lStatic *st, int32_t level,
...
static void r11l_update_key(ArcSprites *s, const R11lStatic *st, int32_t level, int32_t k,
    ...
    if (want_visible && !s->alive[icon_slot]) {
        arc_set_position(s, icon_slot, s->x[clue_slot], s->y[clue_slot]);
        arc_add_sprite(s, icon_slot, *next_order);
        *next_order += 1;
    }
    if (want_hidden && s->alive[icon_slot]) arc_remove_sprite(s, icon_slot);
...
static void r11l_update_blinks(ArcSprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
                               int32_t *next_order) {
...
static void r11l_wobble_tick(ArcSprites *s, const R11lStatic *st, R11lAux *aux, int32_t *next_order) {
    ...
    if (want_hidden && s->alive[slot]) arc_remove_sprite(s, slot);

    if (t % 4 == 0) {
        int32_t b = aux->wobble_blinks + 1;
        aux->wobble_blinks = b;
        if (b >= 5) {
            if (s->alive[slot]) arc_remove_sprite(s, slot);
            aux->wobbling = 0;
            aux->wobble_blinks = 0;
            aux->wobble_ticks = 0;
            aux->reverting = 1;
...
static void r11l_forward_tween(ArcSprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
...
static void r11l_reverse_tween(ArcSprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
                               uint8_t *action_complete) {
    int32_t selected = aux->selected;
    int32_t sel_c = selected >= 0 ? selected : 0;
    int32_t group = st->piece_group[(size_t)level * st->num_slots + sel_c];
...
static void r11l_continue_drag(ArcSprites *s, const R11lStatic *st, int32_t level, R11lAux *aux,
...
void r11l_on_set_level(ArcSprites *sprites, const R11lStatic *st, int32_t level, R11lAux *aux,
    ...
    for (int32_t rank = 1; rank < R11L_NPIECES; rank++) {
        int32_t slot = order[rank];
        if (slot < 0) continue;
        arc_color_remap(sprites, slot, 1, R11L_PIECE_SELECTED, R11L_PIECE_IDLE);
    }
...
void r11l_step_once(ArcSprites *sprites, const ArcCamera *camera, const R11lStatic *st, int32_t level,
...
static void r11l_draw_budget(int8_t *frame, int32_t action_count) {
    ...
    for (int32_t r = 0; r < ARC_FRAME_SIZE; r++) {
        int8_t v = (ARC_FRAME_SIZE - r <= filled) ? R11L_BUDGET_FILLED : R11L_BUDGET_EMPTY;
...
static void r11l_draw_line(int8_t *frame, int32_t x0, int32_t y0, int32_t x1, int32_t y1,
    ...
    for (int32_t step = 0; step < R11L_LINE_STEPS; step++) {
        int inb = x >= 0 && x < ARC_FRAME_SIZE && y >= 0 && y < ARC_FRAME_SIZE;
        ...
        if (want) frame[(size_t)yc * ARC_FRAME_SIZE + xc] = R11L_GUIDE_COLOUR;
...
static void r11l_draw_guides(int8_t *frame, const ArcSprites *s, const ArcCamera *cam,
