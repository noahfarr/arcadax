...
static void sb26_set_visible_slot(ArcSprites *s, int32_t slot, int visible) {
    if (slot < 0 || slot >= s->atlas->num_slots) return;
    arc_set_visible(s, slot, visible);
}

static uint8_t sb26_tracked(const ArcSprites *s, const Sb26Static *st, int32_t i) {
...
static void sb26_lookup(const ArcSprites *s, const Sb26Static *st, int32_t level,
...
static void sb26_border_redraw(ArcSprites *s, int32_t slot, int32_t width, int corner) {
...
static void sb26_rqtmgdegai(ArcSprites *s, const Sb26Static *st, int32_t slot, int32_t offset,
...
static void sb26_dagvovvbpp(ArcSprites *s, const Sb26Static *st, int32_t slot, int32_t offset,
    ...
    int redraw = arc_sprite_pixels(s, slot)[(size_t)1 * pw + probe_col] != 1;
    if (redraw) sb26_border_redraw(s, slot, width, 1);
}

static void sb26_apply_tween_slot(ArcSprites *s, const Sb26Static *st, int32_t role, int32_t fx,
...
static void sb26_paint_row(ArcSprites *s, int32_t slot, int32_t row, int8_t colour) {
    int32_t ph = s->atlas->ph, pw = s->atlas->pw;
    int32_t w = s->w[slot];
    int32_t r = row + 1;
    if (r < 0 || r >= ph) return;
...
static int32_t sb26_click_hit(const ArcSprites *s, const Sb26Aux *aux, const Sb26Static *st,
                              int32_t x, int32_t y) {
    const ArcAtlas *a = s->atlas;
    ...
    for (int32_t i = 0; i < a->num_slots; i++) {
        ...
        if (s->blocking[i] == PIXEL_PERFECT) {
            ...
            if (arc_sprite_pixels(s, i)[(size_t)py * a->pw + px] == -1) continue;
        }
...
static void sb26_push_undo(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux) {
...
static void sb26_handle_action7(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux) {
...
static void sb26_hjewbkcejq(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux, int32_t hit) {
    const ArcAtlas *a = s->atlas;
    ...
    if (aux->selected < 0) {
        if (hit_is_item) {
            ...
            arc_set_position(s, st->mrokwhyjs0, s->x[hit], s->y[hit]);
            aux->selected = hit;
        }
        return;
    }
    ...
    int32_t origin_spot = arc_get_sprite_at(s, s->x[selected], s->y[selected], st->spot_tag, 0);
...
static void sb26_handle_click(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux, int32_t action_x,
                              int32_t action_y) {
    int32_t hit = sb26_click_hit(s, aux, st, action_x, action_y);
    if (hit >= 0) sb26_hjewbkcejq(s, st, aux, hit);
}

static void sb26_handle_default(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux, int32_t action_id,
...
static void sb26_rfdjlhefnd(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux) {
    int32_t n = st->num_slots;
...
static void sb26_handle_action5(ArcSprites *s, const ArcCamera *camera, ArcRenderScratch *scratch,
...
static void sb26_advance_fades(ArcSprites *s, Sb26Aux *aux) {
...
static void sb26_tick_japgbruyb(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux) {
...
static void sb26_tick_xjxrqgaqw(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux) {
    ...
    int8_t colour = step < 4 ? SB26_BLACK : arc_sprite_pixels(s, card_slot)[0];
...
static void sb26_tick_bbiavyren(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux,
    ...
    int is_bg = arc_sprite_pixels(s, st->mjeqtdqvm)[0] == SB26_BACKGROUND_COLOR;
    if (is_bg) {
        int32_t root = st->qaagahahj[(size_t)level * SB26_MAX_FRAMES + 0];
        int32_t rx = s->x[root], ry = s->y[root], rw = s->w[root];
        sb26_dagvovvbpp(s, st, st->mjeqtdqvm, 1, rx, ry, rw, rx, ry, rw, 0);
        arc_color_remap(s, st->mjeqtdqvm, 0, 0, SB26_BACKGROUND_COLOR - 1);
...
static void sb26_tick_ftyhvmeft(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux) {
...
static void sb26_tick_artsfnufc(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux, int32_t *status,
    ...
    aux->artsfnufc += 1;
    int done = arc_sprite_pixels(s, st->mrokwhyjs0)[0] == SB26_BACKGROUND_COLOR;
    if (done) {
        aux->artsfnufc = -1;
        *action_complete = 1;
        if (aux->energy == 0) *status = SB26_GAME_OVER;
...
static void sb26_tick_tween(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux) {
...
static void sb26_sibihgzarf(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux) {
    arc_color_remap(s, st->ayaigjtxp, 0, 0, SB26_RED);
    arc_color_remap(s, st->ohvavdnio, 0, 0, SB26_RED);
    aux->ftyhvmeft = 0;
}

static void sb26_pop_cursor(ArcSprites *s, const Sb26Static *st, Sb26Aux *aux) {
...
static void sb26_pop_to_parent(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux,
                               int32_t target_slot) {
    int32_t depth = aux->stack_depth;
    int32_t top_frame = aux->stack_frame[depth - 1];
...
static void sb26_exhausted(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux) {
    aux->fading[st->mjeqtdqvm] = 1;
    aux->fading[st->ayaigjtxp] = 1;
    aux->fading[st->ohvavdnio] = 1;
    aux->fade_target[st->mjeqtdqvm] = SB26_BACKGROUND_COLOR;
...
static void sb26_push_frame(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux,
    ...
    int32_t pw = s->atlas->pw;
    int8_t wanted_colour = arc_sprite_pixels(s, target_slot)[(size_t)1 * pw + 1];
    int32_t new_rank = 0;
...
static void sb26_check(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux,
                       int32_t *next_order) {
    int32_t depth = aux->stack_depth;
    int32_t top_frame = aux->stack_frame[depth - 1];
    int32_t top_slot = aux->stack_slot[depth - 1];
    ...
    int32_t pw = s->atlas->pw;
    int32_t card_slot = st->card_slots[(size_t)level * SB26_MAX_CARDS + aux->pmygakdvy];
    int confirmed = arc_sprite_pixels(s, card_slot)[(size_t)1 * pw + 1] != -1;

    if (!confirmed && is_plain) {
        int match = arc_sprite_pixels(s, target_slot)[(size_t)1 * pw + 1] ==
...
static void sb26_tick_modqnpqfi(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux,
                                int32_t *next_order) {
...
void sb26_on_set_level(ArcSprites *s, const Sb26Static *st, int32_t level, Sb26Aux *aux,
                       int32_t *next_order) {
    sb26_zero_aux(aux);
...
void sb26_step_once(ArcSprites *s, const ArcCamera *camera, ArcRenderScratch *scratch,
...
void sb26_render_interface(int8_t *frame, const ArcSprites *s, const ArcCamera *camera,
                           ArcRenderScratch *scratch, const Sb26Static *st, const Sb26Aux *aux) {
    (void)st;
    ArcCamera sentinel = *camera;
    ...
    int32_t filled = (ARC_FRAME_SIZE * aux->energy + SB26_INITIAL_ENERGY - 1) / SB26_INITIAL_ENERGY;
