...
enum {
	SB26_PITCH = 6,
	SB26_SLOT_ORIGIN = 2,
	SB26_INITIAL_ENERGY = 64,
	SB26_TWEEN_STEPS = 6
};
enum {
	SB26_BACKGROUND_COLOR = 4,
	SB26_Y_THRESHOLD = 53,
	SB26_HUD_EMPTY = 3,
	SB26_HUD_FILLED = 2
};
...
static void sb26_set_visible_slot(struct arc_sprites *s, int32_t slot,
				  int visible)
{
	if (slot < 0 || slot >= s->atlas->num_slots)
		return;
	arc_set_visible(s, slot, visible);
}

static uint8_t sb26_tracked(const struct arc_sprites *s,
			    const struct sb26_static *st, int32_t i)
{
...
static int32_t sb26_tween_lookup(const struct sb26_static *st, int32_t k,
				 int32_t a, int32_t b)
{
			...
			int32_t *occupant, int *is_frameref)
{
	...
	for (int32_t i = 0; i < n; i++) {
		if (!(s->alive[i] && s->x[i] == cx && s->y[i] == cy))
			continue;
		if (!found_item &&
		    s->tags[(size_t)i * a->num_tags + st->item_tag]) {
			item_slot = i;
			found_item = 1;
		}
		if (!found_spot &&
	...
	*is_frameref =
		found_item && st->is_frameref[(size_t)level * n + item_slot];
}

static void sb26_border_redraw(struct arc_sprites *s, int32_t slot,
			       int32_t width, int corner)
{
	...
	for (int32_t row = 0; row < ph; row++) {
		for (int32_t col = 0; col < pw; col++) {
			...
			int top_bottom =
				(row == 0 || row == h - 1) && col < width;
			int border = row < h && (left_right || top_bottom);
			if (corner) {
				int k = 3;
				int erase_sides = row >= k &&
						  row <= h - k - 1 &&
						  left_right;
				int erase_caps = (row == 0 || row == h - 1) &&
						 col >= k &&
						 col <= width - k - 1;
				if (erase_sides || erase_caps)
					border = 0;
			}
			patch[(size_t)row * pw + col] =
				(int8_t)(border ? colour : -1);
		}
	}
}

static void sb26_rqtmgdegai(struct arc_sprites *s, const struct sb26_static *st,
			    int32_t slot, int32_t offset, int32_t fx,
			    int32_t fy, int32_t tx, int32_t ty, int32_t k)
{
	int32_t x = sb26_tween_lookup(st, k, fx - offset, tx - offset);
	int32_t y = sb26_tween_lookup(st, k, fy - offset, ty - offset);
	arc_set_position(s, slot, x, y);
}

static void sb26_dagvovvbpp(struct arc_sprites *s, const struct sb26_static *st,
			    int32_t slot, int32_t offset, int32_t fx,
			    int32_t fy, int32_t fw, int32_t tx, int32_t ty,
			    int32_t tw, int32_t k)
{
	sb26_rqtmgdegai(s, st, slot, offset, fx, fy, tx, ty, k);
	int32_t width =
		sb26_tween_lookup(st, k, fw + 2 * offset, tw + 2 * offset);
	...
	if (probe_col >= pw)
		probe_col = pw - 1;
	int redraw =
		arc_sprite_pixels(s, slot)[(size_t)1 * pw + probe_col] != 1;
	if (redraw)
		sb26_border_redraw(s, slot, width, 1);
}

static void sb26_apply_tween_slot(struct arc_sprites *s,
				  const struct sb26_static *st, int32_t role,
				  int32_t fx, int32_t fy, int32_t fw,
				  int32_t tx, int32_t ty, int32_t tw, int32_t k)
{
	if (role < 0)
		return;
	int32_t idx = role;
	...
	switch (idx) {
	case 0:
		sb26_dagvovvbpp(s, st, st->mjeqtdqvm, 1, fx, fy, fw, tx, ty, tw,
				k);
		break;
	case 1:
		sb26_dagvovvbpp(s, st, st->ayaigjtxp, 0, fx, fy, fw, tx, ty, tw,
				k);
		break;
	default:
...
static void sb26_paint_row(struct arc_sprites *s, int32_t slot, int32_t row,
			   int8_t colour)
{
	int32_t ph = s->atlas->ph, pw = s->atlas->pw;
	int32_t w = s->w[slot];
	...
	if (r < 0 || r >= ph)
		return;
	int8_t *patch = arc_sprite_pixels_mut(s, slot);
	for (int32_t c = 1; c < w - 1 && c < pw; c++)
		patch[(size_t)r * pw + c] = colour;
}

static int32_t sb26_click_hit(const struct arc_sprites *s,
			      const struct sb26_aux *aux,
			      const struct sb26_static *st, int32_t x,
			      int32_t y)
{
	const struct arc_atlas *a = s->atlas;
	...
	for (int32_t i = 0; i < a->num_slots; i++) {
		...
		if (s->blocking[i] == PIXEL_PERFECT) {
			...
			if (px >= a->pw)
				px = a->pw - 1;
			if (arc_sprite_pixels(s, i)[(size_t)py * a->pw + px] ==
			    -1)
				continue;
		}
		if (!s->tags[(size_t)i * a->num_tags + st->click_tag])
			continue;
		if (aux->click_off[i])
			continue;
...
static void sb26_next_level(const struct sb26_static *st, int32_t level,
...
static void sb26_push_undo(struct arc_sprites *s, const struct sb26_static *st,
			   struct sb26_aux *aux)
{
				...
				struct sb26_aux *aux)
{
	...
	for (int32_t i = 0; i < n; i++) {
		...
		s->x[i] = ux[i];
		s->y[i] = uy[i];
		arc_set_visible(s, i, uv[i]);
	}
	aux->undo_depth = new_depth;
	aux->selected = -1;
}

static void sb26_hjewbkcejq(struct arc_sprites *s, const struct sb26_static *st,
			    struct sb26_aux *aux, int32_t hit)
{
	const struct arc_atlas *a = s->atlas;
	...
	if (aux->selected < 0) {
		if (hit_is_item) {
			arc_color_remap(s, st->mrokwhyjs0, 0, 0, SB26_BLACK);
			arc_set_position(s, st->mrokwhyjs0, s->x[hit],
					 s->y[hit]);
			aux->selected = hit;
		}
		return;
	...
	if (hit_is_item) {
		if (hit == selected) {
			arc_color_remap(s, st->mrokwhyjs0, 0, 0,
					SB26_BACKGROUND_COLOR);
			aux->selected = -1;
			return;
		}
		int both_tray = s->y[selected] > SB26_Y_THRESHOLD &&
				s->y[hit] > SB26_Y_THRESHOLD;
		if (both_tray) {
			arc_set_position(s, st->mrokwhyjs0, s->x[hit],
					 s->y[hit]);
			aux->selected = hit;
		...
		arc_set_position(s, selected, hit_x, hit_y);
		arc_set_position(s, hit, sel_x, sel_y);
		arc_set_position(s, st->mrokwhyjs1, hit_x, hit_y);
		arc_color_remap(s, st->mrokwhyjs1, 0, 0, SB26_BLACK);
		aux->selected = -1;
		aux->artsfnufc = 0;
		aux->energy -= 1;
		return;
	...
	int32_t origin_spot = arc_get_sprite_at(
		s, s->x[selected], s->y[selected], st->spot_tag, 0);
	sb26_set_visible_slot(s, origin_spot, 1);
	sb26_set_visible_slot(s, hit, 0);
	arc_set_position(s, selected, s->x[hit], s->y[hit]);
	arc_set_position(s, st->mrokwhyjs1, s->x[hit], s->y[hit]);
	arc_color_remap(s, st->mrokwhyjs1, 0, 0, SB26_BLACK);
	aux->selected = -1;
	aux->artsfnufc = 0;
	aux->energy -= 1;
...
static void sb26_handle_click(struct arc_sprites *s,
			      const struct sb26_static *st,
			      struct sb26_aux *aux, int32_t action_x,
			      int32_t action_y)
{
	int32_t hit = sb26_click_hit(s, aux, st, action_x, action_y);
	if (hit >= 0)
		sb26_hjewbkcejq(s, st, aux, hit);
}

static void sb26_handle_default(struct arc_sprites *s,
				const struct sb26_static *st,
				struct sb26_aux *aux, int32_t action_id,
				...
				uint8_t *action_complete)
{
	...
	if (is7)
		sb26_handle_action7(s, st, aux);
	int32_t pre_artsfnufc = aux->artsfnufc;
	if (is6)
		sb26_handle_click(s, st, aux, action_x, action_y);
...
static void sb26_rfdjlhefnd(struct arc_sprites *s, const struct sb26_static *st,
			    int32_t level, struct sb26_aux *aux)
{
	int32_t n = st->num_slots;
	...
	arc_set_position(s, st->ohvavdnio, s->x[card0] - 1, s->y[card0] - 1);

	for (int32_t i = 0; i < n; i++) {
		int allow = st->allow_fixed[i] ||
			    st->card_mask[(size_t)level * n + i];
		int hide = s->alive[i] && arc_sprite_visible(s, i) && !allow;
		aux->peek_hidden[i] = (uint8_t)hide;
		if (hide)
			sb26_set_visible_slot(s, i, 0);
	}
...
static void sb26_handle_action5(struct arc_sprites *s,
				...
				struct sb26_aux *aux)
{
	...
	arc_render(s, camera, scratch, aux->snapshot);
...
static void sb26_advance_fades(struct arc_sprites *s, struct sb26_aux *aux)
{
	...
	for (int32_t i = 0; i < n; i++) {
		...
		for (int32_t p = 0; p < area; p++)
			if (patch[p] >= 0)
				patch[p] = (int8_t)step;
		if (step == aux->fade_target[i])
			aux->fading[i] = 0;
	}
}

static void sb26_tick_japgbruyb(struct arc_sprites *s,
				const struct sb26_static *st,
				struct sb26_aux *aux)
{
...
static void sb26_tick_lmvwmlqtw(const struct sb26_static *st, int32_t level,
				struct sb26_aux *aux, int32_t *score,
...
static void sb26_tick_xjxrqgaqw(struct arc_sprites *s,
				const struct sb26_static *st, int32_t level,
				struct sb26_aux *aux)
{
	...
	int is_item, is_fr;
	int32_t target_slot;
	sb26_lookup(s, st, level, top_frame, top_slot, &is_item, &target_slot,
		    &is_fr);
	int32_t card_slot =
		st->card_slots[(size_t)level * SB26_MAX_CARDS + aux->pmygakdvy];
	int8_t colour =
		step < 4 ? SB26_BLACK : arc_sprite_pixels(s, card_slot)[0];
...
static void sb26_tick_bbiavyren(struct arc_sprites *s,
				const struct sb26_static *st, int32_t level,
				struct sb26_aux *aux, int32_t *status,
				uint8_t *action_complete)
{
	...
	int32_t n = st->num_slots;
	for (int32_t i = 0; i < n; i++) {
		if (!st->card_mask[(size_t)level * n + i])
			continue;
		sb26_paint_row(s, i, row, -1);
	}

	int is_bg =
		arc_sprite_pixels(s, st->mjeqtdqvm)[0] == SB26_BACKGROUND_COLOR;
	if (is_bg) {
		int32_t root =
			st->qaagahahj[(size_t)level * SB26_MAX_FRAMES + 0];
		int32_t rx = s->x[root], ry = s->y[root], rw = s->w[root];
		sb26_dagvovvbpp(s, st, st->mjeqtdqvm, 1, rx, ry, rw, rx, ry, rw,
				0);
		arc_color_remap(s, st->mjeqtdqvm, 0, 0,
				SB26_BACKGROUND_COLOR - 1);
	...
	if (step_new >= 8) {
		for (int32_t k = 0; k < SB26_CURSOR_SLOTS; k++)
			if (k < aux->cursor_count)
				s->alive[st->cursor_base + k] = 0;
		sb26_set_visible_slot(s, st->ayaigjtxp, 0);
		for (int32_t i = 0; i < n; i++)
			if (aux->peek_hidden[i])
				sb26_set_visible_slot(s, i, 1);
		aux->bbiavyren = -1;
		aux->cursor_count = 0;
		aux->peek_active = 0;
		for (int32_t i = 0; i < n; i++)
		...
		*action_complete = 1;
		if (aux->energy == 0)
			*status = SB26_GAME_OVER;
	}
}

static void sb26_tick_ftyhvmeft(struct arc_sprites *s,
				const struct sb26_static *st, int32_t level,
				struct sb26_aux *aux)
{
	...
	arc_color_remap(s, st->ayaigjtxp, 0, 0, colour);
	arc_color_remap(s, st->ohvavdnio, 0, 0, colour);
	int32_t step_new = step + 1;
	...
	if (step_new >= 18) {
		...
		for (int32_t k = 0; k < SB26_CURSOR_SLOTS; k++) {
			...
			aux->fade_target[st->cursor_base + k] =
				SB26_BACKGROUND_COLOR;
		}
		aux->fading[st->ohvavdnio] = 1;
...
static void sb26_tick_artsfnufc(struct arc_sprites *s,
				const struct sb26_static *st,
				struct sb26_aux *aux, int32_t *status,
				uint8_t *action_complete)
{
	...
	if (colour < 0)
		colour = 0;
	arc_color_remap(s, st->mrokwhyjs0, 0, 0, (int8_t)colour);
	arc_color_remap(s, st->mrokwhyjs1, 0, 0, (int8_t)colour);
	aux->artsfnufc += 1;
	int done = arc_sprite_pixels(s, st->mrokwhyjs0)[0] ==
		   SB26_BACKGROUND_COLOR;
	if (done) {
		aux->artsfnufc = -1;
		*action_complete = 1;
...
static void sb26_tick_tween(struct arc_sprites *s, const struct sb26_static *st,
			    int32_t level, struct sb26_aux *aux)
{
	int32_t k = aux->jlcrtmkes + 1;
	sb26_apply_tween_slot(s, st, aux->tw_role_a, aux->tw_fx_a, aux->tw_fy_a,
			      aux->tw_fw_a, aux->tw_tx_a, aux->tw_ty_a,
			      aux->tw_tw_a, k);
	sb26_apply_tween_slot(s, st, aux->tw_role_b, aux->tw_fx_b, aux->tw_fy_b,
			      aux->tw_fw_b, aux->tw_tx_b, aux->tw_ty_b,
			      aux->tw_tw_b, k);
	aux->jlcrtmkes = k;
	...
	if (k == SB26_TWEEN_STEPS) {
		...
		int is_item, is_fr;
		int32_t occ;
		sb26_lookup(s, st, level, top_frame, top_slot, &is_item, &occ,
			    &is_fr);
...
static void sb26_sibihgzarf(struct arc_sprites *s, const struct sb26_static *st,
			    struct sb26_aux *aux)
{
	arc_color_remap(s, st->ayaigjtxp, 0, 0, SB26_RED);
	arc_color_remap(s, st->ohvavdnio, 0, 0, SB26_RED);
	aux->ftyhvmeft = 0;
}

static void sb26_pop_cursor(struct arc_sprites *s, const struct sb26_static *st,
			    struct sb26_aux *aux)
{
	int32_t idx = aux->cursor_count - 1;
	if (idx >= 0 && idx < SB26_CURSOR_SLOTS)
		s->alive[st->cursor_base + idx] = 0;
	aux->cursor_count = idx;
	aux->ppsxsxiod = 0;
}

static void sb26_pop_to_parent(struct arc_sprites *s,
			       const struct sb26_static *st, int32_t level,
			       struct sb26_aux *aux, int32_t target_slot)
{
	int32_t depth = aux->stack_depth;
	int32_t top_frame = aux->stack_frame[depth - 1];
	...
	sb26_lookup(s, st, level, parent_frame, parent_slot, &is_item,
		    &parent_target, &is_fr);

	int32_t from_x = s->x[top_frame], from_y = s->y[top_frame],
		from_w = s->w[top_frame];
	int32_t to_x = s->x[parent_frame], to_y = s->y[parent_frame],
		to_w = s->w[parent_frame];
	int32_t old_x = s->x[target_slot], old_y = s->y[target_slot],
		old_w = s->w[target_slot];
	int32_t new_x = s->x[parent_target], new_y = s->y[parent_target],
		new_w = s->w[parent_target];
...
static void sb26_exhausted(struct arc_sprites *s, const struct sb26_static *st,
			   int32_t level, struct sb26_aux *aux)
{
	aux->fading[st->mjeqtdqvm] = 1;
	aux->fading[st->ayaigjtxp] = 1;
	...
	if (narrow)
		sb26_border_redraw(s, st->oyvbxwyug, width, 0);
	int32_t pos_x2 = narrow ? pos_x : pos_x - 28;
	arc_set_position(s, st->oyvbxwyug, pos_x2, pos_y);
	sb26_set_visible_slot(s, st->oyvbxwyug, 1);

	aux->japgbruyb = 0;
}

static void sb26_push_frame(struct arc_sprites *s, const struct sb26_static *st,
			    int32_t level, struct sb26_aux *aux,
			    int32_t *next_order, int32_t target_slot)
{
	...
	arc_set_position(s, new_cursor, s->x[target_slot], s->y[target_slot]);
	arc_color_remap(s, new_cursor, 0, 0, SB26_GREY);
	arc_add_sprite(s, new_cursor, *next_order);
	sb26_set_visible_slot(s, new_cursor, 1);

	int32_t pw = s->atlas->pw;
	int8_t wanted_colour =
		arc_sprite_pixels(s, target_slot)[(size_t)1 * pw + 1];
	int32_t new_rank = 0;
	for (int32_t k = 0; k < SB26_MAX_FRAMES; k++) {
		int32_t fs = st->qaagahahj[(size_t)level * SB26_MAX_FRAMES + k];
		int32_t clipped = fs < 0 ? 0 : fs;
		int8_t fc = arc_sprite_pixels(s, clipped)[0];
	...
	int32_t new_frame =
		st->qaagahahj[(size_t)level * SB26_MAX_FRAMES + new_rank];

	int32_t from_x = s->x[top_frame], from_y = s->y[top_frame],
		from_w = s->w[top_frame];
	int32_t to_x = s->x[new_frame], to_y = s->y[new_frame],
		to_w = s->w[new_frame];
	int32_t old_x = s->x[target_slot], old_y = s->y[target_slot],
		old_w = s->w[target_slot];
	int is_item, is_fr;
	...
	int32_t new_x = s->x[first_child], new_y = s->y[first_child],
		new_w = s->w[first_child];
...
static void sb26_check(struct arc_sprites *s, const struct sb26_static *st,
		       int32_t level, struct sb26_aux *aux, int32_t *next_order)
{
	int32_t depth = aux->stack_depth;
	int32_t top_frame = aux->stack_frame[depth - 1];
	int32_t top_slot = aux->stack_slot[depth - 1];
	...
	int32_t card_slot =
		st->card_slots[(size_t)level * SB26_MAX_CARDS + aux->pmygakdvy];
	int confirmed =
		arc_sprite_pixels(s, card_slot)[(size_t)1 * pw + 1] != -1;

	if (!confirmed && is_plain) {
		int match =
			arc_sprite_pixels(s, target_slot)[(size_t)1 * pw + 1] ==
			arc_sprite_pixels(s, card_slot)[0];
		if (match)
			aux->xjxrqgaqw = 0;
		else
	...
	if (win) {
		...
		aux->fade_target[st->ayaigjtxp] = SB26_BACKGROUND_COLOR;
		for (int32_t i = 0; i < n; i++) {
			if (!st->bg_mask[(size_t)level * n + i])
				continue;
			aux->fading[i] = 1;
	...
	if (advance_cond) {
		if (aux->ppsxsxiod)
			sb26_pop_cursor(s, st, aux);
		depth = aux->stack_depth;
		top_frame = aux->stack_frame[depth - 1];
		top_slot = aux->stack_slot[depth - 1];
		int32_t new_slot = top_slot + 1;
		int32_t nslots =
			st->frame_children[(size_t)level * st->num_slots +
					   top_frame];
		if (new_slot < nslots) {
			int is_item2, is_fr2;
			int32_t new_child;
			sb26_lookup(s, st, level, top_frame, new_slot,
				    &is_item2, &new_child, &is_fr2);
			int32_t from_x = s->x[target_slot],
				from_y = s->y[target_slot],
				from_w = s->w[target_slot];
			int32_t to_x = s->x[new_child], to_y = s->y[new_child],
				to_w = s->w[new_child];
			int32_t new_pmy = aux->pmygakdvy + 1;
			int32_t new_card =
				st->card_slots[(size_t)level * SB26_MAX_CARDS +
					       new_pmy];
	...
	if (is_item && is_frameref) {
		...
		for (int32_t j = 0; j < d2 - 1; j++) {
			if (aux->stack_frame[j] == top_frame &&
			    aux->stack_slot[j] == 0) {
				earlier = 1;
				break;
			}
		...
		int guard =
			top_slot == 0 && earlier && prev_slot_zero && d2 >= 2;
		if (guard)
			sb26_sibihgzarf(s, st, aux);
		else
			sb26_push_frame(s, st, level, aux, next_order,
					target_slot);
	} else {
		sb26_sibihgzarf(s, st, aux);
	}
...
static void sb26_tick_modqnpqfi(struct arc_sprites *s,
				const struct sb26_static *st, int32_t level,
				struct sb26_aux *aux, int32_t *next_order)
{
	...
	aux->modqnpqfi = m;
	if (m == 0)
		sb26_check(s, st, level, aux, next_order);
}
...
void sb26_aux_alloc(struct sb26_aux *aux, int32_t num_slots)
{
	...
	aux->undo_x =
		calloc((size_t)SB26_MAX_UNDO * num_slots, sizeof(int32_t));
	aux->undo_y =
		calloc((size_t)SB26_MAX_UNDO * num_slots, sizeof(int32_t));
	aux->undo_vis =
		calloc((size_t)SB26_MAX_UNDO * num_slots, sizeof(uint8_t));
...
void sb26_zero_aux(struct sb26_aux *aux)
{
	...
	aux->jlcrtmkes = -1;
	aux->tw_role_a = -1;
	aux->tw_fx_a = aux->tw_fy_a = aux->tw_fw_a = aux->tw_tx_a =
		aux->tw_ty_a = aux->tw_tw_a = 0;
	aux->tw_role_b = -1;
	aux->tw_fx_b = aux->tw_fy_b = aux->tw_fw_b = aux->tw_tx_b =
		aux->tw_ty_b = aux->tw_tw_b = 0;
	aux->peek_active = 0;
	memset(aux->snapshot, 0, sizeof aux->snapshot);
}

void sb26_on_set_level(struct arc_sprites *s, const struct sb26_static *st,
		       int32_t level, struct sb26_aux *aux, int32_t *next_order)
{
	sb26_zero_aux(aux);
	...
	for (int32_t i = 0; i < n; i++) {
		...
		aux->click_off[i] =
			(uint8_t)(is_item && s->y[i] <= SB26_Y_THRESHOLD);
	}
	...
	for (int32_t k = 0; k < SB26_MAX_TRAY; k++) {
		...
		if (valid) {
			int32_t src = st->tray_item_slot[(size_t)level *
								 SB26_MAX_TRAY +
							 k];
			s->x[ghost] = s->x[src];
			s->y[ghost] = s->y[src];
	...
	for (int32_t k = 0; k < SB26_MAX_TRAY; k++)
		if (k < st->n_tray[level])
			sb26_set_visible_slot(s, st->tray_ghost_base + k, 0);
	*next_order += st->n_tray[level];

	arc_set_position(s, st->mrokwhyjs0, -10, -10);
	arc_color_remap(s, st->mrokwhyjs0, 0, 0, SB26_BACKGROUND_COLOR);
	arc_add_sprite(s, st->mrokwhyjs0, *next_order);
	sb26_set_visible_slot(s, st->mrokwhyjs0, 1);
	(*next_order)++;

	arc_set_position(s, st->mrokwhyjs1, -10, -10);
	arc_color_remap(s, st->mrokwhyjs1, 0, 0, SB26_BACKGROUND_COLOR);
	arc_add_sprite(s, st->mrokwhyjs1, *next_order);
	...
	arc_add_sprite(s, st->mjeqtdqvm, *next_order);
	sb26_set_visible_slot(s, st->mjeqtdqvm, 1);
	(*next_order)++;

	arc_color_remap(s, st->ayaigjtxp, 0, 0, SB26_BACKGROUND_COLOR);
	arc_add_sprite(s, st->ayaigjtxp, *next_order);
	sb26_set_visible_slot(s, st->ayaigjtxp, 0);
	(*next_order)++;

	arc_color_remap(s, st->ohvavdnio, 0, 0, SB26_PINK);
	arc_set_position(s, st->ohvavdnio, -100, -100);
	arc_add_sprite(s, st->ohvavdnio, *next_order);
	sb26_set_visible_slot(s, st->ohvavdnio, 1);
	(*next_order)++;

	arc_add_sprite(s, st->oyvbxwyug, *next_order);
...
void sb26_step_once(struct arc_sprites *s, const struct arc_camera *camera,
		    struct arc_render_scratch *scratch,
		    const struct sb26_static *st, int32_t level,
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (aux->lmvwmlqtw >= 0) {
		sb26_tick_lmvwmlqtw(st, level, aux, score, status, next_level,
				    action_complete);
		return;
	}
	if (aux->xjxrqgaqw >= 0) {
	...
	sb26_handle_default(s, st, aux, action_id, action_x, action_y,
			    action_complete);
}

void sb26_render_interface(int8_t *frame, const struct arc_sprites *s,
			   const struct arc_camera *camera,
			   struct arc_render_scratch *scratch,
			   const struct sb26_static *st,
			   const struct sb26_aux *aux)
{
	(void)st;
	struct arc_camera sentinel = *camera;
	sentinel.background = -1;
	sentinel.letter_box = -1;
	int8_t coverage[ARC_FRAME_SIZE * ARC_FRAME_SIZE];
	arc_render(s, &sentinel, scratch, coverage);
	if (aux->peek_active) {
		for (int32_t i = 0; i < ARC_FRAME_SIZE * ARC_FRAME_SIZE; i++)
			if (coverage[i] == -1)
				frame[i] = aux->snapshot[i];
	}

	int32_t filled =
		(ARC_FRAME_SIZE * aux->energy + SB26_INITIAL_ENERGY - 1) /
		SB26_INITIAL_ENERGY;
	int8_t *row = frame + (size_t)SB26_Y_THRESHOLD * ARC_FRAME_SIZE;
	for (int32_t c = 0; c < ARC_FRAME_SIZE; c++)
		row[c] =
			(int8_t)(c < filled ? SB26_HUD_FILLED : SB26_HUD_EMPTY);
}
