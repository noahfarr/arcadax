...
static int tu93_attached(const ArcSprites *s, const Tu93Static *st, const Tu93Aux *aux,
                         int32_t i) {
    int32_t ridx = tu93_ridx(aux->rotation[i]);
    int32_t row, col;
    tu93_flag_rc(st, ridx, s->h[i], &row, &col);
    return arc_sprite_pixels(s, i)[(size_t)row * st->pw + col] == TU93_ATTACHED_COLOR;
}

static void tu93_set_attached_one(ArcSprites *s, const Tu93Static *st, const Tu93Aux *aux,
                                  int32_t i) {
    int32_t ridx = tu93_ridx(aux->rotation[i]);
    int32_t row, col;
    tu93_flag_rc(st, ridx, s->h[i], &row, &col);
...
static void tu93_apply_rotation_one(ArcSprites *s, const Tu93Static *st, Tu93Aux *aux,
...
static int tu93_consume_one(ArcSprites *s, const Tu93Static *st, const Tu93Aux *aux,
...
static void tu93_push_pickup(ArcSprites *s, const Tu93Static *st, Tu93Aux *aux, int32_t level,
                             int32_t player) {
...
static void tu93_drift_advance(ArcSprites *s, const Tu93Static *st, const Tu93Aux *aux,
                               int32_t level) {
...
static void tu93_train_advance(ArcSprites *s, const Tu93Static *st, const Tu93Aux *aux,
                               int32_t level) {
...
static void tu93_drift_bounce(ArcSprites *s, const Tu93Static *st, Tu93Aux *aux,
                              int32_t level) {
    int32_t count = st->drift_count[level];
    const int32_t *slots = st->drift_slots + (size_t)level * st->max_drift;
    for (int32_t k = 0; k < count; k++) {
        ...
        int32_t cx = tu93_clip(s->x[i] + cdx * TU93_STEP_PX, 0, ARC_FRAME_SIZE - 1);
        int32_t cy = tu93_clip(s->y[i] + cdy * TU93_STEP_PX, 0, ARC_FRAME_SIZE - 1);
        int walkable = st->walkable[(size_t)level * ARC_FRAME_SIZE * ARC_FRAME_SIZE +
...
static void tu93_train_pickup_and_dequeue(ArcSprites *s, const Tu93Static *st, Tu93Aux *aux,
...
static void tu93_do_move(ArcSprites *s, const Tu93Static *st, Tu93Aux *aux, int32_t level,
...
static void tu93_phase0(ArcSprites *s, const Tu93Static *st, int32_t level, int32_t action_id,
    ...
    int32_t check_x = tu93_clip(px + cdx * TU93_STEP_PX, 0, ARC_FRAME_SIZE - 1);
    int32_t check_y = tu93_clip(py + cdy * TU93_STEP_PX, 0, ARC_FRAME_SIZE - 1);
    int walkable = st->walkable[(size_t)level * ARC_FRAME_SIZE * ARC_FRAME_SIZE +
...
static void tu93_phase1(ArcSprites *s, const Tu93Static *st, int32_t level, Tu93Aux *aux) {
    int32_t player = st->player_slot[level];
...
static void tu93_phase2(ArcSprites *s, const Tu93Static *st, int32_t level, Tu93Aux *aux,
...
void tu93_on_set_level(ArcSprites *sprites, const Tu93Static *st, int32_t level,
                       Tu93Aux *aux) {
    aux->phase = 0;
...
void tu93_step_once(ArcSprites *sprites, const ArcCamera *camera, const Tu93Static *st,
...
void tu93_render_interface(int8_t *frame, const ArcSprites *sprites, const ArcCamera *camera,
