...
static int tu93_behind(int32_t x, int32_t y, int32_t rotation, int32_t tx,
		       int32_t ty, int32_t dist)
{
	if (rotation == 90)
		return x == tx - dist && y == ty;
	if (rotation == 270)
		return x == tx + dist && y == ty;
	if (rotation == 180)
...
static void tu93_flag_rc(const struct tu93_static *st, int32_t ridx, int32_t h,
			 int32_t *row, int32_t *col)
{
	const int32_t *p = h == 5 ? st->flag_pos5 :
				    (h == 7 ? st->flag_pos7 : st->flag_pos3);
	*row = p[ridx * 2 + 0];
	*col = p[ridx * 2 + 1];
}

static int tu93_attached(const struct arc_sprites *s,
			 const struct tu93_static *st,
			 const struct tu93_aux *aux, int32_t i)
{
	int32_t ridx = tu93_ridx(aux->rotation[i]);
	int32_t row, col;
	tu93_flag_rc(st, ridx, s->h[i], &row, &col);
	return arc_sprite_pixels(s, i)[(size_t)row * st->pw + col] ==
	       TU93_ATTACHED_COLOR;
}

static void tu93_set_attached_one(struct arc_sprites *s,
				  const struct tu93_static *st,
				  const struct tu93_aux *aux, int32_t i)
{
	int32_t ridx = tu93_ridx(aux->rotation[i]);
...
static void tu93_apply_rotation_one(struct arc_sprites *s,
				    const struct tu93_static *st,
				    struct tu93_aux *aux, int32_t level,
				    int32_t i, int32_t new_rotation)
{
	...
	int8_t *dst = arc_sprite_pixels_mut(s, i);
	if (h == 5 || h == 7) {
		const int8_t *tmpl = (h == 5 ? st->template5 : st->template7) +
				     (size_t)ridx * area;
		int8_t colour =
			st->own_color[(size_t)level * st->num_slots + i];
		for (int32_t k = 0; k < area; k++)
			dst[k] = tmpl[k] >= 0 ? colour : tmpl[k];
	} else {
		const int8_t *base =
			st->base3 +
...
static int tu93_consume_one(struct arc_sprites *s, const struct tu93_static *st,
			    const struct tu93_aux *aux, int32_t level,
			    int32_t i, int mask)
{
	int32_t h = s->h[i];
	int grow5 = mask && h == 3;
	...
	if (grow5 || grow7) {
		int32_t ridx = tu93_ridx(aux->rotation[i]);
		int32_t area = st->ph * st->pw;
		const int8_t *tmpl = (grow5 ? st->template5 : st->template7) +
				     (size_t)ridx * area;
		int8_t colour =
			st->own_color[(size_t)level * st->num_slots + i];
		int8_t *dst = arc_sprite_pixels_mut(s, i);
...
static void tu93_push_pickup(struct arc_sprites *s,
			     const struct tu93_static *st, struct tu93_aux *aux,
			     int32_t level, int32_t player)
{
	...
	for (int32_t k = 0; k < count; k++) {
		...
		if (!tu93_behind(s->x[i], s->y[i], rot, px, py, TU93_CELL_PX))
			continue;
...
static void tu93_drift_advance(struct arc_sprites *s,
			       const struct tu93_static *st,
			       const struct tu93_aux *aux, int32_t level)
{
...
static void tu93_train_advance(struct arc_sprites *s,
			       const struct tu93_static *st,
			       const struct tu93_aux *aux, int32_t level)
{
...
static void tu93_drift_bounce(struct arc_sprites *s,
			      const struct tu93_static *st,
			      struct tu93_aux *aux, int32_t level)
{
	int32_t count = st->drift_count[level];
	...
	for (int32_t k = 0; k < count; k++) {
		...
		int32_t cx = tu93_clip(s->x[i] + cdx * TU93_STEP_PX, 0,
				       ARC_FRAME_SIZE - 1);
		int32_t cy = tu93_clip(s->y[i] + cdy * TU93_STEP_PX, 0,
				       ARC_FRAME_SIZE - 1);
		int walkable = st->walkable[(size_t)level * ARC_FRAME_SIZE *
						    ARC_FRAME_SIZE +
					    (size_t)cy * ARC_FRAME_SIZE + cx];
		if (walkable)
			continue;
...
static void tu93_train_pickup_and_dequeue(struct arc_sprites *s,
					  const struct tu93_static *st,
					  struct tu93_aux *aux, int32_t level,
					  int32_t player)
{
	...
	for (int32_t k = 0; k < count; k++) {
		...
		int32_t rot = aux->rotation[i];
		int hit = !already_attached &&
			  tu93_behind(s->x[i], s->y[i], rot, px, py,
				      TU93_TRAIN_RANGE_PX);
		if (hit) {
			tu93_set_attached_one(s, st, aux, i);
			aux->queue[i][0] = rot;
...
static void tu93_finish(struct tu93_aux *aux, int32_t *status,
...
static void tu93_do_move(struct arc_sprites *s, const struct tu93_static *st,
			 struct tu93_aux *aux, int32_t level, int32_t player,
			 int32_t px, int32_t py, int32_t rotation)
{
	int32_t count = st->train_count[level];
	const int32_t *slots = st->train_slots + (size_t)level * st->max_train;
	for (int32_t k = 0; k < count; k++) {
		...
		if (!tu93_attached(s, st, aux, i))
			continue;
		int32_t pos =
			tu93_clip(aux->queue_len[i], 0, TU93_MAX_QUEUE - 1);
		aux->queue[i][pos] = rotation;
		aux->queue_len[i] = aux->queue_len[i] + 1 < TU93_MAX_QUEUE ?
					    aux->queue_len[i] + 1 :
					    TU93_MAX_QUEUE;
	}
	int32_t dx, dy;
...
static void tu93_phase0(struct arc_sprites *s, const struct tu93_static *st,
			int32_t level, int32_t action_id, struct tu93_aux *aux,
			int32_t *status, uint8_t *action_complete)
{
	int32_t player = st->player_slot[level];
	int32_t px = s->x[player], py = s->y[player];
	int32_t rotation =
		action_id == TU93_ACTION1 ?
			0 :
			(action_id == TU93_ACTION2 ?
				 180 :
				 (action_id == TU93_ACTION3 ?
					  270 :
					  (action_id == TU93_ACTION4 ? 90 :
								       -1)));
	int valid_action = rotation >= 0;
	if (valid_action)
		aux->steps = aux->steps > 1 ? aux->steps - 1 : 0;

	int32_t cdx, cdy;
	tu93_move1(rotation, &cdx, &cdy);
	int32_t check_x =
		tu93_clip(px + cdx * TU93_STEP_PX, 0, ARC_FRAME_SIZE - 1);
	int32_t check_y =
		tu93_clip(py + cdy * TU93_STEP_PX, 0, ARC_FRAME_SIZE - 1);
	int walkable =
		st->walkable[(size_t)level * ARC_FRAME_SIZE * ARC_FRAME_SIZE +
			     (size_t)check_y * ARC_FRAME_SIZE + check_x];
...
static void tu93_phase1(struct arc_sprites *s, const struct tu93_static *st,
			int32_t level, struct tu93_aux *aux)
{
	int32_t player = st->player_slot[level];
	int32_t board_x = st->board_xy[level * 2 + 0],
		board_y = st->board_xy[level * 2 + 1];
	int32_t px = s->x[player], py = s->y[player];
...
static void tu93_phase2(struct arc_sprites *s, const struct tu93_static *st,
			int32_t level, struct tu93_aux *aux, int32_t *score,
			...
			uint8_t *action_complete)
{
	...
	for (int32_t k = 0; k < count; k++) {
		...
		int is_drift_i =
			st->is_drift[(size_t)level * st->num_slots + i];
		int is_train_i =
			st->is_train[(size_t)level * st->num_slots + i];
		int attached_i = (is_push_i || is_train_i) ?
					 tu93_attached(s, st, aux, i) :
					 0;
		int movable =
			((is_push_i || is_train_i) && attached_i) || is_drift_i;
		if (!movable)
			continue;
		int aligned_i =
			tu93_mod(s->y[i] - board_y, TU93_CELL_PX) == 0 &&
		...
		int32_t cx = s->x[i] + s->w[i] / 2, cy = s->y[i] + s->h[i] / 2;
		if (s->alive[player] && cx == player_cx && cy == player_cy)
			pending_n++;
	}
...
void tu93_zero_aux(struct tu93_aux *aux)
{
	...
	for (int32_t i = 0; i < TU93_NUM_SLOTS; i++) {
		aux->rotation[i] = 0;
		aux->queue_len[i] = 0;
		for (int32_t j = 0; j < TU93_MAX_QUEUE; j++)
			aux->queue[i][j] = 0;
	}
	aux->steps = 0;
}
		       ...
		       struct tu93_aux *aux)
{
	...
	for (int32_t i = 0; i < st->num_slots; i++) {
		aux->rotation[i] =
			st->init_rotation[(size_t)level * st->num_slots + i];
		aux->queue_len[i] = 0;
		for (int32_t j = 0; j < TU93_MAX_QUEUE; j++)
			aux->queue[i][j] = 0;
		sprites->layer[i] = 0;
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (phase == 0) {
		tu93_phase0(sprites, st, level, action_id, aux, status,
			    action_complete);
	} else if (phase == 1) {
		tu93_phase1(sprites, st, level, aux);
	} else {
		tu93_phase2(sprites, st, level, aux, score, status, next_level,
			    action_complete);
	}
}
