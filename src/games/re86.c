...
static inline int8_t center_value(const struct arc_sprites *s, int32_t slot)
{
	...
	return arc_sprite_pixels(s,
				 slot)[(s->h[slot] / 2) * pw + s->w[slot] / 2];
}

static inline void set_center(struct arc_sprites *s, int32_t slot, int8_t value)
{
	int32_t pw = s->atlas->pw;
	mut_pixels(s, slot)[(s->h[slot] / 2) * pw + s->w[slot] / 2] = value;
}

static int8_t dynamic_own_color(const struct arc_sprites *s, int32_t slot)
{
...
static void first_opaque(const struct arc_sprites *s, int32_t slot,
			 int32_t *d_out, int32_t *u_out)
{
	const struct arc_atlas *a = s->atlas;
	const int8_t *p = arc_sprite_pixels(s, slot);
...
static void recompute_natural_center(struct arc_sprites *s,
				     const struct re86_static *st,
				     int32_t level, int32_t slot)
{
	...
	int8_t plain_value =
		(center_row == u || center_col == d) ? own : (int8_t)-1;
	int32_t idx = level * st->num_slots + slot;
	int8_t value;
	if (st->is_reshape[idx])
		value = -1;
	else if (st->is_solid_center[idx])
		value = own;
	else
		value = plain_value;
	int32_t pw = s->atlas->pw;
...
static void hollow_rect(int8_t *out, int32_t pw_stride, int32_t h, int32_t w,
			int8_t color)
{
...
static inline void undo_move(struct arc_sprites *s, int32_t cursor, int32_t dx,
			     int32_t dy)
{
	s->x[cursor] -= dx;
	s->y[cursor] -= dy;
...
static void reshape_effect(struct arc_sprites *s, int32_t cursor, int32_t dx,
			   int32_t dy)
{
	...
	if (dx != 0) {
		if (w > MIN_RESHAPE_DIM) {
			...
			hollow_rect(mut_pixels(s, cursor), pw, new_h, new_w,
				    color);
...
static void redraw_column(struct arc_sprites *s, int32_t cursor, int32_t d,
			  int32_t delta, int32_t u, int8_t color)
{
	const struct arc_atlas *a = s->atlas;
	...
	int32_t dc = d + delta;
	if (dc >= 0 && dc < pw)
		for (int32_t r = 0; r < h && r < ph; r++)
			p[r * pw + dc] = color;
	p[u * pw + d] = color;
	p[(h / 2) * pw + w / 2] = ACTIVE_MARKER;
}

static void redraw_row(struct arc_sprites *s, int32_t cursor, int32_t u,
		       int32_t delta, int32_t d, int8_t color)
{
	const struct arc_atlas *a = s->atlas;
	int32_t ph = a->ph, pw = a->pw;
	int8_t *p = mut_pixels(s, cursor);
	...
	int32_t ur = u + delta;
	if (ur >= 0 && ur < ph)
		for (int32_t c = 0; c < w && c < pw; c++)
			p[ur * pw + c] = color;
	p[u * pw + d] = color;
	p[(h / 2) * pw + w / 2] = ACTIVE_MARKER;
}

static void shift_window_effect(struct arc_sprites *s, int32_t cursor,
				int32_t wall, int32_t dx, int32_t dy)
{
	int8_t color = dynamic_own_color(s, cursor);
	int32_t d, u;
	first_opaque(s, cursor, &d, &u);
	int32_t x = s->x[cursor], y = s->y[cursor], w = s->w[cursor],
		h = s->h[cursor];
	int32_t wx = s->x[wall], wy = s->y[wall], ww = s->w[wall],
		wh = s->h[wall];
	...
	if (dx != 0) {
		...
		if (case_id == 1) {
			if (room_shrink)
				redraw_column(s, cursor, d, shrink, u, color);
			else
				undo_move(s, cursor, dx, dy);
		} else {
			undo_move(s, cursor, dx, dy);
			if (room_grow)
				redraw_column(s, cursor, d, grow, u, color);
		}
	} else {
		...
		if (case_id == 1) {
			if (room_shrink)
				redraw_row(s, cursor, u, shrink, d, color);
			else
				undo_move(s, cursor, dx, dy);
		} else {
			undo_move(s, cursor, dx, dy);
			if (room_grow)
				redraw_row(s, cursor, u, grow, d, color);
		}
	}
}

static int pixel_overlap_pair(const struct arc_sprites *s, int32_t i, int32_t j)
{
	const struct arc_atlas *a = s->atlas;
	int32_t ph = a->ph, pw = a->pw;
...
static int re86_collides_pair(const struct arc_sprites *s, int32_t i, int32_t j)
{
	if (!s->alive[j])
		return 0;
	int32_t xi = s->x[i], yi = s->y[i], wi = s->w[i], hi = s->h[i];
	int32_t xj = s->x[j], yj = s->y[j], wj = s->w[j], hj = s->h[j];
	if (!(xi < xj + wj && xi + wi > xj && yi < yj + hj && yi + hi > yj))
		return 0;
	if (!arc_sprite_collidable(s, i) || !arc_sprite_collidable(s, j))
		return 0;
	if (s->blocking[i] == NOT_BLOCKED || s->blocking[j] == NOT_BLOCKED)
		return 0;
	int pixel_perfect = s->blocking[i] == PIXEL_PERFECT ||
			    s->blocking[j] == PIXEL_PERFECT;
	return !pixel_perfect || pixel_overlap_pair(s, i, j);
}

static int32_t target_check(struct arc_sprites *s, struct re86_aux *aux,
			    const struct re86_static *st, int32_t level,
			    int32_t cursor)
{
	int32_t n = st->num_flood_targets[level];
	const int32_t *targets =
		st->flood_target_slots + (size_t)level * st->max_flood_targets;
	...
	for (int32_t k = 0; k < n; k++) {
		...
		if (!s->alive[j])
			continue;
		if (!re86_collides_pair(s, cursor, j))
			continue;
		int8_t fill = arc_sprite_pixels(s, j)[pw + 1];
		if (own == fill)
			continue;
		target = j;
		break;
	...
	int32_t tx = s->x[target], ty = s->y[target], tw = s->w[target],
		th = s->h[target];
	int32_t cx = s->x[cursor], cy = s->y[cursor];
	int8_t fill = arc_sprite_pixels(s, target)[pw + 1];
...
static int32_t move_and_collide(struct arc_sprites *s, struct re86_aux *aux,
				const struct re86_static *st, int32_t level,
				...
				int32_t camera_width, int32_t camera_height)
{
	...
	if (!(cx >= 0 && cy >= 0 && cx < camera_width && cy < camera_height))
		return -1;
	...
	for (int32_t k = 0; k < nwalls; k++) {
		...
		if (!s->alive[i])
			continue;
		if (!re86_collides_pair(s, cursor, i))
			continue;
		if (st->is_reshape[level * st->num_slots + cursor])
			reshape_effect(s, cursor, dx, dy);
		else
			shift_window_effect(s, cursor, i, dx, dy);
	}
...
static int32_t find_active(const struct arc_sprites *s,
			   const struct re86_static *st, int32_t level)
{
	int32_t nc = st->num_cursors[level];
	const int32_t *cursors =
		st->cursor_slot_by_rank + (size_t)level * st->max_cursors;
	for (int32_t k = 0; k < nc; k++) {
		int32_t slot = cursors[k];
		if (s->alive[slot] && center_value(s, slot) == ACTIVE_MARKER)
			return slot;
	}
	return -1;
}

static inline void spend_step(struct re86_aux *aux)
{
	aux->steps = aux->steps > 0 ? aux->steps - 1 : 0;
}

static int win_check(const struct arc_sprites *s, const struct re86_static *st,
		     int32_t level)
{
	const struct arc_atlas *a = s->atlas;
	int32_t ph = a->ph, pw = a->pw;
	int32_t canvas_h = ARC_FRAME_SIZE + 2 * ph,
		canvas_w = ARC_FRAME_SIZE + 2 * pw;
	int8_t canvas[3 * ARC_FRAME_SIZE * 3 * ARC_FRAME_SIZE];
	memset(canvas, -1, (size_t)canvas_h * canvas_w);
	for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
		memcpy(canvas + (size_t)(ph + r) * canvas_w + pw,
		       st->canvas_template + (size_t)r * ARC_FRAME_SIZE,
		       ARC_FRAME_SIZE);
	...
	const int8_t *target_pixels = arc_sprite_pixels(s, target_slot);
	for (int32_t r = 0; r < ARC_FRAME_SIZE; r++) {
...
static void tail(struct arc_sprites *s, struct re86_aux *aux,
		 struct re86_engine *engine, const struct re86_static *st)
{
	...
	if (won) {
		engine->score += 1;
		engine->next_level = 1;
		if (level == st->num_levels - 1)
			engine->status = RE86_WIN;
	} else if (lost) {
...
static void directional(struct arc_sprites *s, struct re86_aux *aux,
			struct re86_engine *engine,
			const struct re86_static *st, int32_t dx, int32_t dy)
{
	spend_step(aux);
	int32_t level = engine->level_index;
	...
	int32_t touched =
		move_and_collide(s, aux, st, level, cursor, dx, dy,
...
static void cycle_step(struct arc_sprites *s, struct re86_aux *aux,
		       struct re86_engine *engine, const struct re86_static *st)
{
	spend_step(aux);
	int32_t level = engine->level_index;
	...
	if (active >= 0) {
		...
		const int32_t *cursors = st->cursor_slot_by_rank +
					 (size_t)level * st->max_cursors;
		int32_t next_slot = cursors[next_rank];
		for (int32_t k = 0; k < nc; k++)
			if (s->alive[cursors[k]])
				recompute_natural_center(s, st, level,
							 cursors[k]);
...
static void flood_spread(struct arc_sprites *s, const struct re86_aux *aux)
{
	int32_t cursor = aux->flood_cursor_slot,
		target = aux->flood_target_slot;
	int32_t ph = s->atlas->ph, pw = s->atlas->pw;
	int8_t fill = arc_sprite_pixels(s, target)[pw + 1];
	int8_t *p = mut_pixels(s, cursor);
	int8_t matched[ARC_FRAME_SIZE * ARC_FRAME_SIZE];
	int32_t area = ph * pw;
	for (int32_t k = 0; k < area; k++)
		matched[k] = p[k] == fill;
	int8_t next[ARC_FRAME_SIZE * ARC_FRAME_SIZE];
...
static void flood_step(struct arc_sprites *s, struct re86_aux *aux,
		       struct re86_engine *engine, const struct re86_static *st)
{
	int32_t cursor = aux->flood_cursor_slot,
		target = aux->flood_target_slot;
	...
	int8_t fill = arc_sprite_pixels(s, target)[pw + 1];
	const int8_t *p = arc_sprite_pixels(s, cursor);
	int unresolved = 0;
	for (int32_t k = 0; k < area; k++)
		if (p[k] != -1 && p[k] != fill) {
			unresolved = 1;
		    ...
		    int32_t action_id)
{
	...
	case 0:
		directional(sprites, aux, engine, st, 0, -STEP_PX);
		break;
	case 1:
		directional(sprites, aux, engine, st, 0, STEP_PX);
		break;
	case 2:
		directional(sprites, aux, engine, st, -STEP_PX, 0);
		break;
	case 3:
		directional(sprites, aux, engine, st, STEP_PX, 0);
		break;
	default:
		cycle_step(sprites, aux, engine, st);
		break;
	}
}
			   ...
			   const struct re86_static *st, int32_t level_index)
{
	...
	if (2 * rest > budget || (2 * rest == budget && whole % 2 == 1))
		filled += 1;
