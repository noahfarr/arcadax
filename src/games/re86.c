...
static int8_t *mut_pixels(ArcSprites *s, int32_t i) {
...
static inline int8_t center_value(const ArcSprites *s, int32_t slot) {
    int32_t pw = s->atlas->pw;
    return arc_sprite_pixels(s, slot)[(s->h[slot] / 2) * pw + s->w[slot] / 2];
}

static inline void set_center(ArcSprites *s, int32_t slot, int8_t value) {
    int32_t pw = s->atlas->pw;
    mut_pixels(s, slot)[(s->h[slot] / 2) * pw + s->w[slot] / 2] = value;
}

static int8_t dynamic_own_color(const ArcSprites *s, int32_t slot) {
...
static void first_opaque(const ArcSprites *s, int32_t slot, int32_t *d_out, int32_t *u_out) {
    const ArcAtlas *a = s->atlas;
    const int8_t *p = arc_sprite_pixels(s, slot);
...
static void recompute_natural_center(ArcSprites *s, const Re86Static *st, int32_t level, int32_t slot) {
    int32_t d, u;
...
static inline void undo_move(ArcSprites *s, int32_t cursor, int32_t dx, int32_t dy) {
    s->x[cursor] -= dx;
    s->y[cursor] -= dy;
}

static void reshape_effect(ArcSprites *s, int32_t cursor, int32_t dx, int32_t dy) {
...
static void redraw_column(ArcSprites *s, int32_t cursor, int32_t d, int32_t delta, int32_t u, int8_t color) {
    const ArcAtlas *a = s->atlas;
...
static void redraw_row(ArcSprites *s, int32_t cursor, int32_t u, int32_t delta, int32_t d, int8_t color) {
    const ArcAtlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
    int8_t *p = mut_pixels(s, cursor);
    int32_t h = s->h[cursor], w = s->w[cursor];
...
static void shift_window_effect(ArcSprites *s, int32_t cursor, int32_t wall, int32_t dx, int32_t dy) {
    int8_t color = dynamic_own_color(s, cursor);
    int32_t d, u;
...
static int pixel_overlap_pair(const ArcSprites *s, int32_t i, int32_t j) {
    const ArcAtlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
...
static int re86_collides_pair(const ArcSprites *s, int32_t i, int32_t j) {
...
static int32_t target_check(ArcSprites *s, Re86Aux *aux, const Re86Static *st, int32_t level, int32_t cursor) {
...
static int32_t move_and_collide(ArcSprites *s, Re86Aux *aux, const Re86Static *st, int32_t level,
...
static int32_t find_active(const ArcSprites *s, const Re86Static *st, int32_t level) {
...
static int win_check(const ArcSprites *s, const Re86Static *st, int32_t level) {
    const ArcAtlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
    int32_t canvas_h = ARC_FRAME_SIZE + 2 * ph, canvas_w = ARC_FRAME_SIZE + 2 * pw;
    ...
    for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
        memcpy(canvas + (size_t)(ph + r) * canvas_w + pw,
              st->canvas_template + (size_t)r * ARC_FRAME_SIZE, ARC_FRAME_SIZE);
...
static void tail(ArcSprites *s, Re86Aux *aux, Re86Engine *engine, const Re86Static *st) {
...
static void directional(ArcSprites *s, Re86Aux *aux, Re86Engine *engine, const Re86Static *st,
...
static void cycle_step(ArcSprites *s, Re86Aux *aux, Re86Engine *engine, const Re86Static *st) {
    spend_step(aux);
    int32_t level = engine->level_index;
...
static void flood_spread(ArcSprites *s, const Re86Aux *aux) {
...
static void flood_step(ArcSprites *s, Re86Aux *aux, Re86Engine *engine, const Re86Static *st) {
    int32_t cursor = aux->flood_cursor_slot, target = aux->flood_target_slot;
...
void re86_step_once(ArcSprites *sprites, Re86Aux *aux, Re86Engine *engine,
...
void re86_render_interface(int8_t *frame, const Re86Aux *aux, const Re86Static *st, int32_t level_index) {
    ...
    for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) row[c] = c < filled ? 15 : 1;
}
