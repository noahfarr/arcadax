#include "arc/game.h"
#include "dc22.h"

static void adapt_zero_aux(void *aux) { (void)aux; }

static void adapt_on_set_level(ArcGame *game) {
    dc22_on_set_level((const Dc22Static *)game->statics, game->engine.level_index,
                      (Dc22Aux *)game->aux);
}

static void adapt_step_once(ArcGame *game) {
    ArcEngineState *e = &game->engine;
    dc22_step_once(&game->sprites, &game->camera, (const Dc22Static *)game->statics,
                   e->level_index, e->action_id, e->action_x, e->action_y,
                   (Dc22Aux *)game->aux, &e->score, &e->status, &e->next_level,
                   &e->action_complete);
}

static void adapt_render_interface(ArcGame *game, int8_t *frame) {
    dc22_render_interface(frame, &game->sprites, &game->camera,
                          (const Dc22Static *)game->statics, game->engine.level_index,
                          (const Dc22Aux *)game->aux);
}

const ArcHooks dc22_hooks = {adapt_zero_aux, adapt_on_set_level, adapt_step_once,
                          adapt_render_interface};
