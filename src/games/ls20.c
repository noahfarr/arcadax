...
static void ls20_set_slot_visible(struct arc_sprites *sprites, int32_t slot,
				  int visible)
{
	if (slot < 0)
		return;
...
static void ls20_set_tag_visible(struct arc_sprites *sprites, int32_t num_slots,
				 int32_t tag_idx, int visible)
{
	const struct arc_atlas *atlas = sprites->atlas;
	for (int32_t i = 0; i < num_slots; i++) {
		if (!sprites->alive[i])
			continue;
		if (!sprites->tags[(size_t)i * atlas->num_tags + tag_idx])
			continue;
		arc_set_visible(sprites, i, visible);
	}
}

static void ls20_style_loadout(struct arc_sprites *sprites,
			       ...
			       int32_t shape, int32_t color, int32_t rot)
{
	...
	const int8_t *variant = st->htk_variant +
				(((size_t)shape * 4 + color) * 4 + rot) * area;
	memcpy(arc_sprite_pixels_mut(sprites, htk), variant, area);
}

static void ls20_apply_hint(struct arc_sprites *sprites,
			    const struct ls20_static *st, int32_t level,
			    struct ls20_aux *aux)
{
	int any_hint = 0;
...
static int ls20_h_goal(struct arc_sprites *sprites,
		       const struct ls20_static *st, int32_t level,
		       struct ls20_aux *aux, int32_t slot)
{
	...
	int mismatch = !(aux->shape_idx == st->want_shape[idx] &&
			 ...
			 aux->rot_idx == st->want_rot[idx]);
	if (mismatch) {
		arc_color_remap(sprites, st->htk2_slot[level], 0, 0,
				LS20_COLOR_BLACK);
		aux->flash_frame = LS20_FLASH_FRAMES;
	}
	return mismatch;
...
static void ls20_h_refill(struct arc_sprites *sprites,
			  const struct ls20_static *st, int32_t level,
			  struct ls20_aux *aux, int32_t slot)
{
...
static void ls20_h_button(struct arc_sprites *sprites,
			  const struct ls20_static *st, int32_t level,
			  struct ls20_aux *aux, int which)
{
	int32_t shape = aux->shape_idx, color = aux->color_idx,
		rot = aux->rot_idx;
	if (which == 0)
		shape = (shape + 1) % 6;
	else if (which == 1)
	...
	aux->rot_idx = rot;
	if (level == 0)
		ls20_apply_hint(sprites, st, level, aux);
}

static void ls20_process_target(struct arc_sprites *sprites,
				const struct ls20_static *st, int32_t level,
				int32_t target_x, int32_t target_y,
				struct ls20_aux *aux, int *out_blocked,
				int *out_refilled)
{
	...
	for (int32_t i = 0; i < st->num_slots; i++) {
		if (!sprites->alive[i] || tag_code[i] <= 0)
			continue;
		int32_t sx = sprites->x[i], sy = sprites->y[i];
		if (sx < target_x || sx >= target_x + pitch_x ||
		    sy < target_y || sy >= target_y + pitch_y)
			continue;
...
static void ls20_step_patrols(struct arc_sprites *sprites,
			      const struct ls20_static *st, int32_t level,
			      struct ls20_aux *aux,
			      int32_t prev_x[LS20_MAX_PATROLS],
			      ...
			      int32_t prev_dir[LS20_MAX_PATROLS])
{
	...
	for (int32_t k = 0; k < LS20_MAX_PATROLS; k++) {
		...
		for (int oi = 0; oi < 4 && !found; oi++) {
			...
			int32_t cx = px + ddx * LS20_PATROL_CELL,
				cy = py + ddy * LS20_PATROL_CELL;
			if (cx < ax || cx >= ax + aw || cy < ay ||
			    cy >= ay + ah)
				continue;
			int32_t rel_x = ls20_clamp(cx - ax, 0, pw - 1);
			int32_t rel_y = ls20_clamp(cy - ay, 0, ph - 1);
			if (area_pixels[(size_t)rel_y * pw + rel_x] < 0)
				continue;
		...
		if (found)
			arc_set_position(sprites, pslot, px + best_dx,
					 py + best_dy);
		aux->patrol_dir[k] = best_dir;
	}
}

static void ls20_undo_patrols(struct arc_sprites *sprites,
			      const struct ls20_static *st, int32_t level,
			      struct ls20_aux *aux,
			      const int32_t prev_x[LS20_MAX_PATROLS],
			      ...
			      const int32_t prev_dir[LS20_MAX_PATROLS])
{
	for (int32_t k = 0; k < LS20_MAX_PATROLS; k++) {
		...
		if (pslot >= 0)
			arc_set_position(sprites, pslot, prev_x[k], prev_y[k]);
		aux->patrol_dir[k] = prev_dir[k];
	}
}

static void ls20_reset_patrols(struct arc_sprites *sprites,
			       const struct ls20_static *st, int32_t level,
			       struct ls20_aux *aux)
{
	for (int32_t k = 0; k < LS20_MAX_PATROLS; k++) {
		...
		int32_t pslot = st->patrol_slot[idx];
		if (pslot >= 0)
			arc_set_position(sprites, pslot,
					 st->patrol_start_x[idx],
					 st->patrol_start_y[idx]);
		aux->patrol_dir[k] = 0;
	}
}

static void ls20_maybe_start_push(struct arc_sprites *sprites,
				  const struct ls20_static *st, int32_t level,
				  struct ls20_aux *aux)
{
	...
	const int32_t *slots =
		st->pushable_slots + (size_t)level * st->max_pushable;
	int32_t best = -1, best_order = 0;
	for (int32_t k = 0; k < count; k++) {
		int32_t s = slots[k];
		if (!sprites->alive[s])
			continue;
		int32_t sx = sprites->x[s], sy = sprites->y[s],
			sw = sprites->w[s], sh = sprites->h[s];
		if (!(px < sx + sw && px + pw > sx && py < sy + sh &&
		      py + ph > sy))
			continue;
...
static int32_t ls20_restore_removables(struct arc_sprites *sprites,
				       const struct ls20_static *st,
				       int32_t level, int32_t next_order)
{
	int32_t count = st->restorable_count[level];
	const int32_t *slots =
		st->restorable_slots + (size_t)level * st->max_restorable;
	int32_t rank = 0;
	for (int32_t k = 0; k < count; k++) {
		int32_t slot = slots[k];
		if (sprites->alive[slot])
			continue;
...
static void ls20_start_death(struct arc_sprites *sprites,
			     const struct ls20_static *st, int32_t level,
			     ...
			     int32_t *status, uint8_t *action_complete)
{
	...
	int32_t player = st->player_slot[level];
	arc_set_position(sprites, player, st->player_start_x[level],
			 st->player_start_y[level]);
	*next_order = ls20_restore_removables(sprites, st, level, *next_order);
	...
	int32_t shape0 = st->start_shape[level],
		color0 = st->start_color[level], rot0 = st->start_rot[level];
	...
	aux->rot_idx = rot0;
	for (int32_t g = 0; g < LS20_MAX_GOALS; g++)
		aux->goal_done[g] = g >= st->num_goals[level];
	aux->death_frame = LS20_DEATH_FRAMES;
}

static void ls20_advance_death(struct arc_sprites *sprites,
			       const struct ls20_static *st, int32_t level,
			       struct ls20_aux *aux, uint8_t *action_complete)
{
...
static void ls20_advance_flash(struct arc_sprites *sprites,
			       const struct ls20_static *st, int32_t level,
			       struct ls20_aux *aux, uint8_t *action_complete)
{
...
static int ls20_check_goals(struct arc_sprites *sprites,
			    const struct ls20_static *st, int32_t level,
			    struct ls20_aux *aux)
{
	int32_t player = st->player_slot[level];
	...
	for (int32_t g = 0; g < LS20_MAX_GOALS; g++) {
		...
		int matches = aux->shape_idx == st->want_shape[idx] &&
			      aux->color_idx == st->want_color[idx] &&
			      aux->rot_idx == st->want_rot[idx];
		int newly = matches && px == st->goal_x[idx] &&
			    py == st->goal_y[idx];
		if (!newly)
			continue;
		...
		if (st->goal_is_final[idx]) {
			...
			ls20_set_slot_visible(sprites,
					      st->hint_glow_slot[level], 0);
		}
		aux->goal_done[g] = 1;
	}
	for (int32_t g = 0; g < LS20_MAX_GOALS; g++)
...
static void ls20_next_level(const struct ls20_static *st, int32_t level,
...
static void ls20_advance_push(struct arc_sprites *sprites,
			      const struct ls20_static *st, int32_t level,
			      struct ls20_aux *aux, uint8_t *action_complete)
{
	int32_t slot = aux->push_slot, k = aux->push_frame;
	size_t idx = ((size_t)level * st->num_slots + slot) * LS20_PUSH_FRAMES +
		     (k - 1);
	int32_t dx = st->wall_step_dx[idx], dy = st->wall_step_dy[idx];
	arc_move_sprite(sprites, slot, dx, dy);
	int32_t player = st->player_slot[level];
	if (k <= LS20_PUSH_PUSH_PHASE)
		arc_move_sprite(sprites, player, dx, dy);
	...
	if (k == LS20_PUSH_FRAMES) {
		...
		ls20_process_target(sprites, st, level, px, py, aux, &blocked,
				    &refilled);
		*action_complete = 1;
	} else {
		aux->push_frame = k + 1;
...
static void ls20_play_move(struct arc_sprites *sprites,
			   const struct ls20_static *st, int32_t level,
			   int32_t action_id, struct ls20_aux *aux,
			   ...
			   uint8_t *next_level, uint8_t *action_complete)
{
	...
	int32_t prev_x[LS20_MAX_PATROLS], prev_y[LS20_MAX_PATROLS],
		prev_dir[LS20_MAX_PATROLS];
	...
	int blocked, refilled;
	ls20_process_target(sprites, st, level, target_x, target_y, aux,
			    &blocked, &refilled);

	if (blocked)
		ls20_undo_patrols(sprites, st, level, aux, prev_x, prev_y,
				  prev_dir);
	else
		arc_set_position(sprites, player, target_x, target_y);

	if (aux->flash_frame > 0)
		return;
	...
	if (!out_of_steps)
		ls20_maybe_start_push(sprites, st, level, aux);
	if (aux->push_frame > 0)
		return;
	...
	if (all_done) {
		...
		*action_complete = 1;
		...
		ls20_start_death(sprites, st, level, aux, next_order, status,
				 action_complete);
	} else {
		*action_complete = 1;
	}
...
static void ls20_play(struct arc_sprites *sprites, const struct ls20_static *st,
		      int32_t level, int32_t action_id, struct ls20_aux *aux,
		      int32_t *next_order, int32_t *score, int32_t *status,
		      uint8_t *next_level, uint8_t *action_complete)
{
	if (action_id >= 1 && action_id <= 4) {
		ls20_play_move(sprites, st, level, action_id, aux, next_order,
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	if (aux->push_frame > 0) {
		...
		ls20_play(sprites, st, level, action_id, aux, next_order, score,
			   ...
			   const struct ls20_aux *aux)
{
	...
	for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) {
		...
		int8_t colour =
			filled ? LS20_COLOR_STEP_FILLED : LS20_COLOR_STEP_EMPTY;
	...
	for (int32_t b = 0; b < 3; b++) {
		int32_t x0 = 56 + 3 * b;
		int8_t colour = aux->lives > b ? LS20_COLOR_LIVES_FILLED :
						 LS20_COLOR_STEP_EMPTY;
