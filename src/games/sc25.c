...
enum {
	...
	SC25_BAR_EMPTY = 14,
	SC25_SHAKE_COLOR = 8,
	SC25_REFUND_STEPS = 10,
	SC25_FLASH_COLOUR = 11
};
enum {
	SC25_CAST_FRAMES = 8,
	SC25_TELEPORT_FRAMES = 4,
	SC25_MISS_FRAMES = 12,
	SC25_SLIDE_FRAMES = 2,
	SC25_RESIZE_FRAMES = 8,
	SC25_RESIZE_BLOCKED_FRAMES = 12,
...
static const int32_t SC25_FACING_DELTA[4][2] = {
	{ 0, -1 }, { 0, 1 }, { -1, 0 }, { 1, 0 }
};
static const int32_t SC25_FIREBALL_ROTATION[4] = { 90, 270, 0, 180 };

static inline int32_t sc25_min(int32_t a, int32_t b)
{
	return a < b ? a : b;
}
static inline int32_t sc25_max(int32_t a, int32_t b)
{
	return a > b ? a : b;
}
...
static void sc25_recolor_all(struct arc_sprites *s, int32_t slot, int8_t colour)
{
...
static void sc25_recolor_all_if(struct arc_sprites *s, int32_t slot,
				int8_t colour)
{
	if (colour >= 0)
		sc25_recolor_all(s, slot, colour);
}

static void sc25_recolor_bounded(struct arc_sprites *s, int32_t slot,
				 int8_t colour, const int32_t *box)
{
...
static void sc25_compute_grid_bbox(struct sc25_aux *aux,
				   const struct arc_sprites *s,
...
static void sc25_recolor_grid_cells(struct arc_sprites *s,
				    const struct sc25_static *st, int32_t level,
				    int8_t colour, const struct sc25_aux *aux)
{
	const int32_t *grid = st->grid_slot + (size_t)level * 9;
	for (int32_t i = 0; i < 9; i++) {
		int32_t slot = grid[i];
		if (slot >= 0)
			sc25_recolor_bounded(s, slot, colour,
					     aux->grid_bbox[i]);
	}
}

static void sc25_write_grid(struct arc_sprites *s, const struct sc25_static *st,
			    int32_t level, const int8_t colours[3][3],
			    const struct sc25_aux *aux)
{
	...
	for (int32_t r = 0; r < 3; r++)
		for (int32_t c = 0; c < 3; c++) {
			...
			if (slot >= 0)
				sc25_recolor_bounded(s, slot, colours[r][c],
						     aux->grid_bbox[r * 3 + c]);
		}
}

static int32_t sc25_matched_spell(const struct sc25_aux *aux,
				  const struct sc25_static *st, int32_t level)
{
	for (int32_t sp = 0; sp < SC25_NUM_SPELLS; sp++) {
		...
		for (int32_t r = 0; r < 3 && match; r++)
			for (int32_t c = 0; c < 3; c++)
				if ((int)aux->drawn[r][c] !=
				    (int)SC25_PATTERN[sp][r][c]) {
					match = 0;
					break;
				}
		if (match)
			return st->allowed[level * SC25_NUM_SPELLS + sp] ? sp :
									   -1;
	}
	return -1;
}

static void sc25_cell_colour(const struct sc25_aux *aux, int32_t matched,
			     int8_t out[3][3])
{
	int32_t hint = aux->hint;
	int8_t drawn_colour = matched >= 0 ? SC25_SPELL_COLOR[matched] :
					     (int8_t)SC25_MISS_COLOR;
	for (int32_t r = 0; r < 3; r++) {
		for (int32_t c = 0; c < 3; c++) {
			if (aux->drawn[r][c]) {
				out[r][c] = drawn_colour;
				continue;
			}
...
static void sc25_recolour_grid(struct arc_sprites *s,
			       const struct sc25_static *st, int32_t level,
			       struct sc25_aux *aux)
{
...
static void sc25_fill_bar(struct arc_sprites *s, const struct sc25_static *st,
			  int32_t level, const struct sc25_aux *aux)
{
	...
	int32_t filled_base =
		budget > 0 ? (aux->steps * base_total) / budget : 0;
	...
	for (int32_t r = 0; r < ph; r++) {
		if (r >= st->bar_ph || !mask[r])
			continue;
		int8_t colour =
			(int8_t)(r < filled ? SC25_BAR_FILLED : SC25_BAR_EMPTY);
		for (int32_t c = 0; c < pw; c++)
			p[r * pw + c] = colour;
	}
}

static void sc25_apply_overlay_visibility(struct arc_sprites *s,
					  const struct sc25_static *st,
					  int32_t level,
					  const struct sc25_aux *aux)
{
	int32_t a_slot = st->acyylh_ovl[level], s_slot = st->smzaik_ovl[level];
	if (a_slot >= 0)
		arc_set_visible(s, a_slot, aux->scale2);
	if (s_slot >= 0)
		arc_set_visible(s, s_slot, !aux->scale2);
}

static void sc25_reposition_overlay(struct arc_sprites *s,
				    const struct sc25_static *st, int32_t level,
				    const struct sc25_aux *aux)
{
	...
	int32_t target_slot =
		st->tp_slots[(size_t)(level * 2 + 1) * st->max_tp + idx];
	int32_t tx = s->x[target_slot] - 1, ty = s->y[target_slot] - 1;
	arc_set_position(s, a_slot, tx, ty);
}

static void sc25_spend_step(struct arc_sprites *s, const struct sc25_static *st,
			    int32_t level, struct sc25_aux *aux,
			    int32_t *status, uint8_t *action_complete)
{
	...
	*action_complete = 1;
	...
	int32_t budget = st->budget[level];
	if (budget > 0 && aux->steps > budget)
		*status = SC25_GAME_OVER;
}

static void sc25_reset_drawn(struct arc_sprites *s,
			     const struct sc25_static *st, int32_t level,
			     struct sc25_aux *aux)
{
...
static void sc25_finish_effect(struct arc_sprites *s,
			       const struct sc25_static *st, int32_t level,
			       struct sc25_aux *aux, int32_t *status,
...
static void sc25_player_render(const struct sc25_aux *aux,
			       const struct sc25_static *st, int32_t level,
			       int32_t ph, int32_t pw, int8_t *canvas,
			       int32_t *out_h, int32_t *out_w)
{
	int32_t rot = aux->facing_set ? SC25_FACING_ROTATION[aux->facing] :
					st->player_rotation0[level];
...
static void sc25_apply_player_sprite(struct arc_sprites *s,
				     const struct sc25_static *st,
				     int32_t level, struct sc25_aux *aux)
{
		       ...
		       struct sc25_aux *aux)
{
	...
	sc25_recolor_grid_cells(sprites, st, level, (int8_t)SC25_BASE_COLOR,
				aux);
...
static void sc25_next_level(const struct sc25_static *st, int32_t level,
...
static void sc25_step_demo0(struct sc25_aux *aux)
{
	int32_t hint = aux->hint;
	aux->demo0_pending = 0;
...
static void sc25_step_slide(struct arc_sprites *s, const struct sc25_static *st,
			    int32_t level, struct sc25_aux *aux, int32_t *score,
...
static void sc25_step_miss(struct arc_sprites *s, const struct sc25_static *st,
			   int32_t level, struct sc25_aux *aux,
			   uint8_t *action_complete)
{
	...
	if (cyc < 3) {
		for (int32_t r = 0; r < 3; r++)
			for (int32_t c = 0; c < 3; c++)
				colours[r][c] = (int8_t)SC25_SHAKE_COLOR;
	} else {
		int32_t matched = sc25_matched_spell(aux, st, level);
		sc25_cell_colour(aux, matched, colours);
	}
...
static void sc25_step_demo(struct arc_sprites *s, const struct sc25_static *st,
			   int32_t level, struct sc25_aux *aux,
			   uint8_t *action_complete)
{
	...
	for (int32_t r = 0; r < 3; r++) {
		for (int32_t c = 0; c < 3; c++) {
			...
			colours[r][c] = (t && count == active_idx) ?
						(int8_t)14 :
						(int8_t)-1;
			if (t)
				count += 1;
	...
	for (int32_t r = 0; r < 3; r++)
		for (int32_t c = 0; c < 3; c++) {
			int32_t slot = grid[r * 3 + c];
			if (slot >= 0)
				sc25_recolor_all_if(s, slot, colours[r][c]);
		}
	frame += 1;
...
static void sc25_start_teleport(struct arc_sprites *s,
				const struct sc25_static *st, int32_t level,
				struct sc25_aux *aux, int32_t *status,
				uint8_t *action_complete)
{
	...
	int32_t target_slot =
		st->tp_slots[(size_t)(level * 2 + list_idx) * st->max_tp +
			     idx_valid];
	int has_target = count > 0;
	int32_t next_idx = has_target ? (idx + 1) % count : idx;
	if (has_target)
		aux->tp_idx[list_idx] = next_idx;
	...
	memcpy(aux->tp_player_saved, aux->player_base,
	       sizeof aux->tp_player_saved);
...
static void sc25_step_teleport(struct arc_sprites *s,
			       const struct sc25_static *st, int32_t level,
			       struct sc25_aux *aux, int32_t *status,
			       uint8_t *action_complete)
{
	...
	if (frame_next < SC25_TELEPORT_FRAMES) {
		...
		int32_t area = s->atlas->ph * s->atlas->pw;
		const int8_t *clean =
			s->atlas->pixels + (size_t)target_slot * area;
		memcpy(arc_sprite_pixels_mut(s, target_slot), clean,
		       (size_t)area);
		memcpy(aux->player_base, aux->tp_player_saved,
		       sizeof aux->player_base);
...
static void sc25_blocked_offset(const struct sc25_static *st, int32_t level,
				int32_t px, int32_t py, int32_t *out_dx,
				int32_t *out_dy, int *out_ok)
{
	static const int32_t OFF[9][2] = {
		{ -2, -2 }, { -2, -1 }, { -2, 0 }, { -1, -2 }, { -1, -1 },
		{ -1, 0 },  { 0, -2 },	{ 0, -1 }, { 0, 0 },
	};
	const uint8_t *block = st->grow_block +
			       (size_t)level * ARC_FRAME_SIZE * ARC_FRAME_SIZE;
	for (int32_t k = 0; k < 9; k++) {
		int32_t ox = OFF[k][0], oy = OFF[k][1];
		int32_t nx = px + ox, ny = py + oy;
		if (nx < 0 || ny < 0 || nx + 4 > ARC_FRAME_SIZE ||
		    ny + 4 > ARC_FRAME_SIZE)
			continue;
		int hit = 0;
		for (int32_t dy = 0; dy < 4 && !hit; dy++)
			for (int32_t dx = 0; dx < 4; dx++)
				if (block[(size_t)(ny + dy) * ARC_FRAME_SIZE +
					  (nx + dx)]) {
					hit = 1;
					break;
				}
...
static void sc25_start_resize(struct arc_sprites *s,
			      const struct sc25_static *st, int32_t level,
			      struct sc25_aux *aux)
{
...
static void sc25_collect_blockers(struct arc_sprites *s,
				  const struct sc25_static *st, int32_t level,
				  struct sc25_aux *aux)
{
	int32_t slot = st->pluyoo[level];
	...
	const int32_t *blockers =
		st->blockers + (size_t)level * st->max_blockers;
	for (int32_t k = 0; k < count; k++) {
		int32_t bslot = blockers[k];
		if (!s->alive[bslot])
			continue;
		int32_t bx = s->x[bslot], by = s->y[bslot], bw = s->w[bslot],
			bh = s->h[bslot];
		int overlap = (px < bx + bw) && (px + pw > bx) &&
			      (py < by + bh) && (py + ph > by);
		if (overlap) {
			arc_remove_sprite(s, bslot);
			aux->steps =
				sc25_max(aux->steps - SC25_REFUND_STEPS, 0);
			sc25_fill_bar(s, st, level, aux);
		}
	}
}

static void sc25_step_resize(struct arc_sprites *s,
			     const struct sc25_static *st, int32_t level,
			     struct sc25_aux *aux, int32_t *status,
			     uint8_t *action_complete)
{
		...
		if (cleanup) {
			for (int32_t i = 0; i < 2; i++)
				for (int32_t j = 0; j < 2; j++) {
					...
					if (v == 9 || v == 10)
						aux->player_base[i][j] = 4;
				}
		} else {
			aux->player_base[0][0] = 10;
			aux->player_base[0][1] = 10;
			aux->player_base[1][0] = 9;
			aux->player_base[1][1] = 9;
		}
	} else {
		int cur_is_target = (tg % 2) == 1;
		aux->scale2 = (uint8_t)(cur_is_target ? aux->resize_target2 :
							!aux->resize_target2);
	}
	...
	int32_t max_frames =
		blocked ? SC25_RESIZE_BLOCKED_FRAMES : SC25_RESIZE_FRAMES;
...
static void sc25_rotate_pad(int8_t *out, int32_t ph, int32_t pw,
			    const int8_t *base, int32_t base_h, int32_t base_w,
			    int32_t consumed, int32_t k, int32_t true_h,
			    int32_t true_w)
{
	for (int32_t row = 0; row < ph; row++) {
		for (int32_t col = 0; col < pw; col++) {
			...
			switch (k & 3) {
			case 0:
				valid = row < true_h && col < true_w;
				sr = row;
				sc = col;
				break;
			case 1:
				valid = row < true_w && col < true_h;
				sr = col;
				sc = true_w - 1 - row;
				break;
			case 2:
			...
			out[row * pw + col] =
				valid ? base[br * base_w + bc] : (int8_t)-1;
		}
	}
}

static void sc25_fb_canvas(int8_t *out, int32_t ph, int32_t pw,
			   const struct sc25_static *st,
			   const struct sc25_aux *aux, int8_t colour,
			   int mirror_lr, int mirror_ud)
{
	...
	const int8_t *base = st->fb_base + (size_t)aux->fb_scale_idx *
						   st->fb_base_h *
						   st->fb_base_w;

	int8_t canvas[SC25_PATCH_CAP];
	sc25_rotate_pad(canvas, ph, pw, base, base_h, base_w, aux->fb_consumed,
			aux->fb_rot_k, base_h, remaining_w);
	...
	int32_t area = ph * pw;
	for (int32_t i = 0; i < area; i++)
		out[i] = out[i] >= 0 ? colour : out[i];
}

static void sc25_start_fireball(struct arc_sprites *s,
				const struct sc25_static *st, int32_t level,
				struct sc25_aux *aux)
{
	int32_t facing = aux->facing;
	int32_t dx = SC25_FACING_DELTA[facing][0],
		dy = SC25_FACING_DELTA[facing][1];
	int32_t slot = st->pluyoo[level];
	int32_t px = s->x[slot], py = s->y[slot];
	int32_t scale = aux->scale2 ? 2 : 1;
	...
	const int32_t *hit_grid = st->fb_hit_slot + (size_t)level *
							    ARC_FRAME_SIZE *
							    ARC_FRAME_SIZE;
	...
	for (int32_t ii = 1; ii < 64; ii++) {
		...
		if (xs < 0 || xs >= ARC_FRAME_SIZE || ys < 0 ||
		    ys >= ARC_FRAME_SIZE) {
			max_distance = ii - 1;
			break;
		}
	...
	int32_t hit_kind =
		hit_slot >= 0 ?
			st->fb_slot_kind[level * st->num_slots + hit_slot] :
			0;
	...
	const int8_t *base = st->fb_base +
			     (size_t)scale1_idx * st->fb_base_h * st->fb_base_w;
	int8_t canvas[SC25_PATCH_CAP];
	sc25_rotate_pad(canvas, ph, pw, base, base_true_h, base_true_w, 0,
			rot_k, base_true_h, base_true_w);
...
static void sc25_step_fireball(struct arc_sprites *s,
			       const struct sc25_static *st, int32_t level,
			       struct sc25_aux *aux, int32_t *status,
			       uint8_t *action_complete)
{
	...
	if (!aux->fb_shrinking) {
		...
		int need_reposition =
			(w_new <= 3) && (aux->fb_dx > 0 || aux->fb_dy > 0);
		if (need_reposition) {
...
static void sc25_step_cast(struct arc_sprites *s, const struct sc25_static *st,
			   int32_t level, struct sc25_aux *aux, int32_t *status,
			   uint8_t *action_complete)
{
	...
	if (frame >= SC25_CAST_FRAMES) {
		...
		aux->hint = -1;
		if (sp == SC25_TELEPORT)
			sc25_start_teleport(s, st, level, aux, status,
					    action_complete);
		else if (sp == SC25_RESIZE)
			sc25_start_resize(s, st, level, aux);
		else if (sp == SC25_FIREBALL)
			sc25_start_fireball(s, st, level, aux);
	}
}

static void sc25_display_to_grid(const struct arc_camera *camera,
...
static void sc25_grid_cell(struct arc_sprites *s, const struct sc25_static *st,
			   int32_t level, int32_t hit, struct sc25_aux *aux,
			   int32_t *status, uint8_t *action_complete)
{
...
static void sc25_spell_select(struct arc_sprites *s,
			      const struct sc25_static *st, int32_t level,
			      int32_t hit, struct sc25_aux *aux,
			      uint8_t *action_complete)
{
...
static void sc25_step_click(struct arc_sprites *s,
			    ...
			    uint8_t *action_complete)
{
	...
	int32_t kind =
		hit_valid ? st->click_kind[level * st->num_slots + hit] : 3;

	if (kind == 0 || kind == 1)
		sc25_grid_cell(s, st, level, hit, aux, status, action_complete);
	else if (kind == 2)
		sc25_spell_select(s, st, level, hit, aux, action_complete);
	else {
		aux->miss_active = 1;
		aux->miss_frame = 0;
	}
...
static void sc25_step_move(struct arc_sprites *s, const struct sc25_static *st,
			   ...
			   uint8_t *action_complete)
{
	...
	int32_t mdx = SC25_FACING_DELTA[facing][0] * step,
		mdy = SC25_FACING_DELTA[facing][1] * step;
	...
	int32_t sdx = 0, sdy = 0;
	if (moved && exy >= 0) {
		int32_t ex = s->x[exy], ey = s->y[exy], ew = s->w[exy],
			eh = s->h[exy];
		overlap_exit = (slot_x < ex + ew) && (slot_x + pw_ > ex) &&
			       (slot_y < ey + eh) && (slot_y + ph_ > ey);
		...
		if (use_x)
			sdx = ddx > 0 ? step / 2 : -(step / 2);
		else
			sdy = ddy > 0 ? step / 2 : -(step / 2);
	}
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (aux->demo0_pending && not_reset) {
		...
		sc25_step_slide(sprites, st, level, aux, score, status,
		...
		sc25_step_cast(sprites, st, level, aux, status,
			       action_complete);
	} else if (aux->tp_active) {
		sc25_step_teleport(sprites, st, level, aux, status,
				   action_complete);
	} else if (aux->resize_active) {
		sc25_step_resize(sprites, st, level, aux, status,
				 action_complete);
	} else if (aux->fb_active) {
		sc25_step_fireball(sprites, st, level, aux, status,
				   action_complete);
	} else if (action_id == SC25_ACTION6) {
		sc25_step_click(sprites, camera, st, level, action_x, action_y,
				aux, status, action_complete);
	} else if (action_id >= SC25_ACTION1 && action_id <= SC25_ACTION4) {
		sc25_step_move(sprites, st, level, action_id, aux, status,
			       action_complete);
	} else {
		*action_complete = 1;
	}
