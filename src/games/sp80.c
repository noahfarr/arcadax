...
static inline int sp80_contains(const ArcSprites *s, int32_t i, int32_t x, int32_t y) {
    return x >= s->x[i] && y >= s->y[i] && x < s->x[i] + s->w[i] && y < s->y[i] + s->h[i];
}

static inline int sp80_tag(const ArcSprites *s, int32_t i, int32_t tag) {
    return s->tags[(size_t)i * s->atlas->num_tags + tag];
}

static void sp80_display_to_grid(const ArcCamera *camera, int32_t display_x,
...
static void sp80_deselect(ArcSprites *s, const Sp80Static *st, Sp80Aux *aux) {
...
static void sp80_select(ArcSprites *s, const Sp80Static *st, Sp80Aux *aux, int32_t slot) {
...
static void sp80_auto_select(ArcSprites *s, const Sp80Static *st, int32_t level, Sp80Aux *aux) {
...
static int32_t sp80_pick(const ArcSprites *s, const Sp80Static *st, int32_t level,
...
static int sp80_can_move(const ArcSprites *s, const Sp80Static *st, int32_t level,
...
static void sp80_apply_move(ArcSprites *s, const Sp80Static *st, int32_t level,
    ...
    for (int32_t j = 0; j < st->extra_start; j++) {
        if (!arc_collides_pair(s, slot, j, 0)) continue;
        blocked = 1;
...
static int32_t sp80_spawn(ArcSprites *s, const Sp80Static *st, int32_t *next_order,
                          int32_t x, int32_t y) {
...
static void sp80_spread(ArcSprites *s, const Sp80Static *st, int32_t *next_order,
    ...
    if (arc_get_sprite_at(s, tx, ty, -1, 0) >= 0) return;
    int32_t new_slot = sp80_spawn(s, st, next_order, tx, ty);
    sp80_append(fs, fdx, fdy, fa, write_ptr, new_slot, dx, dy);
}

static void sp80_enter_spill(ArcSprites *s, const Sp80Static *st, int32_t level,
...
static void sp80_step_flow(ArcSprites *s, const Sp80Static *st, Sp80Aux *aux,
                           int32_t *next_order) {
...
static void sp80_flash_frame(ArcSprites *s, const Sp80Static *st, int32_t level, Sp80Aux *aux) {
    int32_t flash = aux->flash;
    int8_t goal_colour = (int8_t)((flash % 2 == 1) ? 14 : 1);
    for (int32_t i = 0; i < st->extra_start; i++)
        if (aux->touched[i]) arc_color_remap(s, i, 0, 0, goal_colour);
    ...
    if (flash < 5) {
        ...
        for (int32_t k = 0; k < count; k++) {
            int32_t slot = repw[k];
            if (s->alive[slot] && !aux->filled[slot]) arc_color_remap(s, slot, 0, 0, obstacle_colour);
        }
    }
    aux->flash = flash + 1;
}

static void sp80_finish_failed_spill(ArcSprites *s, const Sp80Static *st, int32_t level,
                                     Sp80Aux *aux, int32_t *status, uint8_t *action_complete) {
    for (int32_t i = st->extra_start; i < st->num_slots; i++) s->alive[i] = 0;

    const int32_t *repw = st->repw_slots + (size_t)level * st->max_repw;
    int32_t repw_count = st->repw_count[level];
    for (int32_t k = 0; k < repw_count; k++) arc_color_remap(s, repw[k], 0, 0, 11);

    const int32_t *waoe = st->waoe_slots + (size_t)level * st->max_waoe;
    int32_t waoe_count = st->waoe_count[level];
    for (int32_t k = 0; k < waoe_count; k++) arc_color_remap(s, waoe[k], 0, 0, 1);
...
static void sp80_step_drain_fail(ArcSprites *s, const Sp80Static *st, int32_t level,
                                 Sp80Aux *aux, int32_t *status, uint8_t *action_complete) {
...
static void sp80_step_drain(ArcSprites *s, const Sp80Static *st, int32_t level, Sp80Aux *aux,
...
static void sp80_step_spill(ArcSprites *s, const Sp80Static *st, int32_t level, Sp80Aux *aux,
...
static void sp80_step_change(ArcSprites *s, const ArcCamera *camera, const Sp80Static *st,
...
void sp80_on_set_level(ArcSprites *sprites, const Sp80Static *st, int32_t level, Sp80Aux *aux) {
    ...
    const int32_t *repw = st->repw_slots + (size_t)level * st->max_repw;
    int32_t repw_count = st->repw_count[level];
    for (int32_t k = 0; k < repw_count; k++) arc_color_remap(sprites, repw[k], 0, 0, 11);

    const int32_t *waoe = st->waoe_slots + (size_t)level * st->max_waoe;
    int32_t waoe_count = st->waoe_count[level];
    for (int32_t k = 0; k < waoe_count; k++) arc_color_remap(sprites, waoe[k], 0, 0, 1);
...
void sp80_render_interface(int8_t *frame, const ArcSprites *sprites, const ArcCamera *camera,
    ...
    if (budget != 0) {
        ...
        for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) frame[c] = (int8_t)(c < filled ? 14 : 0);
    }
    ...
    if (k == 1) {
        for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
            for (int32_t c = 0; c < ARC_FRAME_SIZE; c++)
                tmp[r * ARC_FRAME_SIZE + c] = frame[c * ARC_FRAME_SIZE + (ARC_FRAME_SIZE - 1 - r)];
    } else if (k == 2) {
        for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
            for (int32_t c = 0; c < ARC_FRAME_SIZE; c++)
                tmp[r * ARC_FRAME_SIZE + c] = frame[(ARC_FRAME_SIZE - 1 - r) * ARC_FRAME_SIZE + (ARC_FRAME_SIZE - 1 - c)];
    } else {
        for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
            for (int32_t c = 0; c < ARC_FRAME_SIZE; c++)
                tmp[r * ARC_FRAME_SIZE + c] = frame[(ARC_FRAME_SIZE - 1 - c) * ARC_FRAME_SIZE + r];
    }
