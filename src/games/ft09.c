...
enum {
	FT09_HINT_FRAMES = 4,
	FT09_HINT_COLOUR_ODD = 0,
	FT09_HINT_COLOUR_EVEN = 2
};
...
static void ft09_cell_at(const struct arc_sprites *s,
			 const struct ft09_static *st, int32_t x, int32_t y,
...
static void ft09_advance_colour(struct arc_sprites *s,
				...
				int32_t slot)
{
	...
	const int32_t *palette =
		st->palette + (size_t)level * st->palette_width;
...
static void ft09_stamp(struct arc_sprites *s, const struct ft09_static *st,
		       int32_t level, int32_t slot, int is_pattern)
{
	int32_t pw = s->atlas->pw;
	...
	for (int32_t row = 0; row < 3; row++)
		for (int32_t col = 0; col < 3; col++)
			own[row][col] =
				patch[row * pw + col] == FT09_STAMP_MARKER ? 1 :
									     0;
	own[1][1] = 1;
	...
	for (int32_t k = 0; k < 9; k++) {
		...
		if (target >= 0)
			ft09_advance_colour(s, st, level, target);
	}
}

static int ft09_solved(const struct arc_sprites *s,
		       const struct ft09_static *st, int32_t level)
{
...
static void ft09_clear_level(const struct ft09_static *st, int32_t level,
...
static void ft09_spend_step(struct ft09_aux *aux, int32_t *status,
			    uint8_t *action_complete)
{
	...
	aux->steps = steps;
	if (steps <= 0)
		*status = FT09_GAME_OVER;
	*action_complete = 1;
}

static void ft09_animate_hint(struct arc_sprites *s,
			      const struct ft09_static *st, int32_t level,
			      struct ft09_aux *aux, uint8_t *action_complete)
{
	int32_t hint = aux->hint - 1;
	int32_t slot = st->hint_slot[level];
	int8_t colour =
		hint % 2 == 1 ? FT09_HINT_COLOUR_ODD : FT09_HINT_COLOUR_EVEN;
...
static void ft09_miss(const struct arc_sprites *s, const struct ft09_static *st,
		      int32_t level, int32_t world_x, int32_t world_y,
		      struct ft09_aux *aux, uint8_t *action_complete)
{
...
static void ft09_resolve(struct arc_sprites *s, const struct ft09_static *st,
			 int32_t level, int32_t slot, int is_pattern,
			 int hit_cell, struct ft09_aux *aux, int32_t *score,
			 int32_t *status, uint8_t *next_level,
			 uint8_t *action_complete)
{
	if (hit_cell)
		ft09_stamp(s, st, level, slot, is_pattern);
	if (ft09_solved(s, st, level)) {
		ft09_clear_level(st, level, score, status, next_level,
				 action_complete);
	} else {
...
static void ft09_play(struct arc_sprites *s, const struct arc_camera *camera,
		      const struct ft09_static *st, int32_t level,
		      ...
		      uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (!hit_cell && on_board) {
		...
		ft09_resolve(s, st, level, slot, is_pattern, hit_cell, aux,
...
void ft09_on_set_level(struct arc_sprites *sprites,
		       ...
		       struct ft09_aux *aux)
{
	const struct arc_atlas *a = sprites->atlas;
	int8_t base = (int8_t)st->palette[(size_t)level * st->palette_width];
	for (int32_t i = 0; i < a->num_slots; i++) {
		int is_plain =
			sprites->tags[(size_t)i * a->num_tags + st->plain_tag];
		int is_pattern = sprites->tags[(size_t)i * a->num_tags +
					       st->pattern_tag];
		if (!is_plain && !is_pattern)
			continue;
		if (is_plain) {
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	...
	ft09_play(sprites, camera, st, level, action_id, action_x, action_y,
		  aux, score, status, next_level, action_complete);
}
