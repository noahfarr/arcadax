...
static int32_t lp85_slot_index(const struct lp85_static *st, int32_t level,
			       int32_t slot)
{
	return level * st->num_slots + slot;
}

static void lp85_find_ring_sources(const struct lp85_static *st,
				   ...
				   int32_t *sources)
{
	...
	const int32_t *candidates =
		st->ring_candidates + (size_t)level * st->max_candidates;

	for (int32_t k = 0; k < rl; k++) {
		int32_t found = -1;
		for (int32_t c = 0; c < cand_count; c++) {
			int32_t i = candidates[c];
			if (sprites->alive[i] && sprites->x[i] == from_x[k] &&
			    sprites->y[i] == from_y[k]) {
				found = i;
				break;
			}
		}
...
static void lp85_rotate_ring(const struct lp85_static *st,
			     struct arc_sprites *sprites,
			     struct lp85_scratch *scratch, int32_t level,
			     int32_t slot)
{
	int32_t idx = lp85_slot_index(st, level, slot);
	int32_t rl = st->ring_len[idx];
...
static int lp85_piece_group_satisfied(const struct arc_sprites *sprites,
				      const int32_t *slots, int32_t count,
				      int32_t goal_tag)
{
	for (int32_t k = 0; k < count; k++) {
		...
		if (!sprites->alive[i])
			continue;
		if (arc_get_sprite_at(sprites, sprites->x[i] + 1,
				      sprites->y[i] + 1, goal_tag, 0) < 0)
			return 0;
	}
	return 1;
}

static int lp85_solved(const struct lp85_static *st,
		       const struct arc_sprites *sprites, int32_t level)
{
	const int32_t *piece_a =
		st->piece_a_slots + (size_t)level * st->max_pieces;
	const int32_t *piece_b =
		st->piece_b_slots + (size_t)level * st->max_pieces;
	if (!lp85_piece_group_satisfied(sprites, piece_a,
					st->num_piece_a[level], st->goal_a))
		return 0;
	return lp85_piece_group_satisfied(sprites, piece_b,
					  st->num_piece_b[level], st->goal_b);
}
		    ...
		    struct lp85_aux *aux)
{
	...
	arc_scale_and_offset(&engine->camera, &scale, &x_pad, &y_pad);
	...
	int on_board = local_x >= 0 && local_y >= 0 &&
		       local_x < engine->camera.width &&
		       local_y < engine->camera.height;
	...
	if (is_click && on_board) {
		...
		const int32_t *buttons =
			st->button_slots + (size_t)level * st->max_buttons;
		for (int32_t k = 0; k < nb; k++) {
			int32_t i = buttons[k];
			if (!sprites->alive[i])
				continue;
			if (world_x >= sprites->x[i] &&
			    world_y >= sprites->y[i] &&
			   ...
			   const struct lp85_aux *aux, int8_t *frame)
{
	...
	int32_t total = ARC_FRAME_SIZE * used;
	int32_t whole = total / budget;
	int32_t rest = total % budget;
	int round_up =
		(2 * rest > budget) || (2 * rest == budget && (whole % 2 != 0));
	...
	for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
		frame[(size_t)r * ARC_FRAME_SIZE] =
			r < filled ? LP85_FILLED_COLOUR : LP85_EMPTY_COLOUR;

	int32_t widget_level = engine->level_index - 1;
	for (int32_t i = 0; i < st->num_levels; i++) {
		int32_t start_x = 10 + i * 5;
		int8_t colour = i <= widget_level ? LP85_EMPTY_COLOUR :
						    LP85_FILLED_COLOUR;
		for (int32_t c = 0; c < 4; c++) {
			int32_t x = start_x + c;
			if (x < ARC_FRAME_SIZE)
				frame[ARC_FRAME_SIZE + x] = colour;
		}
	}
}
