...
static void lp85_find_ring_sources(const Lp85Static *st, const ArcSprites *sprites,
...
static void lp85_rotate_ring(const Lp85Static *st, ArcSprites *sprites, Lp85Scratch *scratch,
...
static int lp85_piece_group_satisfied(const ArcSprites *sprites, const int32_t *slots,
                                      int32_t count, int32_t goal_tag) {
    for (int32_t k = 0; k < count; k++) {
        int32_t i = slots[k];
        if (!sprites->alive[i]) continue;
        if (arc_get_sprite_at(sprites, sprites->x[i] + 1, sprites->y[i] + 1, goal_tag, 0) < 0)
            return 0;
    }
    return 1;
}

static int lp85_solved(const Lp85Static *st, const ArcSprites *sprites, int32_t level) {
...
void lp85_render_interface(const Lp85Static *st, const Lp85Engine *engine,
    ...
    for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
        frame[(size_t)r * ARC_FRAME_SIZE] = r < filled ? LP85_FILLED_COLOUR : LP85_EMPTY_COLOUR;
    ...
    for (int32_t i = 0; i < st->num_levels; i++) {
        ...
        for (int32_t c = 0; c < 4; c++) {
            int32_t x = start_x + c;
            if (x < ARC_FRAME_SIZE) frame[ARC_FRAME_SIZE + x] = colour;
        }
    }
}
