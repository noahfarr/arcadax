...
static int8_t *sprite_pixels_mut_(ArcSprites *s, int32_t i) {
...
static int32_t rotation_of(const ArcSprites *s, int32_t slot) {
    const ArcAtlas *a = s->atlas;
...
static void rotate_one(ArcSprites *s, int32_t ph, int32_t pw, int32_t anchor_x, int32_t anchor_y,
                       int32_t k) {
...
static void rotate_closure(ArcSprites *s, const S5i5Static *st, int32_t level, int32_t root) {
    const ArcAtlas *a = s->atlas;
...
static void resize_one(ArcSprites *s, const S5i5Static *st, int32_t level, int32_t root,
                       int32_t new_len) {
    const ArcAtlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
    int32_t h0 = s->h[root], w0 = s->w[root];
...
static void backup_one(ArcSprites *s, S5i5Aux *aux, int32_t k) {
    const ArcAtlas *a = s->atlas;
    int32_t area = a->ph * a->pw;
    ...
    memcpy(aux->backup_pixels + (size_t)k * (size_t)area, arc_sprite_pixels(s, k), (size_t)area);
}

static void backup_closure(ArcSprites *s, S5i5Aux *aux, const S5i5Static *st, int32_t level,
                           int32_t root) {
...
static void do_revert(ArcSprites *s, S5i5Aux *aux) {
...
static int pixel_overlap_(const ArcSprites *s, int32_t i, int32_t j) {
    const ArcAtlas *a = s->atlas;
    int32_t ph = a->ph, pw = a->pw;
...
static int pair_collides(const ArcSprites *s, int32_t i, int32_t j) {
...
static int any_pipe_collision(const ArcSprites *s, const S5i5Static *st, int32_t level) {
...
static int check_win(const ArcSprites *s, const S5i5Static *st, int32_t level) {
...
static void finish(ArcSprites *s, S5i5Engine *e, S5i5Aux *aux, const S5i5Static *st) {
...
static void resolve_move(ArcSprites *s, S5i5Engine *e, S5i5Aux *aux, const S5i5Static *st,
                         int32_t level) {
...
static void process_root_rotation(ArcSprites *s, const S5i5Static *st, int32_t level, int32_t root) {
...
static void process_rotate(ArcSprites *s, S5i5Aux *aux, const S5i5Static *st, int32_t level,
                           int32_t handle) {
...
static void display_to_grid(const ArcCamera *cam, int32_t display_x, int32_t display_y,
...
static void process_resize(ArcSprites *s, S5i5Aux *aux, const S5i5Static *st, const ArcCamera *cam,
...
static void do_click(ArcSprites *s, S5i5Engine *e, S5i5Aux *aux, const S5i5Static *st,
                     const ArcCamera *cam) {
    ...
    int32_t handle = arc_get_sprite_at(s, world_x, world_y, st->handle_tag, 0);
    if (handle >= 0) {
        process_rotate(s, aux, st, level, handle);
        resolve_move(s, e, aux, st, level);
        return;
    }
    int32_t slider = arc_get_sprite_at(s, world_x, world_y, st->slider_tag, 0);
    if (slider >= 0) {
...
void s5i5_step_once(ArcSprites *sprites, const ArcCamera *camera, S5i5Engine *engine, S5i5Aux *aux,
                    const S5i5Static *st) {
    if (aux->pending) {
