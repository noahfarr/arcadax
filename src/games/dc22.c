...
enum {
	DC22_COLOR_DEATH = 4,
	DC22_COLOR_HUD_FILLED = 0,
	DC22_COLOR_HUD_EMPTY = 3,
	DC22_COLOR_VIGNETTE = 5
};
enum {
	DC22_DEATH_FRAMES = 14,
	DC22_DEATH_PENALTY = 20,
	DC22_SHAKE_FRAMES = 2,
	DC22_FLICKER_FRAMES = 5
};
...
static inline int32_t dc22_sort_key_descending(const struct arc_sprites *s,
					       int32_t i)
{
...
static inline int dc22_contains(const struct arc_sprites *s, int32_t i,
				int32_t x, int32_t y)
{
...
static inline int8_t dc22_pixel_value(const struct arc_sprites *s, int32_t i,
				      int32_t x, int32_t y)
{
...
static int dc22_pixel_overlap_ge0(const struct arc_sprites *s, int32_t i,
				  int32_t j)
{
	int32_t ph = s->atlas->ph, pw = s->atlas->pw;
	...
	for (int32_t v = 0; v < ph; v++) {
		...
		for (int32_t u = 0; u < pw; u++) {
			...
			if (uu < 0 || uu >= pw)
				continue;
			if (pi[(size_t)v * pw + u] >= 0 &&
			    pj[(size_t)vv * pw + uu] >= 0)
				return 1;
		}
	}
	return 0;
...
static int dc22_collides_ge0_pair(const struct arc_sprites *s, int32_t i,
				  int32_t j, int ignore_mode)
{
	...
	if (!ignore_mode) {
		if (!arc_sprite_collidable(s, i) ||
		    !arc_sprite_collidable(s, j))
			return 0;
		if (s->blocking[i] == NOT_BLOCKED ||
		    s->blocking[j] == NOT_BLOCKED)
			return 0;
	}
	return dc22_pixel_overlap_ge0(s, i, j);
}

static int dc22_collides_ge0_any(const struct arc_sprites *s, int32_t i,
				 int ignore_mode)
{
	...
	for (int32_t j = 0; j < n; j++) {
		if (j == i)
			continue;
		if (dc22_collides_ge0_pair(s, i, j, ignore_mode))
			return 1;
	}
	return 0;
}

static void dc22_display_to_grid(const struct arc_camera *camera,
...
static int32_t dc22_topmost_visible_opaque(const struct arc_sprites *s,
					   const struct dc22_static *st,
					   int32_t level, int32_t x, int32_t y,
					   const int32_t *cand, int32_t count)
{
	const uint8_t *is_ignore =
		st->is_ignore + (size_t)level * st->num_slots;
	int32_t best = -1, best_key = 0;
	int found = 0;
	for (int32_t k = 0; k < count; k++) {
		int32_t i = cand[k];
		if (!s->alive[i] || is_ignore[i])
			continue;
		if (s->interaction[i] > INTANGIBLE)
			continue;
		if (!dc22_contains(s, i, x, y))
			continue;
		if (dc22_pixel_value(s, i, x, y) < 0)
			continue;
...
static int dc22_on_valid_floor(const struct arc_sprites *s,
			       ...
			       int32_t x, int32_t y, int32_t exclude)
{
	...
	const uint8_t *is_crzsjq =
		st->is_crzsjq + (size_t)level * st->num_slots;
	const uint8_t *is_vcha = st->is_vcha + (size_t)level * st->num_slots;
	int32_t n = s->atlas->num_slots;
	for (int32_t i = 0; i < n; i++) {
		if (!s->alive[i] || i == exclude)
			continue;
		if (is_ignore[i] || is_crzsjq[i] || is_vcha[i])
			continue;
		if (s->interaction[i] != INTANGIBLE)
			continue;
		if (!dc22_contains(s, i, x, y))
...
static int32_t dc22_find_piyqze_hit(const struct arc_sprites *s,
				    const struct dc22_static *st, int32_t level,
				    int32_t x, int32_t y)
{
	const uint8_t *is_ignore =
...
static void dc22_save_undo(struct arc_sprites *s, struct dc22_aux *aux)
{
	int32_t n = s->atlas->num_slots;
	int32_t area = s->atlas->ph * s->atlas->pw;
	for (int32_t i = 0; i < n; i++)
		memcpy(aux->undo_pixels + (size_t)i * area,
		       arc_sprite_pixels(s, i), (size_t)area);
...
static void dc22_restore_undo(struct arc_sprites *s, struct dc22_aux *aux)
{
	int32_t n = s->atlas->num_slots;
	int32_t area = s->atlas->ph * s->atlas->pw;
	for (int32_t i = 0; i < n; i++) {
...
static void dc22_apply_njvd_gating(struct arc_sprites *s,
				   const struct dc22_static *st, int32_t level)
{
	...
	const int32_t *governed =
		st->governed_slots + (size_t)level * st->max_governed;
...
static void dc22_apply_generic_swap(struct arc_sprites *s,
				    const struct dc22_static *st, int32_t level,
				    int32_t code, int32_t exclude)
{
	int32_t idx = level * st->num_codes + code;
	const int32_t *members =
		st->code_member_slots + (size_t)idx * st->max_code_member;
	int32_t count = st->code_member_count[idx];
	const int32_t *next_slot =
		st->next_slot + (size_t)level * st->num_slots;
	const int32_t *next_interaction =
		st->next_interaction + (size_t)level * st->num_slots;
	int8_t snapshot[DC22_MAX_FAMILY_SNAPSHOT];
	for (int32_t k = 0; k < count; k++)
		snapshot[k] = s->interaction[members[k]];
	for (int32_t k = 0; k < count; k++) {
		int32_t i = members[k];
		if (!s->alive[i] || i == exclude)
			continue;
		if (snapshot[k] > INTANGIBLE)
			continue;
...
static void dc22_apply_tewfut_color_cycle(struct arc_sprites *s,
					  const struct dc22_static *st,
					  int32_t level, int32_t code,
					  int32_t exclude)
{
	int32_t idx = level * st->num_codes + code;
	const int32_t *members =
	...
	const uint8_t *is_tewfut =
		st->is_tewfut + (size_t)level * st->num_slots;
	const uint8_t *is_qiukbrokfa =
		st->is_qiukbrokfa + (size_t)level * st->num_slots;
	const int32_t *next_slot =
		st->next_slot + (size_t)level * st->num_slots;
	const int32_t *next_interaction =
		st->next_interaction + (size_t)level * st->num_slots;
	...
	for (int32_t k = 0; k < count; k++) {
		...
		if (!s->alive[i] || i == exclude)
			continue;
		if (!is_tewfut[i] || !is_qiukbrokfa[i])
			continue;
		if (snapshot[k] > INTANGIBLE)
			continue;
		int32_t target = next_slot[i];
...
static void dc22_teleport_if_standing(struct arc_sprites *s,
				      const struct dc22_static *st,
				      int32_t level, int32_t code,
				      int32_t exclude)
{
	...
	const int32_t *teleport_name =
		st->teleport_name + (size_t)level * st->num_slots;

	int32_t standing = -1;
	for (int32_t k = 0; k < count; k++) {
		int32_t i = members[k];
		if (!s->alive[i] || i == exclude || !is_tewfut[i])
			continue;
		if (s->interaction[i] > INTANGIBLE)
			continue;
...
static void dc22_reveal_by_piyqze(struct arc_sprites *s,
				  const struct dc22_static *st, int32_t level,
				  int32_t hit)
{
	if (hit < 0)
	...
	const int32_t *buezna =
		st->buezna_slots + (size_t)level * st->max_buezna;
	int32_t count = st->buezna_count[level];
	for (int32_t k = 0; k < count; k++) {
		int32_t i = buezna[k];
		if (code_of[i] == code)
			s->interaction[i] = INTANGIBLE;
	}
	s->alive[hit] = 0;
}

static void dc22_hide_aybe(struct arc_sprites *s, const struct dc22_static *st,
			   int32_t level)
{
...
static void dc22_crane_anchor(const struct dc22_static *st, int32_t level,
			      int32_t col, int32_t row, int32_t *ax,
			      int32_t *ay)
{
...
static int dc22_crane_move_valid(const struct dc22_static *st, int32_t level,
				 const struct dc22_aux *aux, int32_t direction,
				 int32_t new_col, int32_t new_row)
{
	int32_t ax, ay;
	dc22_crane_anchor(st, level, new_col, new_row, &ax, &ay);
	int in_bounds = ax >= 0 && ax < ARC_FRAME_SIZE && ay >= 0 &&
			ay < ARC_FRAME_SIZE;
	int32_t axc = dc22_clamp(ax, 0, ARC_FRAME_SIZE - 1),
		ayc = dc22_clamp(ay, 0, ARC_FRAME_SIZE - 1);
	int in_track =
		in_bounds &&
		st->vcha_map[(size_t)level * ARC_FRAME_SIZE * ARC_FRAME_SIZE +
			     (size_t)ayc * ARC_FRAME_SIZE + axc];

	int32_t col = aux->crane_col, row = aux->crane_row;
	int grid_ok = 0;
	if (direction == DC22_DIR_UP)
		grid_ok = (col == 0) && (row < 3);
	else if (direction == DC22_DIR_DOWN)
		grid_ok = (col == 0) && (row > 0);
	else if (direction == DC22_DIR_LEFT)
		grid_ok = ((row == 3) || (row == 0)) && (col > 0);
	else if (direction == DC22_DIR_RIGHT)
		grid_ok = ((row == 3) || (row == 0)) && (col < 3);

	return st->crane_is_brixto[level] ? in_track : grid_ok;
}

static void dc22_reposition_attachment(struct arc_sprites *s,
				       const struct dc22_aux *aux,
				       int32_t anchor_x, int32_t anchor_y)
{
	if (aux->attach_kind == DC22_ATTACH_NONE)
		return;
...
static void dc22_move_crane_to(struct arc_sprites *s,
			       const struct dc22_static *st, int32_t level,
			       struct dc22_aux *aux, int32_t col, int32_t row)
{
	int32_t ax, ay;
...
static void dc22_start_shake(struct arc_sprites *s,
			     const struct dc22_static *st, int32_t level,
			     struct dc22_aux *aux, int32_t dx, int32_t dy)
{
	int32_t slot = st->crane_slot[level];
	aux->shake_active = 1;
	aux->shake_frame = 0;
...
static int dc22_bridge_move_blocked(struct arc_sprites *s,
				    const struct dc22_static *st, int32_t level,
...
static void dc22_attach(struct arc_sprites *s, const struct dc22_static *st,
			int32_t level, struct dc22_aux *aux, int32_t kind,
			int32_t slot, int32_t anchor_x, int32_t anchor_y)
{
	aux->attach_kind = kind;
	aux->attach_slot = slot;
	int32_t area = s->atlas->ph * s->atlas->pw;
	int8_t *dst = arc_sprite_pixels_mut(s, st->crane_slot[level]);
...
static void dc22_resolve_crane_click(struct arc_sprites *s,
				     const struct dc22_static *st,
				     int32_t level, struct dc22_aux *aux,
				     int32_t direction, int *blocked,
				     int *is_directional_block,
				     int32_t *shake_dx, int32_t *shake_dy)
{
	int32_t col = aux->crane_col, row = aux->crane_row;
	int32_t new_col = col, new_row = row;
	if (direction == DC22_DIR_LEFT)
		new_col = col - 1;
	else if (direction == DC22_DIR_RIGHT)
		new_col = col + 1;
	if (direction == DC22_DIR_UP)
	...
	*shake_dy = 0;
	...
	if (is_directional) {
		int move_ok = dc22_crane_move_valid(st, level, aux, direction,
						    new_col, new_row);
		if (move_ok) {
			int32_t ax, ay;
			dc22_crane_anchor(st, level, new_col, new_row, &ax,
					  &ay);
			if (aux->attach_kind == DC22_ATTACH_BRIXTO) {
				int32_t slot = aux->attach_slot < 0 ?
						       0 :
						       aux->attach_slot;
				int32_t nx = ax - s->w[slot] / 2;
				int32_t ny = ay - s->h[slot] / 2;
				if (dc22_bridge_move_blocked(s, st, level, slot,
							     nx, ny)) {
					*blocked = 1;
					...
					*shake_dy = DC22_DIR_STEP_Y[direction];
					...
					dc22_move_crane_to(s, st, level, aux,
							   new_col, new_row);
				}
			} else {
				dc22_move_crane_to(s, st, level, aux, new_col,
						   new_row);
			}
		} else {
			*blocked = 1;
	...
	const int32_t *bcand =
		st->brixto_slots + (size_t)level * st->max_brixto;
	...
	if (already) {
		...
		dc22_attach(s, st, level, aux, DC22_ATTACH_BRIXTO, bridge_slot,
			    ax, ay);
	} else if (found_object) {
		dc22_attach(s, st, level, aux, DC22_ATTACH_OBJECT, obj_slot, ax,
			    ay);
	} else {
...
static void dc22_next_level(const struct dc22_static *st, int32_t level,
...
static int dc22_spend_step(struct dc22_aux *aux)
{
	aux->steps -= 1;
	return aux->steps > 0;
}

static inline void dc22_complete(uint8_t *action_complete)
{
	*action_complete = 1;
}
...
static void dc22_noop(const struct dc22_static *st, int32_t level,
		      struct dc22_aux *aux, struct arc_sprites *s,
...
static void dc22_handle_move(struct arc_sprites *s,
			     const struct dc22_static *st, int32_t level,
			     int32_t dx, int32_t dy, struct dc22_aux *aux,
			     int32_t *score, int32_t *status,
			     uint8_t *next_level, uint8_t *action_complete)
{
	...
	int ok = dc22_on_valid_floor(s, st, level, s->x[player], s->y[player],
				     player);
	if (!blocked && !ok) {
		s->x[player] = before_x;
		s->y[player] = before_y;
	...
	int32_t piyqze_hit =
		dc22_find_piyqze_hit(s, st, level, s->x[player], s->y[player]);
...
static void dc22_handle_click(struct arc_sprites *s,
			      const struct arc_camera *camera,
			      const struct dc22_static *st, int32_t level,
			      int32_t action_x, int32_t action_y,
			      struct dc22_aux *aux, int32_t *status,
			      uint8_t *action_complete)
{
	...
	if (on_board) {
		int32_t buezna_hit = dc22_topmost_visible_opaque(
			s, st, level, wx, wy,
			st->buezna_slots + (size_t)level * st->max_buezna,
			st->buezna_count[level]);
		int hit_buezna = buezna_hit >= 0;
		if (hit_buezna) {
			const int32_t *code_of =
				st->code_of + (size_t)level * st->num_slots;
			...
			if (is_color_cycle) {
				dc22_apply_tewfut_color_cycle(s, st, level,
							      code, buezna_hit);
			} else {
				dc22_apply_generic_swap(s, st, level, code,
							buezna_hit);
				dc22_teleport_if_standing(s, st, level, code,
							  buezna_hit);
			}
		}
		...
		if (crane_present) {
			click_hit = dc22_topmost_visible_opaque(
				s, st, level, wx, wy,
				st->sys_click_slots +
					(size_t)level * st->max_sys_click,
				st->sys_click_count[level]);
		}
		int32_t direction = -1;
		if (click_hit >= 0) {
			const int32_t *dir_slot =
				st->dir_slot + (size_t)level * DC22_NUM_DIRS;
			for (int32_t d = 0; d < DC22_NUM_DIRS; d++)
				if (dir_slot[d] == click_hit)
					direction = d;
		}
		...
		if (hit_click) {
			dc22_resolve_crane_click(
				s, st, level, aux, direction, &blocked,
				&is_directional_block, &shake_dx, &shake_dy);
		}

		int32_t player = st->player_slot[level];
		ok = dc22_on_valid_floor(s, st, level, s->x[player],
					 s->y[player], player);
		...
		if (!ok) {
			...
			int has_budget = dc22_spend_step(aux);
			if (has_budget)
				dc22_start_shake(s, st, level, aux, shake_dx,
						 shake_dy);
			else
				dc22_lose_now(status, action_complete);
		}
...
static void dc22_play(struct arc_sprites *s, const struct arc_camera *camera,
		      const struct dc22_static *st, int32_t level,
...
{
	int32_t dx = 0, dy = 0;
	if (action_id == DC22_ACTION3)
		dx = -DC22_STEP_MOVE;
	else if (action_id == DC22_ACTION4)
		dx = DC22_STEP_MOVE;
	if (action_id == DC22_ACTION1)
		dy = -DC22_STEP_MOVE;
	else if (action_id == DC22_ACTION2)
		dy = DC22_STEP_MOVE;
	int is_click = action_id == DC22_ACTION6;
	int is_move = !is_click && (dx != 0 || dy != 0);

	if (is_click) {
		dc22_handle_click(s, camera, st, level, action_x, action_y, aux,
				  status, action_complete);
	} else if (is_move) {
		dc22_handle_move(s, st, level, dx, dy, aux, score, status,
...
static void dc22_animate_death(struct arc_sprites *s,
			       const struct dc22_static *st, int32_t level,
			       struct dc22_aux *aux, int32_t *status,
			       uint8_t *action_complete)
{
	...
	for (int32_t r = 0; r < ph; r++) {
		for (int32_t c = 0; c < pw; c++) {
			...
			if (in_bounds && flat_index <= filled)
				patch[(size_t)r * pw + c] =
					(int8_t)DC22_COLOR_DEATH;
		}
	}
...
static void dc22_animate_shake(struct arc_sprites *s,
			       const struct dc22_static *st, int32_t level,
			       struct dc22_aux *aux, uint8_t *action_complete)
{
	int32_t slot = st->crane_slot[level];
	int32_t x = (aux->shake_frame == 0) ?
			    aux->shake_origin_x + aux->shake_dx :
			    aux->shake_origin_x;
	int32_t y = (aux->shake_frame == 0) ?
			    aux->shake_origin_y + aux->shake_dy :
			    aux->shake_origin_y;
	s->x[slot] = x;
	s->y[slot] = y;
...
static void dc22_animate_flicker(struct arc_sprites *s,
				 const struct dc22_static *st, int32_t level,
				 struct dc22_aux *aux, uint8_t *action_complete)
{
	int32_t slot = st->crane_slot[level];
	...
	const int8_t *src =
		variant2 ? (st->crane_hold_pixels + (size_t)level * area) :
			   (s->atlas->pixels + (size_t)slot * area);
	memcpy(arc_sprite_pixels_mut(s, slot), src, (size_t)area);

	int32_t frame = aux->flicker_frame + 1;
	aux->flicker_frame = frame;

	if (frame == DC22_FLICKER_FRAMES) {
		memcpy(arc_sprite_pixels_mut(s, slot),
		       s->atlas->pixels + (size_t)slot * area, (size_t)area);
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (aux->death_active) {
		dc22_animate_death(sprites, st, level, aux, status,
				   action_complete);
		...
		dc22_play(sprites, camera, st, level, action_id, action_x,
			  action_y, aux, score, status, next_level,
			  action_complete);
	}
}

static void dc22_draw_budget(int8_t *frame, const struct dc22_static *st,
			     int32_t level, const struct dc22_aux *aux)
{
	...
	for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) {
		int filled = (c * budget) >= (ARC_FRAME_SIZE * done);
		row[c] = (int8_t)(filled ? DC22_COLOR_HUD_FILLED :
					   DC22_COLOR_HUD_EMPTY);
	}
}

static void dc22_draw_death_vignette(int8_t *frame,
				     const struct arc_sprites *sprites,
				     const struct arc_camera *camera,
				     const struct dc22_static *st,
				     int32_t level, const struct dc22_aux *aux)
{
	...
	int32_t scale = ARC_FRAME_SIZE / camera->width;
	int32_t sy = ARC_FRAME_SIZE / camera->height;
	if (sy < scale)
		scale = sy;
	int32_t x_offset = (ARC_FRAME_SIZE - camera->width * scale) / 2;
	int32_t y_offset = (ARC_FRAME_SIZE - camera->height * scale) / 2;
	int32_t rel_x = sprites->x[player] - camera->x;
	int32_t rel_y = sprites->y[player] - camera->y;

	if (full) {
		memset(frame, DC22_COLOR_VIGNETTE,
