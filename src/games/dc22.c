...
static inline int32_t dc22_sort_key_descending(const ArcSprites *s, int32_t i) {
    return s->layer[i] * ARC_ORDER_BITS + (ARC_ORDER_BITS - 1 - s->order[i]);
}

static inline int dc22_contains(const ArcSprites *s, int32_t i, int32_t x, int32_t y) {
    return x >= s->x[i] && y >= s->y[i] && x < s->x[i] + s->w[i] && y < s->y[i] + s->h[i];
}

static inline int8_t dc22_pixel_value(const ArcSprites *s, int32_t i, int32_t x, int32_t y) {
...
static int dc22_pixel_overlap_ge0(const ArcSprites *s, int32_t i, int32_t j) {
    int32_t ph = s->atlas->ph, pw = s->atlas->pw;
...
static int dc22_collides_ge0_pair(const ArcSprites *s, int32_t i, int32_t j, int ignore_mode) {
...
static int dc22_collides_ge0_any(const ArcSprites *s, int32_t i, int ignore_mode) {
...
static void dc22_display_to_grid(const ArcCamera *camera, int32_t display_x, int32_t display_y,
...
static int32_t dc22_topmost_visible_opaque(const ArcSprites *s, const Dc22Static *st, int32_t level,
...
static int dc22_on_valid_floor(const ArcSprites *s, const Dc22Static *st, int32_t level,
...
static int32_t dc22_find_piyqze_hit(const ArcSprites *s, const Dc22Static *st, int32_t level,
                                    int32_t x, int32_t y) {
    const uint8_t *is_ignore = st->is_ignore + (size_t)level * st->num_slots;
...
static void dc22_save_undo(ArcSprites *s, Dc22Aux *aux) {
    int32_t n = s->atlas->num_slots;
    int32_t area = s->atlas->ph * s->atlas->pw;
    for (int32_t i = 0; i < n; i++)
        memcpy(aux->undo_pixels + (size_t)i * area, arc_sprite_pixels(s, i), (size_t)area);
...
static void dc22_restore_undo(ArcSprites *s, Dc22Aux *aux) {
    int32_t n = s->atlas->num_slots;
    int32_t area = s->atlas->ph * s->atlas->pw;
    for (int32_t i = 0; i < n; i++) {
...
static void dc22_apply_njvd_gating(ArcSprites *s, const Dc22Static *st, int32_t level) {
...
static void dc22_apply_generic_swap(ArcSprites *s, const Dc22Static *st, int32_t level,
...
static void dc22_apply_tewfut_color_cycle(ArcSprites *s, const Dc22Static *st, int32_t level,
                                          int32_t code, int32_t exclude) {
    int32_t idx = level * st->num_codes + code;
    const int32_t *members = st->code_member_slots + (size_t)idx * st->max_code_member;
    int32_t count = st->code_member_count[idx];
...
static void dc22_teleport_if_standing(ArcSprites *s, const Dc22Static *st, int32_t level,
                                      int32_t code, int32_t exclude) {
    int32_t player = st->player_slot[level];
    int32_t idx = level * st->num_codes + code;
    const int32_t *members = st->code_member_slots + (size_t)idx * st->max_code_member;
...
static void dc22_reveal_by_piyqze(ArcSprites *s, const Dc22Static *st, int32_t level, int32_t hit) {
    if (hit < 0) return;
    const int32_t *code_of = st->code_of + (size_t)level * st->num_slots;
...
static void dc22_hide_aybe(ArcSprites *s, const Dc22Static *st, int32_t level) {
...
static int dc22_crane_move_valid(const Dc22Static *st, int32_t level, const Dc22Aux *aux,
    ...
    int in_bounds = ax >= 0 && ax < ARC_FRAME_SIZE && ay >= 0 && ay < ARC_FRAME_SIZE;
    int32_t axc = dc22_clamp(ax, 0, ARC_FRAME_SIZE - 1), ayc = dc22_clamp(ay, 0, ARC_FRAME_SIZE - 1);
    int in_track = in_bounds && st->vcha_map[(size_t)level * ARC_FRAME_SIZE * ARC_FRAME_SIZE + (size_t)ayc * ARC_FRAME_SIZE + axc];
...
static void dc22_reposition_attachment(ArcSprites *s, const Dc22Aux *aux, int32_t anchor_x, int32_t anchor_y) {
...
static void dc22_move_crane_to(ArcSprites *s, const Dc22Static *st, int32_t level, Dc22Aux *aux,
...
static void dc22_start_shake(ArcSprites *s, const Dc22Static *st, int32_t level, Dc22Aux *aux,
...
static int dc22_bridge_move_blocked(ArcSprites *s, const Dc22Static *st, int32_t level,
...
static void dc22_attach(ArcSprites *s, const Dc22Static *st, int32_t level, Dc22Aux *aux,
...
static void dc22_resolve_crane_click(ArcSprites *s, const Dc22Static *st, int32_t level, Dc22Aux *aux,
...
static void dc22_noop(const Dc22Static *st, int32_t level, Dc22Aux *aux, ArcSprites *s,
...
static void dc22_handle_move(ArcSprites *s, const Dc22Static *st, int32_t level, int32_t dx,
...
static void dc22_handle_click(ArcSprites *s, const ArcCamera *camera, const Dc22Static *st,
...
static void dc22_play(ArcSprites *s, const ArcCamera *camera, const Dc22Static *st, int32_t level,
...
static void dc22_animate_death(ArcSprites *s, const Dc22Static *st, int32_t level, Dc22Aux *aux,
                               int32_t *status, uint8_t *action_complete) {
...
static void dc22_animate_shake(ArcSprites *s, const Dc22Static *st, int32_t level, Dc22Aux *aux,
                               uint8_t *action_complete) {
    int32_t slot = st->crane_slot[level];
...
static void dc22_animate_flicker(ArcSprites *s, const Dc22Static *st, int32_t level, Dc22Aux *aux,
                                 uint8_t *action_complete) {
    int32_t slot = st->crane_slot[level];
    int32_t area = s->atlas->ph * s->atlas->pw;
    ...
    if (frame == DC22_FLICKER_FRAMES) {
        memcpy(arc_sprite_pixels_mut(s, slot), s->atlas->pixels + (size_t)slot * area, (size_t)area);
...
void dc22_step_once(ArcSprites *sprites, const ArcCamera *camera, const Dc22Static *st, int32_t level,
                    int32_t action_id, int32_t action_x, int32_t action_y, Dc22Aux *aux,
...
static void dc22_draw_death_vignette(int8_t *frame, const ArcSprites *sprites, const ArcCamera *camera,
    ...
    if (full) {
        memset(frame, DC22_COLOR_VIGNETTE, (size_t)ARC_FRAME_SIZE * ARC_FRAME_SIZE);
        return;
    }
    ...
    for (int32_t r = 0; r < ARC_FRAME_SIZE; r++) {
        for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) {
            ...
            if (dist > radius) frame[(size_t)r * ARC_FRAME_SIZE + c] = (int8_t)DC22_COLOR_VIGNETTE;
        }
    }
}

void dc22_render_interface(int8_t *frame, const ArcSprites *sprites, const ArcCamera *camera,
                           const Dc22Static *st, int32_t level, const Dc22Aux *aux) {
