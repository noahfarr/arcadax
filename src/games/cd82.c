...
static void cd82_rebuild(ArcSprites *s, const Cd82Static *st, int32_t level,
    ...
    *next_order += 1;
    ...
    arc_set_position(s, st->arrow_slot, CD82_ARROW_X[pos], CD82_ARROW_Y[pos]);
...
static void cd82_move_ring(ArcSprites *s, const Cd82Static *st, int32_t level,
...
static void cd82_paint_canvas(ArcSprites *s, const Cd82Static *st, int32_t level,
...
static void cd82_check_win(ArcSprites *s, const Cd82Static *st, int32_t level,
...
static void cd82_continue_draw(ArcSprites *s, const Cd82Static *st, int32_t level,
...
static void cd82_continue_bounce(ArcSprites *s, const Cd82Static *st, int32_t level,
                                 Cd82Aux *aux, int32_t *score, int32_t *status,
                                 uint8_t *next_level, uint8_t *action_complete) {
    ...
    if (aux->bounce_forward) {
        ...
        if (aux->bounce_counter <= 0) {
            arc_set_position(s, st->arrow_slot, CD82_ARROW_X[dir], CD82_ARROW_Y[dir]);
            aux->bouncing = 0;
            cd82_check_win(s, st, level, score, status, next_level);
            *action_complete = 1;
        }
...
static void cd82_handle_click(ArcSprites *s, const ArcCamera *camera, const Cd82Static *st,
...
static void cd82_dispatch(ArcSprites *s, const ArcCamera *camera, const Cd82Static *st,
