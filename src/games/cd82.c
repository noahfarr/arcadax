...
static const int32_t CD82_POS_ROTATION[8] = {
	180, 0, 270, 90, 0, 180, 90, 270
};
...
static int cd82_coublenfir(int32_t direction, int32_t r, int32_t c)
{
	switch (direction) {
	case 0:
		return r >= 0 && r < 3 && c >= 3 && c < 7;
	case 4:
		return r >= 7 && r < 10 && c >= 3 && c < 7;
	case 6:
		return r >= 3 && r < 7 && c >= 0 && c < 3;
	case 2:
		return r >= 3 && r < 7 && c >= 7 && c < 10;
	default:
		return 0;
	}
}

static void cd82_rebuild(struct arc_sprites *s, const struct cd82_static *st,
			 int32_t level, struct cd82_aux *aux,
			 int32_t *next_order)
{
	...
	int8_t *bdst = arc_sprite_pixels_mut(s, st->basket_slot);
	for (int32_t k = 0; k < area; k++)
		bdst[k] = bis15[k] ? color : btpl[k];
	arc_set_position(s, st->basket_slot, CD82_POS_X[pos], CD82_POS_Y[pos]);
	...
	arc_set_interaction(s, st->basket_slot, TANGIBLE);
	...
	*next_order += 1;
	...
	int8_t *adst = arc_sprite_pixels_mut(s, st->arrow_slot);
	for (int32_t k = 0; k < area; k++)
		adst[k] = ais15[k] ? color : atpl[k];
	arc_set_position(s, st->arrow_slot, CD82_ARROW_X[pos],
			 CD82_ARROW_Y[pos]);
	...
	arc_set_interaction(s, st->arrow_slot, TANGIBLE);
	s->blocking[st->arrow_slot] = PIXEL_PERFECT;
}

static void cd82_move_ring(struct arc_sprites *s, const struct cd82_static *st,
			   int32_t level, struct cd82_aux *aux,
			   int32_t *next_order, int32_t direction)
{
...
static void cd82_paint_canvas(struct arc_sprites *s,
			      const struct cd82_static *st, int32_t level,
			      int8_t color,
			      int (*mask_fn)(int32_t, int32_t, int32_t),
			      int32_t mask_arg)
{
	int32_t slot = st->canvas_slot[level];
	int32_t pw = st->pw;
	int8_t *px = arc_sprite_pixels_mut(s, slot);
	for (int32_t r = 0; r < 10; r++)
		for (int32_t c = 0; c < 10; c++)
			if (mask_fn(mask_arg, r, c))
				px[r * pw + c] = color;
}

static void cd82_next_level(const struct cd82_static *st, int32_t level,
...
static void cd82_check_win(struct arc_sprites *s, const struct cd82_static *st,
			   int32_t level, int32_t *score, int32_t *status,
			   uint8_t *next_level)
{
	int32_t canvas_slot = st->canvas_slot[level];
	int32_t answer_slot = st->answer_slot[level];
	const int8_t *canvas = arc_sprite_pixels(s, canvas_slot);
	const int8_t *answer = arc_sprite_pixels(s, answer_slot);
	int32_t pw = st->pw;
	int equal = 1;
	...
	if (equal)
		cd82_next_level(st, level, score, status, next_level);
}

static void cd82_start_draw(struct cd82_aux *aux)
{
	aux->drawing = 1;
	aux->draw_forward = 1;
	aux->draw_counter = 0;
...
static void cd82_continue_draw(struct arc_sprites *s,
			       const struct cd82_static *st, int32_t level,
			       struct cd82_aux *aux, int32_t *score,
			       ...
			       uint8_t *action_complete)
{
	...
	if (aux->draw_forward) {
		arc_move_sprite(s, st->basket_slot, dx, dy);
		aux->draw_counter += 1;
		int reached = aux->draw_counter >= CD82_DRAW_REACH;
		if (reached && !aux->draw_painted) {
			cd82_paint_canvas(s, st, level, (int8_t)aux->color,
					  cd82_rtjwayrycq, pos);
			aux->draw_painted = 1;
		}
		if (reached)
		...
		if (aux->draw_counter <= 0) {
			arc_set_position(s, st->basket_slot, CD82_POS_X[pos],
					 CD82_POS_Y[pos]);
			aux->drawing = 0;
...
static void cd82_start_bounce(struct cd82_aux *aux)
{
	aux->bouncing = 1;
...
static void cd82_continue_bounce(struct arc_sprites *s,
				 const struct cd82_static *st, int32_t level,
				 struct cd82_aux *aux, int32_t *score,
				 int32_t *status, uint8_t *next_level,
				 uint8_t *action_complete)
{
	...
	if (aux->bounce_forward) {
		arc_move_sprite(s, st->arrow_slot, dx, dy);
		...
		if (reached)
			aux->bounce_forward = 0;
	} else {
		arc_move_sprite(s, st->arrow_slot, -dx, -dy);
		aux->bounce_counter -= 1;
		if (aux->bounce_counter <= 0) {
			arc_set_position(s, st->arrow_slot, CD82_ARROW_X[dir],
					 CD82_ARROW_Y[dir]);
			aux->bouncing = 0;
			cd82_check_win(s, st, level, score, status, next_level);
			*action_complete = 1;
...
static void cd82_handle_click(struct arc_sprites *s,
			      ...
			      uint8_t *action_complete)
{
	...
	int32_t hit = arc_get_sprite_at(s, wx, wy, -1, 1);
	hit = on_board ? hit : -1;
	int32_t hit_clip = hit < 0 ? 0 : hit;
	int is_arrow =
		hit >= 0 && hit == st->arrow_slot && s->alive[st->arrow_slot];
	int is_palette =
		hit >= 0 &&
		st->palette_mask[(size_t)level * st->num_slots + hit_clip];

	if (is_arrow) {
		cd82_start_bounce(aux);
	} else if (is_palette) {
		int32_t color = arc_sprite_pixels(s, hit_clip)[2 * st->pw + 2];
		int32_t marker_slot = st->marker_slot[level];
		int32_t hx = s->x[hit_clip], hy = s->y[hit_clip];
		arc_set_position(s, marker_slot, hx, hy + 5);
		aux->color = color;
		cd82_rebuild(s, st, level, aux, next_order);
	}
	if (!aux->bouncing)
...
static void cd82_dispatch(struct arc_sprites *s,
			  const struct arc_camera *camera,
			  const struct cd82_static *st, int32_t level,
			  ...
			  uint8_t *action_complete)
{
	...
	case CD82_ACTION5:
		...
		break;
	case CD82_ACTION6:
		cd82_handle_click(s, camera, st, level, action_x, action_y, aux,
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (aux->drawing) {
		cd82_continue_draw(sprites, st, level, aux, score, status,
				   next_level, action_complete);
	} else if (aux->bouncing) {
		cd82_continue_bounce(sprites, st, level, aux, score, status,
				     next_level, action_complete);
	} else {
		cd82_dispatch(sprites, camera, st, level, action_id, action_x,
			      action_y, aux, next_order, action_complete);
	}
}
...
void cd82_render_interface(int8_t *frame, int32_t action_count)
{
	...
	if (steps < 0)
		steps = 0;
	if (steps > CD82_STEP_BUDGET)
		steps = CD82_STEP_BUDGET;
	int32_t total = ARC_FRAME_SIZE * steps;
	int32_t whole = total / CD82_STEP_BUDGET,
		rest = total % CD82_STEP_BUDGET;
