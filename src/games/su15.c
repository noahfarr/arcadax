...
static int su15_overlap(const struct arc_sprites *s, int32_t a, int32_t b)
{
	return s->x[a] < s->x[b] + s->w[b] && s->x[b] < s->x[a] + s->w[a] &&
	       s->y[a] < s->y[b] + s->h[b] && s->y[b] < s->y[a] + s->h[a];
}

static int su15_within_radius(int32_t bx, int32_t by, int32_t w, int32_t h,
...
static void su15_clamp_position(int32_t x, int32_t y, int32_t w, int32_t h,
				int32_t *ox, int32_t *oy)
{
	*ox = su15_clampi(x, 0, SU15_BOARD_SIZE - w);
	int32_t yhi = SU15_CLICK_Y_MAX < SU15_BOARD_SIZE - h ?
			      SU15_CLICK_Y_MAX :
			      SU15_BOARD_SIZE - h;
	*oy = su15_clampi(y, SU15_CLICK_Y_MIN, yhi);
}

static int32_t su15_advance_axis(int32_t pos, int32_t floor_c, int32_t round_c,
				 int tie)
{
...
static void su15_display_to_grid(const struct arc_camera *camera,
...
static void su15_spawn_pixels_one(const struct su15_static *st, int32_t tier,
				  int32_t fruit_kind, int8_t *out_pixels,
				  ...
				  int32_t *out_layer)
{
	...
	if (tier >= 0) {
		...
		memcpy(out_pixels, st->tier_pixels + (size_t)t * area,
		       (size_t)area);
		*out_w = st->tier_w[t];
		*out_h = st->tier_h[t];
		*out_layer = st->tier_layer[t];
		...
		int32_t f =
			su15_clampi(fruit_kind, 0, SU15_NUM_FRUIT_KINDS - 1);
		memcpy(out_pixels, st->fruit_pixels + (size_t)f * area,
		       (size_t)area);
		*out_w = st->fruit_w[f];
		*out_h = st->fruit_h[f];
...
static void su15_pull_lookup(const struct su15_static *st, int32_t dx,
			     int32_t dy, int32_t *floor_x, int32_t *round_x,
			     int *tie_x, int32_t *floor_y, int32_t *round_y,
			     int *tie_y)
{
...
static void su15_clear_glow(struct arc_sprites *s, const struct su15_static *st)
{
	int8_t *patch = arc_sprite_pixels_mut(s, st->glow_slot);
	memset(patch, -1, (size_t)(st->ph * st->pw));
}

static void su15_draw_glow(struct arc_sprites *s, const struct su15_static *st,
			   const struct su15_aux *aux)
{
	int32_t idx = su15_clampi(aux->pull_frame, 0, SU15_GLOW_STATES - 1);
	int8_t *patch = arc_sprite_pixels_mut(s, st->glow_slot);
	memset(patch, -1, (size_t)(st->ph * st->pw));
	const int8_t *pattern =
		st->glow_states + (size_t)idx * SU15_GLOW_SIZE * SU15_GLOW_SIZE;
	for (int32_t row = 0; row < SU15_GLOW_SIZE; row++)
		memcpy(patch + (size_t)row * st->pw,
		       pattern + (size_t)row * SU15_GLOW_SIZE, SU15_GLOW_SIZE);
...
static void su15_apply_flash_color(struct arc_sprites *s,
				   const struct su15_static *st,
				   const struct su15_aux *aux, int lit)
{
	int32_t area = st->ph * st->pw;
	...
	for (int k = 0; k < 2; k++) {
		...
		int32_t w, h, layer;
		su15_spawn_pixels_one(st, aux->tier[slot],
				      aux->fruit_kind[slot], pix, &w, &h,
				      &layer);
		int8_t *dst = arc_sprite_pixels_mut(s, slot);
		if (lit) {
			for (int32_t p = 0; p < area; p++)
				dst[p] = pix[p] >= 0 ?
						 (int8_t)SU15_COLOR_EMPTY :
						 pix[p];
		} else {
...
static void su15_begin_mismatch_flash(struct arc_sprites *s,
				      const struct su15_static *st,
				      struct su15_aux *aux, int32_t a,
				      int32_t b)
{
...
static void su15_merge_and_flash(struct arc_sprites *s,
				 const struct su15_static *st,
				 struct su15_aux *aux, int32_t *next_order,
				 const int32_t *cand, int32_t m, int is_blob)
{
	if (m < 2)
		return;
	...
	for (int32_t k = 0; k < m; k++) {
		for (int32_t j = 0; j < m; j++) {
			...
			adj[k][j] =
				(uint8_t)(su15_overlap(s, cand[k], cand[j]) &&
					  key[cand[k]] == key[cand[j]]);
		}
	}
	...
	int32_t group_count[SU15_MAX_CAND], avg_cx[SU15_MAX_CAND],
		avg_cy[SU15_MAX_CAND];
	for (int32_t k = 0; k < m; k++) {
	...
	int32_t max_key =
		is_blob ? SU15_NUM_TIERS - 1 : SU15_NUM_FRUIT_KINDS - 1;
	int any_member = 0;
	...
	for (int32_t k = 0; k < m; k++) {
		...
		if (keep_as_new) {
			...
			int32_t w, h, layer;
			if (is_blob)
				su15_spawn_pixels_one(st, new_key, -1, pix, &w,
						      &h, &layer);
			else
				su15_spawn_pixels_one(st, -1, new_key, pix, &w,
						      &h, &layer);
			memcpy(arc_sprite_pixels_mut(s, slot), pix,
			       (size_t)(st->ph * st->pw));
			s->w[slot] = w;
			s->h[slot] = h;
			s->layer[slot] = layer;
			int32_t nx, ny;
			su15_clamp_position(avg_cx[k] - w / 2,
					    avg_cy[k] - h / 2, w, h, &nx, &ny);
			s->x[slot] = nx;
			s->y[slot] = ny;
			...
			if (is_blob)
				...
				aux->fruit_kind[slot] = -1;
		}
	}
	if (any_member)
		*next_order = old_next_order + st->num_slots;
}

static void su15_check_fruit_merge(struct arc_sprites *s,
				   const struct su15_static *st, int32_t level,
				   struct su15_aux *aux, int32_t *next_order)
{
	const int32_t *fruits =
		st->fruit_slot + (size_t)level * SU15_MAX_FRUITS;
	...
	if (m >= 2)
		su15_merge_and_flash(s, st, aux, next_order, cand, m, 0);
}

static void su15_check_blob_merge(struct arc_sprites *s,
				  const struct su15_static *st, int32_t level,
				  struct su15_aux *aux, int32_t *next_order)
{
	...
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		...
		if (slot >= 0 && aux->pulled[slot] && !aux->consuming[slot] &&
		    s->alive[slot])
			cand[m++] = slot;
	}
	if (m >= 2)
		su15_merge_and_flash(s, st, aux, next_order, cand, m, 1);
}

static void su15_check_blob_merge_from_settle(struct arc_sprites *s,
					      const struct su15_static *st,
					      int32_t level,
					      struct su15_aux *aux,
					      int32_t *next_order)
...
static int su15_any_blob_in_move_phase(const struct su15_static *st,
				       int32_t level,
				       const struct su15_aux *aux)
{
	const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		...
		if (slot < 0)
			continue;
		if (!aux->consuming[slot] || aux->consume_dying[slot])
			continue;
		int32_t f = aux->consume_frame[slot];
		if (f >= SU15_WOBBLE_FRAMES && f < SU15_SWALLOW_TOTAL_FRAMES)
			return 1;
	}
	return 0;
}

static void su15_trigger_swallow(struct arc_sprites *s, struct su15_aux *aux,
				 int32_t fruit_slot, int32_t blob_slot)
{
	if (aux->consuming[blob_slot])
		return;
	int32_t tier = aux->tier[blob_slot];
	int dying = tier <= 0;
	int32_t target_tier = dying ? -1 : tier - 1;
	int32_t bcx = s->x[blob_slot] + s->w[blob_slot] / 2,
		bcy = s->y[blob_slot] + s->h[blob_slot] / 2;
	int32_t fcx = s->x[fruit_slot] + s->w[fruit_slot] / 2,
		fcy = s->y[fruit_slot] + s->h[fruit_slot] / 2;
...
static void su15_swallow_scan(struct arc_sprites *s,
			      const struct su15_static *st, int32_t level,
			      struct su15_aux *aux)
{
	const int32_t *fruits =
	...
	for (int32_t k = 0; k < SU15_MAX_FRUITS; k++) {
		int32_t f = fruits[k];
		eligible_fruit[k] =
			(uint8_t)(f >= 0 && s->alive[f] &&
				  !aux->near_click[f] && aux->eat_lock[f] <= 0);
	}
	uint8_t eligible_blob[SU15_MAX_BLOBS];
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		int32_t b = blobs[k];
		eligible_blob[k] =
			(uint8_t)(b >= 0 && s->alive[b] && !aux->consuming[b]);
	}

	for (int32_t fi = 0; fi < SU15_MAX_FRUITS; fi++) {
		if (!eligible_fruit[fi])
			continue;
		int32_t f = fruits[fi];
		for (int32_t bi = 0; bi < SU15_MAX_BLOBS; bi++) {
			...
			if (aux->consuming[b])
				continue;
			if (su15_overlap(s, f, b))
				su15_trigger_swallow(s, aux, f, b);
		}
	}
}

static void su15_move_fruits_toward_target(struct arc_sprites *s,
					   const struct su15_static *st,
					   int32_t level, struct su15_aux *aux)
{
	const int32_t *fruits =
		st->fruit_slot + (size_t)level * SU15_MAX_FRUITS;
	const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
	...
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		int32_t b = blobs[k];
		if (b >= 0 && s->alive[b] && !aux->consuming[b])
			avail[navail++] = b;
	}
	if (navail == 0)
		return;
	for (int32_t fi = 0; fi < SU15_MAX_FRUITS; fi++) {
		int32_t f = fruits[fi];
		if (f < 0 || !s->alive[f] || aux->near_click[f] ||
		    aux->eat_lock[f] > 0)
			continue;
		int32_t fcx = s->x[f] + s->w[f] / 2,
			fcy = s->y[f] + s->h[f] / 2;
		int32_t best = avail[0];
		int64_t bestd = -1;
		for (int32_t k = 0; k < navail; k++) {
			int32_t b = avail[k];
			int32_t bcx = s->x[b] + s->w[b] / 2,
				bcy = s->y[b] + s->h[b] / 2;
		...
		int32_t bcx = s->x[best] + s->w[best] / 2,
			bcy = s->y[best] + s->h[best] / 2;
		...
		int32_t nx, ny;
		su15_clamp_position(s->x[f] + stepx, s->y[f] + stepy, s->w[f],
				    s->h[f], &nx, &ny);
		s->x[f] = nx;
		s->y[f] = ny;
	}
...
static void su15_advance_swallow_animations(struct arc_sprites *s,
					    const struct su15_static *st,
					    int32_t level, struct su15_aux *aux)
{
	const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		int32_t i = blobs[k];
		if (i < 0 || !aux->consuming[i])
			continue;
		int dying = aux->consume_dying[i];
		int32_t total = dying ? SU15_SWALLOW_DYING_FRAMES :
					SU15_SWALLOW_TOTAL_FRAMES;
		...
		if (promote_now) {
			...
			int8_t pix[SU15_MAX_PATCH_AREA];
			int32_t w, h, layer;
			su15_spawn_pixels_one(st, aux->tier[i],
					      aux->fruit_kind[i], pix, &w, &h,
					      &layer);
			memcpy(arc_sprite_pixels_mut(s, i), pix,
			       (size_t)(st->ph * st->pw));
			s->w[i] = w;
			s->h[i] = h;
		}
		...
		int32_t bound = SU15_DELTA_BOUND;
		int32_t ix =
			su15_clampi(aux->consume_dx[i], -bound, bound) + bound;
		int32_t iy =
			su15_clampi(aux->consume_dy[i], -bound, bound) + bound;
		int32_t mult_idx =
			su15_clampi(frame - SU15_WOBBLE_FRAMES, 0, 3);
		size_t tbl = ((size_t)mult_idx * SU15_DELTA_SIZE + (size_t)ix) *
				     SU15_DELTA_SIZE +
			     (size_t)iy;
		...
		int32_t moved_x = su15_advance_axis(aux->consume_start_x[i],
						    floor_x, round_x, tie_x);
		int32_t moved_y = su15_advance_axis(aux->consume_start_y[i],
						    floor_y, round_y, tie_y);
		if (!in_wobble) {
			aux->consume_pos_x[i] = moved_x;
			aux->consume_pos_y[i] = moved_y;
		}

		int32_t raw_x = in_wobble ? aux->consume_start_x[i] :
					    aux->consume_pos_x[i];
		int32_t raw_y = in_wobble ?
					aux->consume_start_y[i] +
						su15_wobble_offset[frame % 4] :
					aux->consume_pos_y[i];
		int32_t nx, ny;
...
static void su15_settle_fruits(struct arc_sprites *s,
			       const struct su15_static *st, int32_t level,
			       struct su15_aux *aux, int32_t *next_order)
{
	const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
	...
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		int32_t b = blobs[k];
		if (b >= 0 && aux->consuming[b]) {
			any_consuming = 1;
			break;
		}
	}
	if (any_consuming)
		su15_advance_swallow_animations(s, st, level, aux);

	for (int32_t k = 0; k < SU15_MAX_FRUITS; k++) {
		int32_t f = fruits[k];
		if (f >= 0 && aux->eat_lock[f] > 0)
			aux->eat_lock[f]--;
	}
	...
	int gate = aux->pull_frame < SU15_PULL_FRAMES && any_fruit_exists &&
		   any_blob_alive && !frozen;
	if (gate) {
		su15_move_fruits_toward_target(s, st, level, aux);
		su15_swallow_scan(s, st, level, aux);
		su15_check_blob_merge_from_settle(s, st, level, aux,
						  next_order);
	}
}

static void su15_pull_move_one(struct arc_sprites *s,
			       const struct su15_static *st,
			       const struct su15_aux *aux, int32_t i)
{
	int32_t cx = s->x[i] + s->w[i] / 2, cy = s->y[i] + s->h[i] / 2;
	int32_t ddx =
		su15_clampi(aux->click_x - cx, -SU15_PULL_STEP, SU15_PULL_STEP);
	int32_t ddy =
		su15_clampi(aux->click_y - cy, -SU15_PULL_STEP, SU15_PULL_STEP);
	int32_t greedy_x = s->x[i] + ddx, greedy_y = s->y[i] + ddy;

	int32_t fx, rx, fy, ry;
	int tx, ty;
	su15_pull_lookup(st, aux->pull_dx[i], aux->pull_dy[i], &fx, &rx, &tx,
			 &fy, &ry, &ty);
...
static void su15_pull_step(struct arc_sprites *s, const struct su15_static *st,
			   int32_t level, const struct su15_aux *aux)
{
	...
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		...
		if (i < 0)
			continue;
		if (aux->pulled[i] && !aux->consuming[i])
			su15_pull_move_one(s, st, aux, i);
	}
	for (int32_t k = 0; k < SU15_MAX_FRUITS; k++) {
		int32_t i = fruits[k];
		if (i < 0)
			continue;
		if (aux->near_click[i] && !frozen)
			su15_pull_move_one(s, st, aux, i);
	}
}

static int su15_pkrdtzfrth(struct arc_sprites *s, const struct su15_static *st,
			   int32_t level, struct su15_aux *aux,
			   int32_t *next_order)
{
...
static int su15_win_check_point(const struct arc_sprites *s,
				const struct su15_static *st, int32_t level,
				const struct su15_aux *aux)
{
	...
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		...
		for (int32_t z = 0; z < SU15_MAX_ZONES; z++) {
			...
			if (zi < 0)
				continue;
			if (cx >= s->x[zi] && cx < s->x[zi] + s->w[zi] &&
			    cy >= s->y[zi] && cy < s->y[zi] + s->h[zi]) {
				in_zone = 1;
				break;
			}
		}
	...
	for (int32_t k = 0; k < SU15_MAX_FRUITS; k++) {
		...
		if (kd >= 0 && kd < SU15_NUM_FRUIT_KINDS)
			kind_counts[kd]++;
	}

	const int32_t *wtype =
		st->win_type + (size_t)level * SU15_MAX_WIN_TERMS;
	const int32_t *wvalue =
		st->win_value + (size_t)level * SU15_MAX_WIN_TERMS;
	const int32_t *wcount =
		st->win_count + (size_t)level * SU15_MAX_WIN_TERMS;
	for (int32_t k = 0; k < SU15_MAX_WIN_TERMS; k++) {
		if (wcount[k] < 0)
			continue;
		int32_t actual = wtype[k] == 0 ? tier_counts[wvalue[k]] :
						 kind_counts[wvalue[k]];
		if (actual != wcount[k])
			return 0;
	}
	return 1;
...
static int su15_win_check_weighted(const struct arc_sprites *s,
				   const struct su15_static *st, int32_t level,
				   const struct su15_aux *aux)
{
	const int32_t *blobs = st->blob_slot + (size_t)level * SU15_MAX_BLOBS;
	...
	int32_t needed = 0;
	for (int32_t k = 0; k < SU15_MAX_WIN_TERMS; k++) {
		if (wcount[k] >= 0 && wtype[k] == 0)
			needed += ((int32_t)1 << wvalue[k]) * wcount[k];
	}
	return actual_sum >= needed;
}

static void su15_paint_zone_pieces(struct arc_sprites *s,
				   const struct su15_static *st, int32_t level,
				   int32_t color)
{
	...
	for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
		int32_t z = za[k];
		if (z >= 0)
			arc_color_remap(s, z, 0, 0, (int8_t)color);
	}
	for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
		int32_t z = zb[k];
		if (z >= 0)
...
static void su15_restore_zone_pixels(struct arc_sprites *s,
				     const struct su15_static *st,
				     int32_t level)
{
	int32_t area = st->ph * st->pw;
	...
	for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
		...
		if (z < 0)
			continue;
		memcpy(arc_sprite_pixels_mut(s, z),
		       st->level_pixels +
			       ((size_t)level * st->num_slots + z) * area,
		       (size_t)area);
	}
	for (int32_t k = 0; k < SU15_MAX_ZONES; k++) {
		int32_t z = zb[k];
...
static void su15_push_undo(struct arc_sprites *s, const struct su15_static *st,
			   struct su15_aux *aux)
{
...
static void su15_pop_undo(struct arc_sprites *s, const struct su15_static *st,
			  int32_t level, struct su15_aux *aux,
			  int32_t *next_order)
{
	if (aux->undo_top <= 0)
	...
	for (int32_t k = 0; k < nb; k++) {
		...
		if (ua[slot]) {
			...
			for (int32_t m = 0; m < nb; m++)
				if (ua[bslot[m]] && uo[bslot[m]] < uo[slot])
					r++;
			rank = r;
		} else {
			int32_t before = 0;
			for (int32_t m = 0; m < nb; m++)
				if (ua[bslot[m]] && bslot[m] < slot)
					before++;
			rank = num_blobs_alive + slot - before;
		}
		int8_t pix[SU15_MAX_PATCH_AREA];
		int32_t w, h, layer;
		su15_spawn_pixels_one(st, ut[slot], uf[slot], pix, &w, &h,
				      &layer);
		memcpy(arc_sprite_pixels_mut(s, slot), pix,
		       (size_t)(st->ph * st->pw));
		s->w[slot] = w;
	...
	for (int32_t k = 0; k < nf; k++) {
		...
		if (ua[slot]) {
			...
			for (int32_t m = 0; m < nf; m++)
				if (ua[fslot[m]] && uo[fslot[m]] < uo[slot])
					r++;
			rank = r;
		} else {
			int32_t before = 0;
			for (int32_t m = 0; m < nf; m++)
				if (ua[fslot[m]] && fslot[m] < slot)
					before++;
	...
	*next_order = old_next_order + n;
	...
	int should_clear = aux->not_yet_flash &&
			   su15_win_check_weighted(s, st, level, aux);
	if (should_clear) {
...
static void su15_handle_tutorial_hide_if_moved(struct arc_sprites *s,
					       const struct su15_static *st,
					       int32_t level,
					       struct su15_aux *aux)
{
...
static void su15_hide_tutorial_arrow_if_clicked(struct arc_sprites *s,
						const struct su15_static *st,
						int32_t level, int32_t wx,
						int32_t wy)
{
	if (level != 0)
		return;
	...
	if (slot < 0)
		return;
	int hit = wx >= s->x[slot] && wx < s->x[slot] + s->w[slot] &&
		  wy >= s->y[slot] && wy < s->y[slot] + s->h[slot];
	if (hit)
		s->x[slot] = 500;
}

static void su15_begin_pull(struct arc_sprites *s, const struct su15_static *st,
			    int32_t level, struct su15_aux *aux, int32_t gx,
			    int32_t gy)
{
	...
	for (int32_t k = 0; k < SU15_MAX_BLOBS; k++) {
		...
		if (i < 0 || !s->alive[i])
			continue;
		int near =
			su15_within_radius(s->x[i], s->y[i], s->w[i], s->h[i],
					   gx, gy, SU15_CLICK_RADIUS);
...
static void su15_advance_pulling(struct arc_sprites *s,
				 const struct su15_static *st, int32_t level,
				 struct su15_aux *aux, int32_t *next_order,
...
static void su15_advance_win_flash(struct arc_sprites *s,
				   const struct su15_static *st, int32_t level,
				   struct su15_aux *aux, int32_t *score,
...
static void su15_end_mismatch_flash(struct arc_sprites *s,
				    const struct su15_static *st, int32_t level,
				    struct su15_aux *aux, int32_t *next_order,
				    int32_t *status, uint8_t *action_complete)
{
...
static void su15_advance_mismatch_flash(struct arc_sprites *s,
					const struct su15_static *st,
					int32_t level, struct su15_aux *aux,
					...
					uint8_t *action_complete)
{
	...
	if (next_frame >= 16)
		su15_end_mismatch_flash(s, st, level, aux, next_order, status,
					action_complete);
}

static void su15_handle_idle_action(struct arc_sprites *s,
				    const struct arc_camera *camera,
				    const struct su15_static *st, int32_t level,
				    int32_t action_id, int32_t action_x,
				    int32_t action_y, struct su15_aux *aux,
				    int32_t *next_order,
				    uint8_t *action_complete)
{
	if (action_id == SU15_ACTION6) {
		int32_t wx, wy;
		int valid;
		su15_display_to_grid(camera, action_x, action_y, &wx, &wy,
				     &valid);
		if (valid) {
			su15_push_undo(s, st, aux);
			su15_hide_tutorial_arrow_if_clicked(s, st, level, wx,
							    wy);
			su15_begin_pull(s, st, level, aux, wx, wy);
		}
		if (aux->phase != SU15_PHASE_PULLING)
			*action_complete = 1;
		    ...
		    uint8_t *action_complete)
{
	...
	if (phase == SU15_PHASE_IDLE) {
		su15_handle_idle_action(sprites, camera, st, level, action_id,
					action_x, action_y, aux, next_order,
					action_complete);
	} else if (phase == SU15_PHASE_WIN_FLASH) {
		su15_advance_win_flash(sprites, st, level, aux, score, status,
				       next_level, action_complete);
	} else if (phase == SU15_PHASE_MISMATCH_FLASH) {
		su15_advance_mismatch_flash(sprites, st, level, aux, next_order,
					    status, action_complete);
	} else {
		su15_advance_pulling(sprites, st, level, aux, next_order,
				     status, action_complete);
	}
}
