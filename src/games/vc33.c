...
static inline size_t vc33_ln(const struct vc33_static *st, int32_t level,
			     int32_t slot)
{
	return (size_t)level * (size_t)st->num_slots + (size_t)slot;
}

static inline size_t vc33_lnn(const struct vc33_static *st, int32_t level,
			      int32_t a, int32_t b)
{
...
static inline int vc33_horiz(const struct vc33_static *st, int32_t level)
{
	return st->grav_x[level] != 0;
}

static inline int vc33_positive(const struct vc33_static *st, int32_t level)
{
	return st->grav_x[level] > 0 || st->grav_y[level] > 0;
}

static inline int32_t vc33_signed(const struct vc33_static *st, int32_t level)
{
	return vc33_horiz(st, level) ? st->grav_x[level] : st->grav_y[level];
}

static inline int32_t vc33_along(const struct arc_sprites *s,
...
static inline int32_t vc33_along_size(const struct arc_sprites *s,
				      const struct vc33_static *st,
				      int32_t level, int32_t idx)
{
	return vc33_horiz(st, level) ? s->w[idx] : s->h[idx];
}

static inline int32_t vc33_perp(const struct arc_sprites *s,
				const struct vc33_static *st, int32_t level,
				int32_t idx)
{
...
static inline int32_t vc33_perp_size(const struct arc_sprites *s,
				     const struct vc33_static *st,
				     int32_t level, int32_t idx)
{
...
static inline int32_t vc33_front(const struct arc_sprites *s,
				 const struct vc33_static *st, int32_t level,
				 int32_t idx)
{
...
static inline int32_t vc33_back(const struct arc_sprites *s,
				const struct vc33_static *st, int32_t level,
				int32_t idx)
{
	int32_t a = vc33_along(s, st, level, idx);
...
static int vc33_attached(const struct arc_sprites *s,
			 const struct vc33_static *st, int32_t level,
			 int32_t sensor_idx, int32_t pipe_idx)
{
	...
	return perp_s >= perp_p && perp_s < perp_p + perp_size_p &&
	       front_s == back_p;
}

static int32_t vc33_first_pipe(const struct arc_sprites *s,
			       const struct vc33_static *st, int32_t level,
			       int32_t coupler_idx, int side)
{
	...
	for (int32_t k = 0; k < count; k++) {
		int32_t p = slots[k];
		if (vc33_back(s, st, level, p) != front_c)
			continue;
		int32_t perp_p = vc33_perp(s, st, level, p);
		int32_t perp_size_p = vc33_perp_size(s, st, level, p);
		int match = side == VC33_SIDE_LEFT ?
				    (perp_p + perp_size_p == perp_c) :
...
static int vc33_coupler_active(const struct arc_sprites *s,
			       const struct vc33_static *st, int32_t level,
			       int32_t coupler_idx)
{
	return vc33_first_pipe(s, st, level, coupler_idx, VC33_SIDE_LEFT) >=
		       0 &&
	       vc33_first_pipe(s, st, level, coupler_idx, VC33_SIDE_RIGHT) >= 0;
}

static int32_t vc33_growth_cap(const struct arc_sprites *s,
			       const struct vc33_static *st, int32_t level,
			       int32_t pipe_idx)
{
	int has_sensor = 0;
	...
	const int32_t *slots =
		st->sensor_slots + (size_t)level * st->max_sensors;
	for (int32_t k = 0; k < count && !has_sensor; k++) {
		if (vc33_attached(s, st, level, slots[k], pipe_idx))
			has_sensor = 1;
	}
	size_t idx = vc33_ln(st, level, pipe_idx);
	int32_t floor_val = st->pipe_floor_max_front[idx] -
			    (has_sensor ? st->sensor_adjust[level] : 0);
	int32_t wall_val = st->pipe_wall_extreme[idx];
	return st->pipe_uses_floor[idx] ? floor_val : wall_val;
}

static void vc33_write_pipe(struct arc_sprites *s, int32_t idx, int32_t x,
			    int32_t y, int32_t w, int32_t h)
{
	...
	int8_t *patch = arc_sprite_pixels_mut(s, idx);
	for (int32_t r = 0; r < ph; r++) {
		for (int32_t c = 0; c < pw; c++) {
			patch[(size_t)r * pw + c] =
				(r < h && c < w) ? (int8_t)0 : (int8_t)-1;
		}
	}
}

static void vc33_refresh_couplers(struct arc_sprites *s,
				  const struct vc33_static *st, int32_t level,
				  int32_t *next_order)
{
	for (int32_t k = 0; k < VC33_MAX_COUPLERS; k++)
		arc_remove_sprite(s, st->icon_base + k);
	for (int32_t k = 0; k < VC33_MAX_COUPLERS; k++) {
		int32_t coupler_slot =
			st->level_coupler_slot[vc33_lc(level, k)];
		if (coupler_slot < 0)
			continue;
		int active = vc33_coupler_active(s, st, level, coupler_slot);
		int8_t new_color = active ? VC33_COUPLER_ACTIVE_COLOR :
					    VC33_COUPLER_INACTIVE_COLOR;
		arc_color_remap(s, coupler_slot, 0, 0, new_color);
		if (active) {
			int32_t icon_slot = st->icon_base + k;
			int32_t icon_x = s->x[coupler_slot] +
					 st->icon_dx[vc33_lc(level, k)];
			int32_t icon_y = s->y[coupler_slot] +
					 st->icon_dy[vc33_lc(level, k)];
			arc_set_position(s, icon_slot, icon_x, icon_y);
			arc_set_interaction(s, icon_slot, TANGIBLE);
...
static void vc33_resize(struct arc_sprites *s, const struct vc33_static *st,
			int32_t level, int32_t lever_idx, int32_t *next_order)
{
	...
	int32_t gx = st->grav_x[level], gy = st->grav_y[level],
		g = vc33_signed(st, level);
	int32_t scount = st->sensor_count[level];
	const int32_t *sslots =
		st->sensor_slots + (size_t)level * st->max_sensors;
	...
	int32_t a_src = vc33_along(s, st, level, src),
		s_src = vc33_along_size(s, st, level, src);
	int32_t new_a_src = a_src + (g > 0 ? g : 0),
		new_s_src = s_src - (g < 0 ? -g : g);
	vc33_write_pipe(s, src, horiz ? new_a_src : s->x[src],
			horiz ? s->y[src] : new_a_src,
			horiz ? new_s_src : s->w[src],
			horiz ? s->h[src] : new_s_src);

	int32_t a_dst = vc33_along(s, st, level, dst),
		s_dst = vc33_along_size(s, st, level, dst);
	int32_t new_a_dst = a_dst - (g > 0 ? g : 0),
		new_s_dst = s_dst + (g < 0 ? -g : g);
	vc33_write_pipe(s, dst, horiz ? new_a_dst : s->x[dst],
			horiz ? s->y[dst] : new_a_dst,
			horiz ? new_s_dst : s->w[dst],
			horiz ? s->h[dst] : new_s_dst);

	vc33_refresh_couplers(s, st, level, next_order);
}

static void vc33_sensor_target(const struct arc_sprites *s,
			       const struct vc33_static *st, int32_t level,
			       int32_t i, int32_t other_perp,
			       int32_t other_perp_size, int32_t *tx,
			       int32_t *ty)
{
...
static void vc33_add_pass(const struct arc_sprites *s,
			  const struct vc33_static *st, int32_t level,
			  int32_t pipe_c, int32_t other_perp,
			  int32_t other_perp_size, int32_t *slot_arr,
			  int32_t *tx_arr, int32_t *ty_arr, int32_t *ptr)
{
	int32_t count = st->sensor_count[level];
	const int32_t *slots =
		st->sensor_slots + (size_t)level * st->max_sensors;
	for (int32_t k = 0; k < count; k++) {
		int32_t i = slots[k];
		if (!vc33_attached(s, st, level, i, pipe_c))
			continue;
		int32_t tx, ty;
		vc33_sensor_target(s, st, level, i, other_perp, other_perp_size,
				   &tx, &ty);
		int32_t p = *ptr;
		if (p < 0)
			p = 0;
		if (p > VC33_MAX_QUEUE - 1)
			p = VC33_MAX_QUEUE - 1;
		slot_arr[p] = i;
		tx_arr[p] = tx;
		ty_arr[p] = ty;
...
static void vc33_build_queue(const struct arc_sprites *s,
			     const struct vc33_static *st, int32_t level,
			     int32_t coupler_idx, int32_t *slot_arr,
			     int32_t *tx_arr, int32_t *ty_arr,
			     int32_t *queue_len)
{
	int32_t left_pipe =
		vc33_first_pipe(s, st, level, coupler_idx, VC33_SIDE_LEFT);
	int32_t right_pipe =
		vc33_first_pipe(s, st, level, coupler_idx, VC33_SIDE_RIGHT);
	...
	int32_t ptr = 1;
	vc33_add_pass(s, st, level, left_pipe_c, rp_perp, rp_perp_size,
		      slot_arr, tx_arr, ty_arr, &ptr);
	vc33_add_pass(s, st, level, right_pipe_c, lp_perp, lp_perp_size,
		      slot_arr, tx_arr, ty_arr, &ptr);
...
static int vc33_win_check(const struct arc_sprites *s,
			  const struct vc33_static *st, int32_t level)
{
	int32_t scount = st->sensor_count[level];
	const int32_t *sslots =
	...
	const int32_t *mslots =
		st->marker_slots + (size_t)level * st->max_markers;
	...
	for (int32_t si = 0; si < scount; si++) {
		...
		for (int32_t mi = 0; mi < mcount && !ok; mi++) {
			...
			int color_ok = st->sensor_marker_color[vc33_lnn(
				st, level, sensor, marker)];
			int32_t wall_col = st->marker_wall[midx];
			if (wall_col < 0)
				wall_col = 0;
			int wall_ok = st->wall_touch_mask[vc33_lnn(
				st, level, mp, wall_col)];
			if (along_ok && color_ok && wall_ok)
				ok = 1;
		}
		if (!ok)
			return 0;
...
static void vc33_resolve(const struct arc_sprites *s,
			 const struct vc33_static *st, int32_t level,
			 const struct vc33_aux *aux, int32_t *score,
...
static void vc33_display_to_grid(const struct arc_camera *camera,
...
static void vc33_handle_action(struct arc_sprites *s,
			       const struct arc_camera *camera,
			       const struct vc33_static *st, int32_t level,
			       int32_t action_id, int32_t action_x,
			       int32_t action_y, struct vc33_aux *aux,
			       ...
			       uint8_t *action_complete)
{
	if (action_id == VC33_ACTION6) {
		...
		int32_t wx, wy;
		int on_board;
		vc33_display_to_grid(camera, action_x, action_y, &wx, &wy,
				     &on_board);
		int32_t hit =
			on_board ? arc_get_sprite_at(s, wx, wy, -1, 0) : -1;
		int32_t hc = hit < 0 ? 0 : hit;
		int is_lever =
			hit >= 0 && st->lever_mask[vc33_ln(st, level, hc)];
		int is_coupler =
			hit >= 0 && st->coupler_mask[vc33_ln(st, level, hc)];

		if (is_lever) {
			vc33_resize(s, st, level, hc, next_order);
		} else if (is_coupler) {
			if (vc33_coupler_active(s, st, level, hc)) {
				int32_t slot_arr[VC33_MAX_QUEUE],
					tx_arr[VC33_MAX_QUEUE],
					ty_arr[VC33_MAX_QUEUE], qlen;
				vc33_build_queue(s, st, level, hc, slot_arr,
						 tx_arr, ty_arr, &qlen);
				for (int32_t k = 0; k < VC33_MAX_COUPLERS;
				     k++) {
					int32_t slot = st->icon_base + k;
					if (s->alive[slot])
						arc_set_visible(s, slot, 0);
				}
				for (int32_t k = 0; k < VC33_MAX_QUEUE; k++) {
	...
	if (aux->queue_len > 0)
		return;
	vc33_resolve(s, st, level, aux, score, status, next_level,
		     action_complete);
}

static void vc33_continue_queue(struct arc_sprites *s,
				const struct vc33_static *st, int32_t level,
				struct vc33_aux *aux, int32_t *next_order,
				...
				uint8_t *next_level, uint8_t *action_complete)
{
	...
	if (dx == 0 && dy == 0) {
		...
		if (new_idx >= aux->queue_len) {
			...
			aux->queue_len = 0;
			vc33_refresh_couplers(s, st, level, next_order);
			vc33_resolve(s, st, level, aux, score, status,
		...
		int use_dx = adx >= ady;
		int32_t nx =
			cx + (use_dx ? (dx > 0 ? 1 : (dx < 0 ? -1 : 0)) : 0);
		int32_t ny =
			cy + (use_dx ? 0 : (dy > 0 ? 1 : (dy < 0 ? -1 : 0)));
...
{
	if (aux->queue_len > 0) {
		vc33_continue_queue(sprites, st, level, aux, next_order, score,
				    status, next_level, action_complete);
	} else {
		vc33_handle_action(sprites, camera, st, level, action_id,
			   ...
			   int32_t level, const struct vc33_aux *aux)
{
	...
	for (int32_t c = 0; c < ARC_FRAME_SIZE; c++) {
		frame[c] = (int8_t)(c < filled ? VC33_BUDGET_FILLED_COLOR :
						 VC33_BUDGET_EMPTY_COLOR);
	}
}
