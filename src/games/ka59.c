...
static inline size_t ka59_area(const struct ka59_static *st)
{
	return (size_t)st->ph * (size_t)st->pw;
}

static void ka59_recolor_masked(int8_t *patch, const uint8_t *mask,
				int8_t colour, const int32_t *box, int32_t pw)
{
...
static void ka59_compute_box_bbox(struct ka59_aux *aux,
...
static void ka59_display_to_grid(const struct arc_camera *camera,
...
static int ka59_push_chain(struct arc_sprites *s, const struct ka59_static *st,
			   ...
			   int32_t depth)
{
	...
	const int32_t *border =
		st->border_slots + (size_t)level * st->max_border;
	...
	const int32_t *occ =
		st->occupant_slots + (size_t)level * st->max_occupant;
	...
	for (int32_t k = 0; k < occ_n; k++) {
		if (arc_collides_pair(s, slot, occ[k], 0))
			hit_slots[hit_n++] = occ[k];
	}

	int any_blocked = 0;
	for (int32_t k = 0; k < hit_n; k++) {
		if (ka59_push_chain(s, st, level, hit_slots[k], dx, dy,
				    depth - 1)) {
			any_blocked = 1;
			break;
		}
...
static int ka59_strip_touches(struct arc_sprites *s,
			      const struct ka59_static *st, int32_t level,
			      int32_t x0, int32_t y0, int32_t width,
			      int32_t height)
{
	...
	int8_t save_interaction = s->interaction[slot],
	       save_blocking = s->blocking[slot];
...
static void ka59_refresh_borders(struct arc_sprites *s,
				 const struct ka59_static *st, int32_t level,
				 const struct ka59_aux *aux)
{
	...
	for (int32_t k = 0; k < box_n; k++) {
		...
		const uint8_t *mask =
			st->box_outline_mask +
			((size_t)level * st->num_slots + slot) * area;
		ka59_recolor_masked(patch, mask, KA59_HIGHLIGHT,
				    aux->box_bbox + slot * 4, pw);
	}
	...
	int touched[4];
	touched[0] =
		ka59_strip_touches(s, st, level, box_x, box_y - 1, box_w, 1);
	touched[1] = ka59_strip_touches(s, st, level, box_x, box_y + box_h,
					box_w, 1);
	touched[2] =
		ka59_strip_touches(s, st, level, box_x - 1, box_y, 1, box_h);
	touched[3] = ka59_strip_touches(s, st, level, box_x + box_w, box_y, 1,
					box_h);
...
static int ka59_targets_satisfied(const struct arc_sprites *s,
				  const int32_t *container, int32_t container_n,
...
static int ka59_is_won(const struct arc_sprites *s,
		       const struct ka59_static *st, int32_t level)
{
			   ...
			   int32_t level, struct ka59_aux *aux)
{
	...
	for (int32_t k = 0; k < bomb_n; k++) {
		...
		int32_t total_bad = aux->fuse_first_cycle[b] ?
					    st->fuse_total_bad[idx] :
					    s->h[b];
		int last_row_bad = aux->fuse_first_cycle[b] ?
					   (int)st->fuse_last_row_bad[idx] :
					   1;
		...
		if (p_index < 0)
			p_index = 0;
		if (p_index > st->max_bomb_h)
			p_index = st->max_bomb_h;

		const int8_t *frame_src =
			st->fuse_frames +
			(((idx * 2 + (size_t)cycle) *
				  (size_t)(st->max_bomb_h + 1) +
			  (size_t)p_index)) *
				area;
		...
		aux->fuse_first_cycle[b] =
			(uint8_t)(aux->fuse_first_cycle[b] && !completes);
...
static void ka59_tail_common(struct arc_sprites *s,
			     const struct ka59_static *st, int32_t level,
			     struct ka59_aux *aux, int32_t *score,
...
static void ka59_tail_with_tick(struct arc_sprites *s,
				const struct ka59_static *st, int32_t level,
				struct ka59_aux *aux, int32_t *score,
				int32_t *status, uint8_t *next_level,
				uint8_t *action_complete)
{
	...
	ka59_tail_common(s, st, level, aux, score, status, next_level,
			 action_complete);
}

static void ka59_select_box(struct arc_sprites *s, const struct ka59_static *st,
			    int32_t level, struct ka59_aux *aux,
			    int32_t new_slot)
{
	size_t area = ka59_area(st);
	...
	const uint8_t *old_mask =
		st->center_dot_mask +
		((size_t)level * st->num_slots + old_slot) * area;
	ka59_recolor_masked(old_patch, old_mask, KA59_DESELECTED_DOT,
			    aux->box_bbox + old_slot * 4, pw);

	int8_t *new_patch = arc_sprite_pixels_mut(s, new_slot);
	const uint8_t *new_mask =
		st->center_dot_mask +
		((size_t)level * st->num_slots + new_slot) * area;
	ka59_recolor_masked(new_patch, new_mask, KA59_SELECTED_DOT,
			    aux->box_bbox + new_slot * 4, pw);

	aux->active_box = new_slot;
}

static void ka59_handle_click(struct arc_sprites *s,
			      ...
			      uint8_t *action_complete)
{
	...
	if (on_board && hit >= 0)
		ka59_select_box(s, st, level, aux, hit);

	ka59_tail_common(s, st, level, aux, score, status, next_level,
			 action_complete);
}

static int ka59_probe_move(struct arc_sprites *s, const struct ka59_static *st,
			   int32_t level, int32_t slot, int32_t dx, int32_t dy,
			   struct ka59_aux *aux)
{
	s->x[slot] += dx;
	...
	for (int32_t k = 0; k < wall_n && !hard_hit; k++)
		if (arc_collides_pair(s, slot, wall[k], 0))
			hard_hit = 1;
	const int32_t *border =
		st->border_slots + (size_t)level * st->max_border;
	int32_t border_n = st->border_count[level];
...
static void ka59_handle_move(struct arc_sprites *s,
			     const struct ka59_static *st, int32_t level,
			     int32_t dir_idx, struct ka59_aux *aux,
			     ...
			     uint8_t *action_complete)
{
	...
	if (any_pushable) {
		...
		const uint8_t *edge =
			st->box_edge_masks +
			((((size_t)level * st->num_slots + active) * 4) +
			 (size_t)dir_idx) *
				area;
		ka59_recolor_masked(patch, edge, KA59_HIGHLIGHT,
				    aux->box_bbox + active * 4, st->pw);
		...
		for (int32_t row = 0; row < ph; row++) {
			for (int32_t col = 0; col < pw; col++) {
				...
				case 2:
					...
					break;
				default:
					strip = col == cw - 1 && row >= 1 &&
						row < 1 + box_h;
					break;
				}
				cpatch[(size_t)row * pw + col] =
					(int8_t)(strip ? 0 : -1);
			}
		}
		...
		ka59_tail_with_tick(s, st, level, aux, score, status,
				    next_level, action_complete);
	}
}

static int ka59_retry_push_frame(struct arc_sprites *s,
				 const struct ka59_static *st, int32_t level,
				 struct ka59_aux *aux)
{
	const int32_t *occ =
	...
	for (int32_t k = 0; k < occ_n; k++) {
		int32_t i = occ[k];
		if (!aux->push_active[i])
			continue;

		int wall_hit = 0;
		const int32_t *wall =
			st->wall_slots + (size_t)level * st->max_wall;
		int32_t wall_n = st->wall_count[level];
		for (int32_t w = 0; w < wall_n && !wall_hit; w++)
			if (arc_collides_pair(s, i, wall[w], 0))
				wall_hit = 1;

		int settle_now =
			(aux->retry_frame >= KA59_RETRY_THRESHOLD) && !wall_hit;
		int result;
		if (settle_now) {
			result = 1;
		} else {
			result = ka59_push_chain(
				s, st, level, i, aux->recoil_dx[i],
				aux->recoil_dy[i], KA59_MAX_PUSH_DEPTH);
		}
...
static void ka59_push_retry_dispatch(struct arc_sprites *s,
				     const struct ka59_static *st,
				     int32_t level, struct ka59_aux *aux,
...
static void ka59_spawn_explosion_frame(struct arc_sprites *s,
				       const struct ka59_static *st,
				       int32_t level, struct ka59_aux *aux,
				       int32_t *next_order)
{
	...
	for (int32_t bi = 0; bi < bomb_n; bi++) {
		...
		const int8_t *pix =
			st->explosion_pixels + (idx * 3 + (size_t)frame) * area;
		...
		int32_t dx = st->explosion_recoil_dx[idx],
			dy = st->explosion_recoil_dy[idx];
		for (int32_t oi = 0; oi < occ_n; oi++) {
			int32_t o = occ[oi];
			if (!arc_collides_pair(s, ex_slot, o, 0))
				continue;
			ka59_push_chain(s, st, level, o, dx, dy,
					KA59_MAX_PUSH_DEPTH);
			aux->recoil_dx[o] = dx;
			aux->recoil_dy[o] = dy;
			aux->has_recoil[o] = 1;
...
static void ka59_finalize_explosion(struct arc_sprites *s,
				    const struct ka59_static *st, int32_t level,
				    struct ka59_aux *aux, int32_t *score,
				    int32_t *status, uint8_t *next_level,
				    uint8_t *action_complete)
{
	...
	for (int32_t bi = 0; bi < bomb_n; bi++) {
		...
		const int8_t *spent =
			st->fuse_frames +
			((idx * 2 + 1) * (size_t)(st->max_bomb_h + 1) + 0) *
				area;
		int8_t *dst = arc_sprite_pixels_mut(s, b);
	...
	for (int32_t oi = 0; oi < occ_n; oi++) {
		...
		int32_t wall_n = st->wall_count[level];
		for (int32_t w = 0; w < wall_n && !wall_hit; w++)
			if (arc_collides_pair(s, o, wall[w], 0))
				wall_hit = 1;
		if (!wall_hit)
			continue;

		int blocked =
			ka59_push_chain(s, st, level, o, aux->recoil_dx[o],
					aux->recoil_dy[o], KA59_MAX_PUSH_DEPTH);
		if (!blocked)
			any_retry = 1;
	}
	...
	for (int32_t bi = 0; bi < bomb_n; bi++)
		aux->exploding[bombs[bi]] = 0;
	ka59_tail_common(s, st, level, aux, score, status, next_level,
			 action_complete);
}

static void ka59_explosion_dispatch(struct arc_sprites *s,
				    const struct ka59_static *st, int32_t level,
				    ...
				    uint8_t *action_complete)
{
	if (aux->retry_frame < 3) {
		...
		ka59_finalize_explosion(s, st, level, aux, score, status,
					next_level, action_complete);
	}
}

static void ka59_dispatch_action(struct arc_sprites *s,
				 const struct arc_camera *camera,
				 const struct ka59_static *st, int32_t level,
				 int32_t action_id, int32_t action_x,
				 int32_t action_y, struct ka59_aux *aux,
				 int32_t *next_order, int32_t *score,
				 int32_t *status, uint8_t *next_level,
				 uint8_t *action_complete)
{
	switch (action_id) {
	case KA59_ACTION1:
		ka59_handle_move(s, st, level, 0, aux, next_order, score,
				 status, next_level, action_complete);
		break;
	case KA59_ACTION2:
		ka59_handle_move(s, st, level, 1, aux, next_order, score,
				 status, next_level, action_complete);
		break;
	case KA59_ACTION3:
		ka59_handle_move(s, st, level, 2, aux, next_order, score,
				 status, next_level, action_complete);
		break;
	case KA59_ACTION4:
		ka59_handle_move(s, st, level, 3, aux, next_order, score,
				 status, next_level, action_complete);
		break;
	case KA59_ACTION6:
		ka59_handle_click(s, camera, st, level, action_x, action_y, aux,
		       ...
		       struct ka59_aux *aux, int32_t *next_order)
{
	*next_order = st->num_slots;
	...
	const uint8_t *mask = st->center_dot_mask +
			      ((size_t)level * st->num_slots + active) * area;
	ka59_recolor_masked(patch, mask, KA59_SELECTED_DOT,
			    aux->box_bbox + active * 4, st->pw);
		    ...
		    uint8_t *next_level, uint8_t *action_complete)
{
	...
	for (int32_t k = 0; k < occ_n && !any_push; k++)
		if (aux->push_active[occ[k]])
			any_push = 1;

	if (any_push) {
		ka59_push_retry_dispatch(sprites, st, level, aux, score, status,
					 next_level, action_complete);
		return;
	}
	...
	for (int32_t k = 0; k < bomb_n && !any_exploding; k++)
		if (aux->exploding[bombs[k]])
			any_exploding = 1;

	if (any_exploding) {
		ka59_explosion_dispatch(sprites, st, level, aux, next_order,
					score, status, next_level,
					action_complete);
		return;
	}

	ka59_dispatch_action(sprites, camera, st, level, action_id, action_x,
			     action_y, aux, next_order, score, status,
			     next_level, action_complete);
}
