...
{
	struct arc_scene_game *game = calloc(1, sizeof(struct arc_scene_game));
	game->atlas = *atlas;
	game->hooks = hooks;
	game->aux = aux;
...
{
	game->engine.status = SCENE_GAME_OVER;
}
...
void arc_scene_game_set_level(struct arc_scene_game *game, int32_t index)
{
	...
	if (game->hooks->build_level)
		game->hooks->build_level(game);
}

static void scene_full_reset(struct arc_scene_game *game)
{
	game->engine.score = 0;
...
static void scene_level_reset(struct arc_scene_game *game)
{
	arc_scene_game_set_level(game, game->engine.level_index);
	game->engine.status = SCENE_NOT_FINISHED;
}

static void scene_handle_reset(struct arc_scene_game *game)
{
...
static void scene_advance_level(struct arc_scene_game *game)
{
			     ...
			     int32_t max_out)
{
	...
	while (!(!e->next_level && e->action_complete) &&
		...
		arc_scene_display(&game->scene, &game->atlas, &game->scratch,
				  game->background);
		if (frames && count < max_out)
			arc_scene_game_frame(
				game, frames + (size_t)count * ARC_FRAME_SIZE *
						       ARC_FRAME_SIZE);
		count++;
	}
	return count;
...
{
	return scene_perform(game, action_id, action_x, action_y, frames,
			     max_out);
}
...
{
	memset(&game->engine, 0, sizeof(struct arc_scene_engine_state));
...
int32_t arc_scene_game_step(struct arc_scene_game *game, int32_t action,
			    int8_t *frame, int32_t *reward, uint8_t *terminated)
{
	...
	*terminated = game->engine.status == SCENE_WIN ||
		      game->engine.status == SCENE_GAME_OVER;
	if (frame)
		arc_scene_game_frame(game, frame);
	return count;
}
