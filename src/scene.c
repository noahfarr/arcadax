...
void arc_scene_table_alloc(ArcSceneTable *scene, int32_t n) {
    ...
    scene->frame = malloc((size_t)ARC_FRAME_SIZE * ARC_FRAME_SIZE);
    scene->dirty = 1;
}
...
void arc_scene_table_clear(ArcSceneTable *scene, int8_t background) {
    ...
    memset(scene->frame, background, (size_t)ARC_FRAME_SIZE * ARC_FRAME_SIZE);
    scene->dirty = 1;
}

void arc_scene_touch(ArcSceneTable *scene) { scene->dirty = 1; }

static inline int64_t draw_key(const ArcSceneTable *scene, int32_t i) {
...
void arc_scene_composite(const ArcSceneTable *scene, const ArcSceneAtlas *atlas,
    ...
    for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
        memcpy(out + (size_t)r * ARC_FRAME_SIZE, canvas + (size_t)(ph + r) * cw + pw,
              ARC_FRAME_SIZE);
}

void arc_scene_display(ArcSceneTable *scene, const ArcSceneAtlas *atlas, ArcSceneScratch *scratch,
                   int8_t background) {
    if (scene->dirty) arc_scene_composite(scene, atlas, scratch, background, scene->frame);
    scene->dirty = 0;
}
