...
static inline int64_t draw_key(const struct arc_scene_table *scene, int32_t i)
{
	if (scene->image[i] < 0)
		return (int64_t)1 << 40;
			 ...
			 int8_t *out)
{
	...
	for (int32_t a = 0; a < count; a++) {
		...
		int32_t y0 = scratch->bbox[img * 4 + 0],
			y1 = scratch->bbox[img * 4 + 1];
		int32_t x0 = scratch->bbox[img * 4 + 2],
			x1 = scratch->bbox[img * 4 + 3];
	...
	for (int32_t r = 0; r < ARC_FRAME_SIZE; r++)
		memcpy(out + (size_t)r * ARC_FRAME_SIZE,
...
{
	if (scene->dirty)
		arc_scene_composite(scene, atlas, scratch, background,
				    scene->frame);
	scene->dirty = 0;
}
